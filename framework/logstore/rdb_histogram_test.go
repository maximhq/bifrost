...
func TestLatencyHistogram_LargeDatasetCorrectness(t *testing.T) {
	store := newTestSQLiteStore(t)
	base := time.Date(2026, 3, 23, 20, 0, 0, 0, time.UTC)

	// Insert 1000 rows with latency = i+1, TPS = i+1 to verify
	// the query handles large datasets correctly.
	logs := make([]Log, 1000)
	for i := range logs {
		lat := float64(i + 1) // 1..1000
		tps := float64(i + 1)
		logs[i] = Log{
			ID:              fmt.Sprintf("h-large-%04d", i),
	...
	// Insert in batches of 100 to stay within SQLite's SQL variable limit.
	for batchStart := 0; batchStart < len(logs); batchStart += 100 {
		batchEnd := batchStart + 100
		if batchEnd > len(logs) {
			batchEnd = len(logs)
		}
		mustCreateLogs(t, store, logs[batchStart:batchEnd])
	}
	...
	// N=1000, CEIL(0.9*1000)=900 => latency p90 = 900
	approxEqualFloat64(t, bucket.P90Latency, 900)
	// CEIL(0.95*1000)=950 => p95 = 950
	approxEqualFloat64(t, bucket.P95Latency, 950)
	// CEIL(0.99*1000)=990 => p99 = 990
	approxEqualFloat64(t, bucket.P99Latency, 990)

	if bucket.P90TokensPerSecond == nil {
		t.Fatalf("expected p90_tokens_per_second to be non-nil")
	}
	// TPS p90: N=1000, CEIL(0.9*1000)=900 => 900
	approxEqualFloat64(t, *bucket.P90TokensPerSecond, 900)

	if bucket.TotalRequests != 1000 {
		t.Fatalf("expected total_requests=1000, got %d", bucket.TotalRequests)
	}
}

func TestLatencyHistogram_TwoValuePercentileBoundary(t *testing.T) {
	store := newTestSQLiteStore(t)
	base := time.Date(2026, 3, 23, 21, 10, 0, 0, time.UTC)

	// Two values: [100, 200]. N=2.
	// p90: CEIL(0.9*2)=2 => MIN where rank>=2 => 200
	// p95: CEIL(0.95*2)=2 => 200
	// p99: CEIL(0.99*2)=2 => 200
	mustCreateLogs(t, store, []Log{
		{ID: "h-2val-1", Timestamp: base, Object: "chat_completion", Provider: "openai", Model: "gpt", Status: "success", Latency: floatPtr(100)},
		{ID: "h-2val-2", Timestamp: base.Add(1 * time.Second), Object: "chat_completion", Provider: "openai", Model: "gpt", Status: "success", Latency: floatPtr(200)},
	})
	...
	approxEqualFloat64(t, bucket.P90Latency, 200)
	approxEqualFloat64(t, bucket.P95Latency, 200)
	approxEqualFloat64(t, bucket.P99Latency, 200)
	approxEqualFloat64(t, bucket.AvgLatency, 150)
}

func TestLatencyHistogram_CrossMetricNullConsistency(t *testing.T) {
	store := newTestSQLiteStore(t)
	base := time.Date(2026, 3, 23, 22, 20, 0, 0, time.UTC)

	// Latency always present, TPS always null, TTFT sometimes present.
	// Ensures metrics don't leak across: TPS must be nil, TTFT must use only non-null values.
	mustCreateLogs(t, store, []Log{
		{ID: "h-cross-1", Timestamp: base, Object: "chat_completion", Provider: "openai", Model: "gpt", Status: "success", Latency: floatPtr(100), TimeToFirstToken: floatPtr(10)},
		{ID: "h-cross-2", Timestamp: base.Add(1 * time.Second), Object: "chat_completion", Provider: "openai", Model: "gpt", Status: "success", Latency: floatPtr(120)},
		{ID: "h-cross-3", Timestamp: base.Add(2 * time.Second), Object: "chat_completion", Provider: "openai", Model: "gpt", Status: "success", Latency: floatPtr(140), TimeToFirstToken: floatPtr(20)},
	})
	...
	// TPS: all null => must remain nil
	if bucket.AvgTokensPerSecond != nil {
		t.Fatalf("expected avg_tokens_per_second nil (all TPS null), got %v", *bucket.AvgTokensPerSecond)
	}
	if bucket.P90TokensPerSecond != nil {
		t.Fatalf("expected p90_tokens_per_second nil (all TPS null), got %v", *bucket.P90TokensPerSecond)
	}

	// TTFT: non-null set = [10, 20], N=2, CEIL(0.9*2)=2 => 20
	if bucket.AvgTimeToFirstToken == nil {
		t.Fatalf("expected avg_time_to_first_token to be non-nil")
	}
	approxEqualFloat64(t, *bucket.AvgTimeToFirstToken, 15)
	if bucket.P90TimeToFirstToken == nil {
		t.Fatalf("expected p90_time_to_first_token to be non-nil")
	}
	approxEqualFloat64(t, *bucket.P90TimeToFirstToken, 20)

	// Latency: all present, N=3
	approxEqualFloat64(t, bucket.AvgLatency, 120)
	// CEIL(0.9*3)=3 => p90 = 140
	approxEqualFloat64(t, bucket.P90Latency, 140)

	if bucket.TotalRequests != 3 {
		t.Fatalf("expected total_requests=3, got %d", bucket.TotalRequests)
	}
}
