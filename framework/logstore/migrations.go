...
var logstoreMigrationSteps = []migrationStep{
	...
	{IDs: []string{"logs_add_batch_embedding_input_column"}, run: migrationAddBatchEmbeddingInputColumn},
}
...
func migrationAddBatchEmbeddingInputColumn(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	migrationName := "logs_add_batch_embedding_input_column"
	logger.Info("[logstore] starting migration %s", migrationName)
	defer logger.Info("[logstore] finished migration %s", migrationName)
	opts := *migrator.DefaultOptions
	opts.UseTransaction = true
	m := migrator.New(db, &opts, []*migrator.Migration{{
		...
		Migrate: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			return addColumnIfNotExists(tx, logger, &Log{}, "batch_embedding_input")
		},
		Rollback: func(tx *gorm.DB) error {
			tx = tx.WithContext(ctx)
			return dropColumnIfExists(tx, logger, &Log{}, "batch_embedding_input")
		},
	}})
	if err := m.Migrate(); err != nil {
