...
func runEmbeddingInputMigrationCases(t *testing.T, db *gorm.DB, runMigration func(ctx context.Context) error) {
	...
	t.Run("Idempotent", func(t *testing.T) {
		...
		require.NoError(t, migrationAddEmbeddingInputColumn(ctx, db, testLogger{}), "first run")
		val1 := getEmbeddingInputValue(t, db, "emb-idem")

		require.NoError(t, migrationAddEmbeddingInputColumn(ctx, db, testLogger{}), "second run should be a no-op")
