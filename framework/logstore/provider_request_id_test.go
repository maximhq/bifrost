...
func TestProviderRequestIDPostgresIndexIsConcurrent(t *testing.T) {
	for _, index := range performanceIndexes {
		if index.name != "idx_logs_provider_request_id" {
			continue
		}
		require.Equal(t, "logs", index.table)
		require.True(t, strings.HasPrefix(index.sql, "CREATE INDEX CONCURRENTLY IF NOT EXISTS "), index.sql)
		require.Contains(t, index.sql, "ON logs(provider_request_id)")
		return
	}
	t.Fatal("idx_logs_provider_request_id is missing from performanceIndexes")
}
