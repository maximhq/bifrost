...
func (s *RDBLogStore) GetAvailableToolNames(ctx context.Context) []string {
	var toolNames []string
	s.db.WithContext(ctx).Model(&MCPToolLog{}).Distinct("tool_name").Pluck("tool_name", &toolNames)
	return toolNames
}

// GetAvailableServerLabels returns all unique server labels from the MCP tool logs.
func (s *RDBLogStore) GetAvailableServerLabels(ctx context.Context) []string {
	var serverLabels []string
	s.db.WithContext(ctx).Model(&MCPToolLog{}).Distinct("server_label").Where("server_label != ''").Pluck("server_label", &serverLabels)
	return serverLabels
}
