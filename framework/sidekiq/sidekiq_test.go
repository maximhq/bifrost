...
func TestCompleteDoesNotOverwriteReapedFailure(t *testing.T) {
	...
	if ok, err := store.ClaimSidekiqJob(context.Background(), "stale", "owner-A", staleBefore); err != nil || !ok {
	...
	if err := store.CompleteSidekiqJob(context.Background(), "stale", "owner-A", "final"); err == nil {
...
// TestInflightDedupPreventsDoubleSpawn verifies that repeated dispatchOnce calls for
// the same pending job (e.g. two ticks before a semaphore slot frees) do not create
...
func TestInflightDedupPreventsDoubleSpawn(t *testing.T) {
	...
	// Occupy the sole semaphore slot.
	...
	r.dispatchOnce() // claims the slot with "blocker"
	time.Sleep(20 * time.Millisecond) // let it enter the handler

	// Now enqueue "waiter" — its goroutine parks on the semaphore.
	store.seed("waiter", "k", tables.SidekiqStatusPending, 0)
	r.dispatchOnce() // first spawn for waiter
	r.dispatchOnce() // second dispatch tick — must not spawn a duplicate
	r.dispatchOnce() // third — still no duplicate

	// Release the blocker so waiter can proceed.
	close(gate)
