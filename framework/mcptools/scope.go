...
// read regardless of what a tool asks for - so widening a query can never
// surface data the caller could not fetch from the logs API directly. What
// this solves is a different failure: on a deployment serving many teams and
// customers, "what did we spend last week?" has several correct answers, and
// silently picking the widest one produces a confident number about the wrong
// thing.
//
// The rule is:
//
//   - When the caller has opted in with a default user, their own traffic is
//     the default. That is the question people usually mean from the
//     dashboard, and it is the one they can always check.
//   - When there is no default, there is no narrowing. The tool answers over
//     whatever the row-level scope already permits.
//   - An explicit scope in the question always wins over the default. Asking
//     about another team is a legitimate question; the store decides whether
//     the answer is allowed.
//
// The default is an explicit opt-in owned by this package, not an inference
// from schemas.BifrostContextKeyUserID. Warp's dashboard transport opts in
// (see WithDefaultUserScope); the /mcp gateway never does, so an external
// harness whose OAuth token happens to carry a user id is not silently
// narrowed by user on top of the tenant scope it was admitted with.

// defaultUserScopeKey is the context key WithDefaultUserScope writes.
// Unexported so nothing outside this package can set it without going through
// the helper.
type defaultUserScopeKey struct{}

// WithDefaultUserScope returns a context whose tool calls default to userID's
// own traffic when the question names no scope of its own. An empty userID
// returns ctx unchanged.
func WithDefaultUserScope(ctx context.Context, userID string) context.Context {
	if userID == "" {
		return ctx
	}
	return context.WithValue(ctx, defaultUserScopeKey{}, userID)
}

// DefaultUserScope reads the opt-in WithDefaultUserScope wrote. Empty when the
// caller never opted in.
func DefaultUserScope(ctx context.Context) string {
	userID, _ := ctx.Value(defaultUserScopeKey{}).(string)
	return userID
}

// Scope is the resolved default for one tool call.
type Scope struct {
	// HasIdentity reports whether a default user was opted in.
	HasIdentity bool
	UserID      string
}

// scopeFromContext derives the caller's default scope from the opt-in alone.
func scopeFromContext(ctx context.Context) Scope {
	userID := DefaultUserScope(ctx)
	if userID == "" {
		return Scope{}
	}
...
// no scope of its own.
//
// "Named no scope" means every dimension is empty. A question that mentions
// any one of them is taken as deliberate and left alone - narrowing "how did
// team X do?" to the asker's own traffic would answer a question nobody asked,
...
// filtersNameAScope reports whether the caller asked about a particular
// slice of traffic.
//
...
// or "all" (whatever the row-level scope permits).
//
...
// purpose: the consuming prompt spells out what each of the three means, and
// restating that on every result would be the same paragraph paid for again
// on every call, replayed for the rest of the conversation.
