...
func newEvictableAuthServer(t *testing.T) *evictableAuthServer {
	t.Helper()
	as := &evictableAuthServer{known: map[string]bool{}}
...
func TestReregister_RecoversFromEvictedRegistration(t *testing.T) {
	...
	previousClientID, newClientID, err := provider.ReregisterDynamicClient(ctx, oauthConfigID)
	require.NoError(t, err)
...
func TestReregisterDynamicClient_CascadesEveryBoundToken(t *testing.T) {
	...
	_, _, err := provider.ReregisterDynamicClient(ctx, oauthConfigID)
	require.NoError(t, err)
...
func TestReregisterDynamicClient_UnknownConfig_Errors(t *testing.T) {
	...
	_, _, err := provider.ReregisterDynamicClient(context.Background(), "does-not-exist")
