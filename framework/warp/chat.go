...
func (s *Service) RunTurn(ctx context.Context, turn *Turn, sink func(Event) bool) ChatResponse {
	...
	agent := NewAgent(turn.chat, s.costFuncFor(turn.config), s.logs, s.governance, ScopeFromContext(runCtx), turn.config, turn.utcOffsetMinutes, turn.timezone, s.semantic)
