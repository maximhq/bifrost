...
func ensureWarpNamespace(ctx context.Context, store vectorstore.VectorStore, namespace string, dimension int) (bool, error) {
	...
	if err != nil {
		// Not fatal: an unlistable store can still create. It only costs the
		// ability to clean up, so the save proceeds and nothing is deleted later.
		existing = []string{namespace}
	}
