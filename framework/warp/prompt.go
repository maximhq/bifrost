...
- query_usage_by and query_model_performance already return a trend against the immediately preceding period of equal length on every row (has_previous_period, requests_trend, tokens_trend, cost_trend). Read that off the result you already have instead of calling again to check direction. For a plain total or time series, query_metrics's compare_to_previous does the same in one call.
...
- When nobody is identified there is no sensible default. Call ask_user with the teams, customers and business units describe_filter_space reported as options, rather than asking in prose or writing the choices out as a list in your answer - only ask_user renders as something the person can click. Asking one short question beats answering the wrong one.
...
func sanitizeTimezone(name string) string {
	...
	// "Local" is a special case time.LoadLocation accepts (per its own docs)
	// as an alias for time.Local - the server process's own OS-configured
	// zone, not a real IANA identifier. It would load without error and read
	// exactly like a legitimate zone name, but it names wherever this server
	// happens to be deployed, never the asker's - the opposite of what this
	// value exists to carry. UTC is excluded the same way just above, for the
	// same reason: it carries nothing an offset of zero doesn't already say.
