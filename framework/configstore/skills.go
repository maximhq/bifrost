...
func runPendingSkillObjectWrites(ctx context.Context, objectStore objectstore.ObjectStore, writes []pendingSkillObjectWrite) error {
	if objectStore == nil || len(writes) == 0 {
		return nil
	}
	for _, write := range writes {
		if err := objectStore.Put(ctx, write.key, write.data, write.metadata); err != nil {
			return fmt.Errorf("failed to write skill file object %s: %w", write.key, err)
		}
	}
	return nil
}
...
const (
	...
	// maxSkillFileContentSize is the maximum allowed size for inline text or
	// dataurl file content (10 MB). Uploads go directly to object storage and
	// are not subject to this limit.
	maxSkillFileContentSize = 10 * 1024 * 1024
)
...
func validateSkillSourceType(sourceType string) error {
	switch sourceType {
	case tables.SkillSourceTypeURL, tables.SkillSourceTypeDataURL, tables.SkillSourceTypeFilepath, tables.SkillSourceTypeText, tables.SkillSourceTypeUpload:
		return nil
	default:
		return fmt.Errorf("skill file source_type must be one of url, dataurl, filepath, text, or upload")
	}
}
...
func parseSkillSemver(version string) (skillVersionParts, error) {
	matches := skillSemverPattern.FindStringSubmatch(strings.TrimSpace(version))
	if matches == nil {
...
func StoreSkillFileContent(ctx context.Context, tx *gorm.DB, objectStore objectstore.ObjectStore, skillID string, file *tables.TableSkillFile, data []byte) (*pendingSkillObjectWrite, error) {
	...
	if objectStore != nil {
		...
		if key == nil || strings.TrimSpace(*key) == "" {
			generated := fmt.Sprintf("skills/%s/files/%s/%s", skillID, file.ID, path.Base(file.Path))
			key = &generated
		}
		file.StorageKey = key
		file.BlobID = nil
...
func validateSkillFileReference(tx *gorm.DB, skillID string, file *tables.TableSkillFile) error {
	...
	if file.StorageKey != nil && strings.TrimSpace(*file.StorageKey) != "" {
		...
		ownPrefix := fmt.Sprintf("skills/%s/files/", skillID)
		if !strings.HasPrefix(key, ownPrefix) {
			if file.UploadID == nil || strings.TrimSpace(*file.UploadID) == "" {
				return fmt.Errorf("storage_key %q is not scoped to skill %q and requires upload_id", key, skillID)
			}
			uploadPrefix := fmt.Sprintf("skills/uploads/%s/", strings.TrimSpace(*file.UploadID))
			if !strings.HasPrefix(key, uploadPrefix) {
				return fmt.Errorf("storage_key %q does not match upload_id %q", key, strings.TrimSpace(*file.UploadID))
			}
			if path.Base(key) != path.Base(file.Path) {
				return fmt.Errorf("storage_key %q does not match file path %q", key, file.Path)
			}
		}
...
func validateSkillFileSource(file *tables.TableSkillFile) ([]byte, error) {
	...
	case tables.SkillSourceTypeFilepath:
		if file.SourceFilepath == nil || strings.TrimSpace(*file.SourceFilepath) == "" {
			return nil, fmt.Errorf("filepath source_type requires source_filepath")
		}
		if !filepath.IsAbs(*file.SourceFilepath) {
			return nil, fmt.Errorf("source_filepath must be absolute")
		}
		file.StorageKey = nil
		file.BlobID = nil
		return nil, nil
	case tables.SkillSourceTypeText:
		if file.InlineContent == nil || *file.InlineContent == "" {
			if file.StorageKey != nil || file.BlobID != nil {
				return nil, nil
			}
...
func (s *RDBConfigStore) CreateSkill(ctx context.Context, skill *tables.TableSkill, version string, objectStore objectstore.ObjectStore) error {
	...
	if err := s.DB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := bumpAllSkillsVersionTx(ctx, tx, allSkillsVersionBumpMinor, existingSkillCount == 0); err != nil {
			return err
		}
	...
	return runPendingSkillObjectWrites(ctx, objectStore, objectWrites)
}
...
func (s *RDBConfigStore) ListSkillVersions(ctx context.Context, skillID string, limit, offset int) ([]tables.TableSkillVersion, int64, error) {
	var total int64
...
func (s *RDBConfigStore) UpdateSkill(ctx context.Context, skill *tables.TableSkill, version string, serve bool, objectStore objectstore.ObjectStore) error {
	...
	if err := s.DB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if serve {
			// Update skill row to serve this version.
			skill.LatestVersion = version
			if err := tx.Model(&tables.TableSkill{}).
				Where("id = ?", skill.ID).
				Select("Description", "License", "Compatibility", "Metadata", "ExtraFrontmatter", "AllowedTools", "SkillMDBody", "LatestVersion", "CreatedBy", "ConfigHash", "UpdatedAt").
				Updates(skill).Error; err != nil {
				return err
			}
		}
		...
		if serve {
			bump, err := allSkillsBumpForVersionChange(existing.LatestVersion, version)
			if err != nil {
				return err
			}
			if err := bumpAllSkillsVersionTx(ctx, tx, bump, false); err != nil {
				return err
			}
		}
...
func (s *RDBConfigStore) ListSkills(ctx context.Context, params SkillListQueryParams) ([]tables.TableSkill, int64, error) {
	...
	query = query.Omit("skill_md_body").Order("created_at DESC")
	if params.Limit > 0 {
