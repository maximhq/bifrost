...
// final metadata (counts, summary). Fenced on owner_id AND status = running so a
// job that was reaped and re-claimed elsewhere is not marked complete by its former
// owner, and — critically — so a job the reaper already flipped to failed (because
// this owner ran past the stale threshold) is not silently resurrected to completed,
...
// Fenced on owner_id AND status = running so a former owner cannot overwrite a
