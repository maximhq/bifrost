...
type TablePromptVersion struct {
	...
	// Relationships
	Messages []TablePromptVersionMessage `gorm:"foreignKey:VersionID" json:"messages,omitempty"`
}
...
func (v *TablePromptVersion) AfterFind(tx *gorm.DB) error {
	if v.ModelParamsJSON != nil && *v.ModelParamsJSON != "" {
		if err := json.Unmarshal([]byte(*v.ModelParamsJSON), &v.ModelParams); err != nil {
			return err
		}
	}
	return nil
