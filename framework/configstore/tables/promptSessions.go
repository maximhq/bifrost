...
type TablePromptSession struct {
	...
	Version         *TablePromptVersion `gorm:"foreignKey:VersionID" json:"version,omitempty"`
	...
	// Relationships
	Messages []TablePromptSessionMessage `gorm:"foreignKey:SessionID" json:"messages,omitempty"`
}
...
func (s *TablePromptSession) AfterFind(tx *gorm.DB) error {
	if s.ModelParamsJSON != nil && *s.ModelParamsJSON != "" {
		if err := json.Unmarshal([]byte(*s.ModelParamsJSON), &s.ModelParams); err != nil {
			return err
		}
	}
	return nil
