...
// OdinConfigRowID is the primary key of the one and only Odin config row.
// Odin is deployment-wide, so the table is a singleton: writes upsert this ID
// rather than inserting, which keeps "which row is current?" from ever being a
// question the read path has to answer.
const OdinConfigRowID uint = 1

// TableOdinConfig stores the dashboard agent's model settings.
//
...
// TableName sets the table name for the Odin config model.
func (TableOdinConfig) TableName() string { return "odin_config" }
