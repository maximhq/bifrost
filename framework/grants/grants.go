...
const (
	...
	// LimitHolderProjectModelConfig is a per-model limit a project imposes on requests running
	// inside it. It is not the caller's, and not the deployment's: the same caller reaching the
	// same model outside the project answers to neither.
