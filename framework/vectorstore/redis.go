...
// knnScoreAlias is the alias FT.SEARCH assigns to the KNN distance
// ("... AS score"). It is also what GetNearest sorts and returns on, which is
// why the two must stay in sync - see isKNNSortByUnsupported.
const knnScoreAlias = "score"

// isKNNSortByUnsupported reports whether errMsg (already lowercased) means the
// backend refused to sort on the KNN result alias, so the query should be
// retried without SORTBY.
//
// The alias is not an index attribute, and backends disagree on whether SORTBY
// may name it. RediSearch accepts it. valkey-search resolves SORTBY against the
// index schema only and rejects the whole command, which made semantic cache
// lookups fail permanently on Valkey deployments. The two report it very
// differently, so both wordings are matched here:
//
...
func (s *RedisStore) GetNearest(ctx context.Context, namespace string, vector []float32, queries []Query, selectFields []string, threshold float64, limit int64) ([]SearchResult, error) {
	...
	args := []interface{}{
		"FT.SEARCH", namespace,
		fmt.Sprintf("%s=>[KNN %d @embedding $vec AS %s]", hybridQuery, knnLimit, knnScoreAlias),
		"PARAMS", "2", "vec", queryBytes,
	}

	// Remember where the SORTBY clause starts so the compat retry below can drop
	// exactly these two arguments. Searching args for the literal instead would
	// also match a caller-supplied value: namespace and the RETURN field names go
	// into the same slice, and one of them equalling "SORTBY" would make the retry
	// skip an unrelated argument pair and send a malformed command.
	sortByAt := len(args)
	args = append(args, "SORTBY", knnScoreAlias)
	...
	returnFields := []string{knnScoreAlias}
	if len(selectFields) > 0 {
	...
	if result.Err() != nil {
		...
		if isKNNSortByUnsupported(errMsg) {
			...
			compatArgs := make([]interface{}, 0, len(args)-2)
			compatArgs = append(compatArgs, args[:sortByAt]...)
			compatArgs = append(compatArgs, args[sortByAt+2:]...)
			result = s.client.Do(ctx, compatArgs...)
		}
		if result.Err() != nil {
	...
	for _, result := range results {
		...
		if scoreValue, exists := result.Properties[knnScoreAlias]; exists {
