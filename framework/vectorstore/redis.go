...
type RedisConfig struct {
	...
	Dimension int `json:"dimension,omitempty"` // Dimension for vector search (optional)
}
...
func (s *RedisStore) CreateNamespace(ctx context.Context, namespace string, properties map[string]VectorStoreProperties) error {
	...
	if s.config.Dimension <= 0 {
		return fmt.Errorf("redis vector index %q: dimension must be > 0 (got %d)", namespace, s.config.Dimension)
	}
...
func (s *RedisStore) GetNearest(ctx context.Context, namespace string, vector []float32, queries []Query, selectFields []string, threshold float64, limit int64) ([]SearchResult, error) {
	...
	if s.config.Dimension > 0 && len(vector) != s.config.Dimension {
		return nil, fmt.Errorf("query vector dim %d != configured dim %d", len(vector), s.config.Dimension)
	}
...
func (s *RedisStore) Add(ctx context.Context, namespace string, id string, embedding []float32, metadata map[string]interface{}) error {
	...
	if len(embedding) > 0 {
		if s.config.Dimension > 0 && len(embedding) != s.config.Dimension {
			return fmt.Errorf("embedding dim %d != configured dim %d", len(embedding), s.config.Dimension)
		}
...
func (s *RedisStore) deleteAllWithCursor(ctx context.Context, namespace string, queries []Query, cursor *string) ([]DeleteResult, error) {
	...
	batchSize := 100 // Process in batches to avoid overwhelming Redis
...
func (s *RedisStore) DeleteNamespace(ctx context.Context, namespace string) error {
	...
	// First, delete all keys with the namespace prefix
	...
	for {
		keys, newCursor, err := s.client.Scan(ctx, cursor, pattern, 1000).Result()
		if err != nil {
...
func (s *RedisStore) Close(ctx context.Context, namespace string) error {
	// Close should perform the same cleanup as DeleteNamespace
	// Delete all data and drop the index
