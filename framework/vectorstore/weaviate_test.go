	...
	tests := []struct {
		...
		},
		{
			...
			queries: []Query{
				...
				{Field: "count", Operator: "LessThan", Value: int64(100)},
				{Field: "active", Operator: "Equal", Value: true},
				{Field: "score", Operator: "GreaterThanEqual", Value: 95.5},
			},
			isNil: false,
		},
	}
...
func TestConvertOperator(t *testing.T) {
	tests := []struct {
		...
		{"UnknownOperator", filters.Equal}, // Default to Equal
		{"", filters.Equal},                // Default to Equal
	}
...
func TestWeaviateStore_FilteringScenarios(t *testing.T) {
	...
	t.Run("Filter by string equality", func(t *testing.T) {
		...
		results, cursor, err := setup.Store.GetAll(setup.ctx, queries, nil, 10)
		require.NoError(t, err)
	...
	t.Run("Filter by numeric comparison", func(t *testing.T) {
		...
		results, _, err := setup.Store.GetAll(setup.ctx, queries, nil, 10)
		require.NoError(t, err)
	...
	t.Run("Pagination test", func(t *testing.T) {
		...
		results, cursor, err := setup.Store.GetAll(setup.ctx, nil, nil, 2)
		require.NoError(t, err)
		assert.Len(t, results, 2)

		if cursor != nil {
			// Get next page
			nextResults, _, err := setup.Store.GetAll(setup.ctx, nil, cursor, 2)
			require.NoError(t, err)
...
func TestWeaviateStore_DeleteOperations(t *testing.T) {
	...
	t.Run("Single delete", func(t *testing.T) {
		// Delete one object
		err := setup.Store.Delete(setup.ctx, []string{testKeys[0]})
		require.NoError(t, err)
	...
	t.Run("Batch delete", func(t *testing.T) {
		...
		err := setup.Store.Delete(setup.ctx, testKeys[1:])
		require.NoError(t, err)
	...
	t.Run("Delete non-existent keys", func(t *testing.T) {
		...
		err := setup.Store.Delete(setup.ctx, []string{"non-existent-1", "non-existent-2"})
		assert.NoError(t, err) // Weaviate typically doesn't error on missing deletes
	})
}
...
func TestWeaviateStore_EdgeCases(t *testing.T) {
	...
	t.Run("Boundary conditions for similarity search", func(t *testing.T) {
		...
		results, err := setup.Store.GetNearest(setup.ctx, testEmbedding, nil, 0.0, 10)
		require.NoError(t, err)
		assert.GreaterOrEqual(t, len(results), 1)

		// Test with threshold = 1 (should match very little)
		results, err = setup.Store.GetNearest(setup.ctx, testEmbedding, nil, 1.0, 10)
		require.NoError(t, err)
		// Should still find exact match
		assert.GreaterOrEqual(t, len(results), 1)

		// Test with limit = 0
		results, err = setup.Store.GetNearest(setup.ctx, testEmbedding, nil, 0.5, 0)
		require.NoError(t, err)
...
func TestWeaviateStore_CompleteUseCases(t *testing.T) {
	...
	t.Run("Document Storage & Retrieval Scenario", func(t *testing.T) {
		...
		results, _, err = setup.Store.GetAll(setup.ctx, combinedQuery, nil, 10)
		require.NoError(t, err)
		assert.Len(t, results, 2) // doc1, doc3

		// Vector similarity search
		queryEmbedding := documents[0].embedding // Similar to doc1
		vectorResults, err := setup.Store.GetNearest(setup.ctx, queryEmbedding, nil, 0.8, 10)
		require.NoError(t, err)
	...
	t.Run("User Content Management Scenario", func(t *testing.T) {
		...
		results, _, err := setup.Store.GetAll(setup.ctx, aliceQuery, nil, 10)
		require.NoError(t, err)
		...
		results, _, err = setup.Store.GetAll(setup.ctx, techEnQuery, nil, 10)
		require.NoError(t, err)
		...
		vectorResults, err := setup.Store.GetNearest(setup.ctx, queryEmbedding, aliceFilter, 0.5, 10)
		require.NoError(t, err)
	...
	t.Run("Semantic Cache-like Workflow", func(t *testing.T) {
		...
		results, _, err := setup.Store.GetAll(setup.ctx, hashQuery, nil, 10)
		require.NoError(t, err)
		...
		vectorResults, err := setup.Store.GetNearest(setup.ctx, similarEmbedding, userLangFilter, 0.7, 10)
		require.NoError(t, err)
...
func TestWeaviateStore_Performance(t *testing.T) {
	...
	t.Run("Vector search performance", func(t *testing.T) {
		...
		for i := 0; i < numSearches; i++ {
			_, err := setup.Store.GetNearest(setup.ctx, queryEmbedding, nil, 0.8, 10)
			require.NoError(t, err)
		}
		duration := time.Since(start)
