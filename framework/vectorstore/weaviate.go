...
func (s *WeaviateStore) GetChunk(ctx context.Context, id string) (any, error) {
...
func (s *WeaviateStore) GetChunks(ctx context.Context, ids []string) ([]any, error) {
...
func (s *WeaviateStore) GetAll(ctx context.Context, queries []Query, selectFields []string, cursor *string, count int64) ([]any, *string, error) {
...
func (s *WeaviateStore) GetNearest(
	...
	for _, o := range objs {
		...
		// Safely extract distance/score with default value
		var score float64
		if distanceRaw, exists := additional["distance"]; exists && distanceRaw != nil {
			switch v := distanceRaw.(type) {
			case float64:
				score = v
			case float32:
				score = float64(v)
