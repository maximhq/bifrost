...
func tieredCacheCreationInputTokenRate(pricing *configstoreTables.TableModelPricing, totalTokens int, tier serviceTier) float64 {
	...
	// Priority is the flat ≤272k rate; larger contexts already returned the long-context rate above (mirrors the sibling rate functions; OpenAI publishes no priority long-context cache-write rate).
