...
func (mc *ModelCatalog) computeModelsForProvider(provider schemas.ModelProvider) []string {
	...
	rule := mc.keyconf.ModelRuleFor(provider)
...
func (mc *ModelCatalog) appendAllowedDatasheetModels(out []string, models []string, restricted bool, rule schemas.ModelRule, providerName string) []string {
	if len(models) == 0 {
		return out
	}
	seen := make(map[string]struct{}, len(out))
...
func (mc *ModelCatalog) computeProvidersForModel(model string) []schemas.ModelProvider {
	...
	for _, p := range mc.keyconf.Providers() {
		...
		rule := mc.keyconf.ModelRuleFor(p)
