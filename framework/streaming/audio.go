...
func (a *Accumulator) processAccumulatedAudioStreamingChunks(requestID string, bifrostErr *schemas.BifrostError, isFinalChunk bool) (*AccumulatedData, error) {
	...
	if len(accumulator.AudioStreamChunks) > 0 {
		...
		if lastChunk.TokenUsage != nil {
			...
			if lastChunk.TokenUsage.InputTokensDetails != nil {
				data.TokenUsage.TokenDetails = &schemas.TokenDetails{
					AudioTokens: lastChunk.TokenUsage.InputTokensDetails.AudioTokens,
				}
			}
		}
	}
	...
	if result != nil {
		if isFinalChunk {
			if a.pricingManager != nil {
				cost := a.pricingManager.CalculateCostWithCacheDebug(result, provider, model, schemas.SpeechStreamRequest)
