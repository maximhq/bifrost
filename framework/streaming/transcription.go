...
func (a *Accumulator) processAccumulatedTranscriptionStreamingChunks(requestID string, bifrostErr *schemas.BifrostError, isFinalChunk bool) (*AccumulatedData, error) {
	...
	if len(accumulator.TranscriptionStreamChunks) > 0 {
		...
		if lastChunk.TranscriptionUsage != nil {
			...
			if lastChunk.TranscriptionUsage.InputTokenDetails != nil {
				data.TokenUsage.TokenDetails = &schemas.TokenDetails{
					AudioTokens: lastChunk.TranscriptionUsage.InputTokenDetails.AudioTokens,
				}
			}
		}
	}
	...
	if result != nil {
		if isFinalChunk {
			if a.pricingManager != nil {
				cost := a.pricingManager.CalculateCostWithCacheDebug(result, provider, model, schemas.TranscriptionStreamRequest)
