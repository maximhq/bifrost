...
// dead-session check and, more importantly, why both have to exist.
//
...
func TestIsDeadSessionError_TypedSentinelAndTextFallback(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			// chain, so the text is the only thing to go on.
			name: "non-404 server is caught by text alone",
			err:  errors.New("transport error: request failed with status 422: Unexpected message, expect initialize request"),
			want: true,
		},
		{
			name: "an ordinary transport failure is not a dead session",
			err:  errors.New("transport error: failed to send request: connection reset by peer"),
			want: false,
		},
		{
