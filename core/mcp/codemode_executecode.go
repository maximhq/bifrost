...
func (h *ToolsHandler) createExecuteToolCodeTool() schemas.ChatTool {
	executeToolCodeProps := map[string]interface{}{
		"code": map[string]interface{}{
			"type":        "string",
			"description": "TypeScript code to execute. The code will be transpiled to JavaScript and validated before execution. Import/export statements will be stripped. You can use async/await syntax for async operations. Example: const result = await serverName.toolName({arg: 'value'}); console.log(result); return result;",
		},
	}
...
func (h *ToolsHandler) handleExecuteToolCode(ctx context.Context, toolCall schemas.ChatAssistantMessageToolCall) (*schemas.ChatMessage, error) {
	toolName := "unknown"
...
func (h *ToolsHandler) executeCode(ctx context.Context, code string) ExecutionResult {
	logs := []string{}
	strippedLines := []int{}
	...
	timeoutCtx, cancel := context.WithTimeout(ctx, h.toolExecutionTimeout)
	defer cancel()
		...
		for toolName, binding := range tools {
			...
			serverObj.Set(toolName, func(call goja.FunctionCall) goja.Value {
				...
				go func() {
					...
					result, err := h.callMCPTool(timeoutCtx, clientNameFinal, toolNameFinal, argsMap, appendLog)
...
func (h *ToolsHandler) callMCPTool(ctx context.Context, clientName, toolName string, args map[string]interface{}, appendLog func(string)) (interface{}, error) {
	...
	for _, tool := range tools {
		if tool.Function != nil && tool.Function.Name != "" {
			client = h.clientManager.GetClientForTool(tool.Function.Name)
			if client != nil {
				break
			}
		}
