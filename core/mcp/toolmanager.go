...
func (m *ToolsManager) executeToolInternal(ctx *schemas.BifrostContext, toolCall *schemas.ChatAssistantMessageToolCall) (*schemas.ChatMessage, string, string, error) {
	...
	if client.ExecutionConfig.AuthType == schemas.MCPAuthTypePerUserOauth {
		...
		if sessionToken == "" {
			// No session token — the user hasn't authenticated with this upstream provider yet.
			// Return an error with the URL to initiate upstream OAuth via Bifrost's proxy.
			...
			// Extract base URL from redirect URI (strip /api/oauth/callback)
			baseURL := strings.TrimSuffix(redirectURI, "/api/oauth/callback")
			// Build upstream authorize URL pointing to Bifrost's upstream OAuth proxy
			authorizeURL := fmt.Sprintf("%s/api/oauth/per-user/upstream/authorize?mcp_client_id=%s&session=%s",
				baseURL, client.ExecutionConfig.ID, "")
		...
		// Have session → look up upstream token by identity (persistent) or session (fallback)
