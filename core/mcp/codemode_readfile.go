...
// createReadToolFileTool creates the readToolFile tool definition
func (m *CodeModeToolsHandler) createReadToolFileTool() schemas.ChatTool {
	readToolFileProps := map[string]interface{}{
		...
		"startLine": map[string]interface{}{
			"type":        "number",
			"description": "Optional 1-based starting line number for partial file read (inclusive)",
		},
		...
		Function: &schemas.ChatToolFunction{
			...
			Description: schemas.Ptr(
				"Reads a virtual .d.ts declaration file for a specific MCP server, generating TypeScript type definitions " +
					...
					"to read only a portion of the file. This tool generates pseudo declaration files that describe the available " +
...
func (m *CodeModeToolsHandler) generateTypeDefinitions(clientName string, tools []schemas.ChatTool) string {
	...
	for _, tool := range tools {
		...
		// Parse tool name for JavaScript compatibility (used in virtual TypeScript files)
		...
		inputInterfaceName := m.toPascalCase(toolName) + "Input"
		sb.WriteString("// ----------------------------------------------------------------------------\n")
		...
		if tool.Function.Parameters != nil && tool.Function.Parameters.Properties != nil {
			...
			for _, propName := range propNames {
				...
				tsType := m.jsonSchemaToTypeScript(propMap)
				optional := ""
		...
		responseInterfaceName := m.toPascalCase(toolName) + "Response"
...
// jsonSchemaToTypeScript converts a JSON Schema type to TypeScript type
func (m *CodeModeToolsHandler) jsonSchemaToTypeScript(prop map[string]interface{}) string {
	...
	if typeVal, ok := prop["type"].(string); ok {
		...
		case "array":
			...
			if items, ok := prop["items"].(map[string]interface{}); ok {
				itemsType = m.jsonSchemaToTypeScript(items)
			}
...
func (m *CodeModeToolsHandler) toPascalCase(s string) string {
	if s == "" {
		return s
	}
