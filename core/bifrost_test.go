	...
	ma.keys[schemas.OpenAI] = []schemas.Key{
		{
			...
			Value:  *schemas.NewSecretVar(fmt.Sprintf("sk-test-%s", schemas.ModelFamilyOpenAI)),
			Weight: 100,
		},
	}
	return ma
	...
	t.Run("scale up test", func(t *testing.T) {
		for _, test := range tests {
			...
			t.Run(test.Name, func(t *testing.T) {
				...
				// Move dummyReq to the outer scope so it can be drained later
				...
				// Drain the channels to unblock the deadlocked workers
				// so they can safely exit and allow Shutdown() to complete.
				go func() {
					for {
						select {
						case <-dummyReq.Err:
