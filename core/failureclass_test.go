...
func TestFailureClass_Policy(t *testing.T) {
	...
	// A location block is the gateway's, shared by every key: labelled, never rotated.
	if schemas.FailureClassRegionBlocked.IsPerKey() || schemas.FailureClassRegionBlocked.FailReason() != "region_blocked_error" {
		t.Errorf("region_blocked should carry its label and no per-key policy")
	}
