...
func TestExecuteRequestWithRetriesCapturesStreamingHeaderBeforeFirstChunk(t *testing.T) {
	...
	// The current ID and its attempt record must be published atomically before
	// CheckFirstStreamChunkForError blocks. Otherwise a fast provider goroutine
	// can finish its logging post-hook here and persist an empty trail.
	trail, _ := ctx.Value(schemas.BifrostContextKeyProviderRequestIDTrail).([]schemas.ProviderRequestIDRecord)
	if len(trail) != 1 || trail[0].RequestID != "req-stream-early" || trail[0].Attempt != 0 {
		t.Fatalf("provider request ID trail was not captured before the first stream chunk: %#v", trail)
	}
