...
// ProviderQueue struct maintains ActiveWorkers integer which is the no. of workers that are active.
// It also has a quit channel which is used to send quit signals that are used to scale the workers down.
// TODO: Now just write the nc.Copy things in the provider.go files!!
