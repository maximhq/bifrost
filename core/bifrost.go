...
//     key cannot reach, an exhausted quota, a retired model). These are NEVER
...
func executeRequestWithRetries[T any](
	...
	// credential rejected) from a model problem (no key serves it) when choosing what to
	// return, and the last error that was about the model rather than the key, which is
	// the one worth returning when both kinds are in the pool.
	var deadKeyClasses map[string]schemas.FailureClass
	var lastModelError *schemas.BifrostError
	...
	for attempts = 0; attempts <= config.NetworkConfig.MaxRetries+extraAttempts; attempts++ {
		...
		if keyProvider != nil && (attempts == 0 || lastWasPerKeyFailure) {
			...
			if err != nil {
				...
				if errors.Is(err, errAllKeysFiltered) {
					// A model error stays a model error, as it does when the pool runs out.
					if lastModelError != nil {
						return zero, lastModelError
					}
				...
				if errors.Is(err, errAllKeysDead) {
					...
					// is the fact the caller can act on, whichever key died last: a 404 for
					// a model no key serves must stay a 404. Otherwise every key died of an
					// account-level failure, and within the configured budget that collapses
					// into the synthetic 502, since the raw 401 would falsely blame the
					// caller's own key.
					if lastModelError != nil {
						return zero, lastModelError
					}
		...
		if attempts > 0 {
			...
			// a model this key cannot reach, an exhausted quota) where waiting offers nothing
			// against a dead key.
			//
		...
		if isPerKeyFailure && keyProvider != nil {
			...
			if isPermanentKeyFailure {
				...
				if class == schemas.FailureClassModelAccess || class == schemas.FailureClassModelGone {
					lastModelError = bifrostError
				}
			} else {
				if usedKeyIDs == nil {
					usedKeyIDs = make(map[string]bool)
