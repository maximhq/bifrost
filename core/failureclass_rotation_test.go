...
func TestExecuteRequestWithRetries_KeylessProviderDoesNotRetryPermanentFailure(t *testing.T) {
	...
	handler := func(k schemas.Key) (string, *schemas.BifrostError) {
		calls++
		return "", providerError(404, "invalid_request_error", "model_not_found", "The model `x` does not exist or you do not have access to it.")
	}
...
// A region block is about the caller's location, which every key shares: one attempt, no
// walk through the pool, and the trail still says what happened.
func TestExecuteRequestWithRetries_RegionBlockDoesNotWalkPool(t *testing.T) {
	ctx := rotationTestContext()
	calls := 0
	handler := func(k schemas.Key) (string, *schemas.BifrostError) {
		calls++
		return "", providerError(400, "FAILED_PRECONDITION", "400", "User location is not supported for the API use.")
	}
	_, err := executeRequestWithRetries(ctx, createTestConfig(3, 0, 0), handler,
		poolKeyProvider([]schemas.Key{rotationKeyA, rotationKeyB}),
	...
	trail := attemptTrail(t, ctx, 1)
	if trail[0].FailureClass != schemas.FailureClassRegionBlocked || trail[0].FailReason == nil || *trail[0].FailReason != "region_blocked_error" || trail[0].TriggeredRotation {
		t.Errorf("record = %+v, want region_blocked / region_blocked_error, no rotation", trail[0])
	}
}
