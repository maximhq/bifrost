...
func RunToolCallsStreamingTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	WrapTestScenario(t, client, ctx, testConfig, "ToolCallsStreamingChatCompletions", ModelTypeChat, func(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
		return runToolCallsStreamingChatCompletionsHelper(t, client, ctx, testConfig)
	})

	// Test Responses API streaming with tool calls
	WrapTestScenario(t, client, ctx, testConfig, "ToolCallsStreamingResponses", ModelTypeChat, func(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
		return runToolCallsStreamingResponsesHelper(t, client, ctx, testConfig)
	})
}

func runToolCallsStreamingChatCompletionsHelper(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	...
	if err != nil {
		return fmt.Errorf("chat completion stream with tools failed: %v", err)
	}
	...
	if len(finalToolCalls) == 0 {
		return fmt.Errorf("no tool calls found in streaming response")
	}

	for i, toolCall := range finalToolCalls {
		if toolCall.ID == "" || toolCall.Name == "" || toolCall.Arguments == "" {
			return fmt.Errorf("tool call %d missing required fields: ID=%v, Name=%v, Arguments=%v",
				i, toolCall.ID != "", toolCall.Name != "", toolCall.Arguments != "")
		}
	}

	if err := validateStreamingToolCallsHelper(finalToolCalls, "Chat Completions"); err != nil {
		return err
	}
...
func runToolCallsStreamingResponsesHelper(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	...
	validationResult := WithResponsesStreamValidationRetry(t, retryConfig, retryContext,
		...
		streamComplete:
			...
			if err := validateStreamingToolCallsHelper(finalToolCalls, "Responses API"); err != nil {
				return ResponsesStreamValidationResult{
					Passed:       false,
	...
	if !validationResult.Passed {
		...
		return fmt.Errorf("responses streaming tool calls validation failed after retries: %s", errorMsg)
	}
...
// Deprecated: Use validateStreamingToolCallsHelper for new code
func validateStreamingToolCalls(t *testing.T, toolCalls []ToolCallInfo, apiName string) {
	if err := validateStreamingToolCallsHelper(toolCalls, apiName); err != nil {
		t.Fatal(err)
	}
}

// validateStreamingToolCallsHelper validates that all tool calls have ID, name, and arguments
func validateStreamingToolCallsHelper(toolCalls []ToolCallInfo, apiName string) error {
	if len(toolCalls) == 0 {
