...
func RunEnd2EndToolCallingTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("EndToEndToolCalling", func(t *testing.T) {
		WrapTestScenario(t, client, ctx, testConfig, "EndToEndToolCalling", ModelTypeChat, runEndToEndToolCallingTestForModel)
	})
}

func runEndToEndToolCallingTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	// =============================================================================
	...
	if !result1.BothSucceeded {
		...
		return fmt.Errorf("❌ End2EndToolCalling_Step1 dual API test failed: %v", errors)
	}
	...
	if len(chatToolCalls) == 0 {
		return fmt.Errorf("Expected at least one tool call in Chat Completions API response for 'weather'")
	}
	if len(responsesToolCalls) == 0 {
		return fmt.Errorf("Expected at least one tool call in Responses API response for 'weather'")
	}
	...
	if !result2.BothSucceeded {
		...
		return fmt.Errorf("❌ End2EndToolCalling_Step2 dual API test failed: %v", errors)
	}
