...
// RunTranscriptionTest executes the transcription test scenario using multi-model testing framework
// This function now supports testing multiple transcription models - the test passes only if ALL models pass
func RunTranscriptionTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("Transcription", func(t *testing.T) {
		WrapTestScenario(t, client, ctx, testConfig, "Transcription", ModelTypeTranscription, runTranscriptionTestForModel)
	})
}

// runTranscriptionTestForModel runs the transcription test for a specific model
// The config passed here will have only ONE model in TranscriptionModels array
func runTranscriptionTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	...
	for _, tc := range roundTripCases {
		...
		err := runRoundTripTestCase(t, client, ctx, testConfig, tc)
		if err != nil {
			return fmt.Errorf("round trip test %s failed: %w", tc.name, err)
		}
	}
	...
	for _, tc := range customCases {
		tc := tc // Capture loop variable
		err := runCustomTestCase(t, client, ctx, testConfig, tc)
		if err != nil {
			return fmt.Errorf("custom test %s failed: %w", tc.name, err)
		}
	}
...
// runRoundTripTestCase runs a single round-trip test case
func runRoundTripTestCase(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig, tc struct {
	name           string
	text           string
	voiceType      string
	format         string
	...
	if testConfig.Provider == schemas.HuggingFace && strings.HasPrefix(GetTranscriptionModelOrFirst(testConfig), "fal-ai/") {
		...
		if err != nil {
			return fmt.Errorf("failed to read audio fixture %s: %w", filePath, err)
		}
		...
		if err != nil {
			return fmt.Errorf("TTS generation failed for round-trip test after retries: %v", GetErrorMessage(err))
		}
		if ttsResponse == nil || len(ttsResponse.Audio) == 0 {
			return fmt.Errorf("TTS returned invalid or empty audio for round-trip test after retries")
		}
		...
		if writeErr != nil {
			return fmt.Errorf("failed to save temp audio file: %w", writeErr)
		}
	...
	if bifrostErr != nil {
		return fmt.Errorf("Transcription_RoundTrip_%s request failed after retries: %v", tc.name, GetErrorMessage(bifrostErr))
	}

	// Validate round-trip transcription (complementary to main validation)
	if err := validateTranscriptionRoundTripError(t, transcriptionResponse, tc.text, tc.name, testConfig); err != nil {
...
// runCustomTestCase runs a single custom test case
func runCustomTestCase(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig, tc struct {
	name           string
	text           string
	language       *string
	responseFormat *string
	...
	if testConfig.Provider == schemas.HuggingFace && strings.HasPrefix(GetTranscriptionModelOrFirst(testConfig), "fal-ai/") {
		...
		if readErr != nil {
			return fmt.Errorf("failed to read audio fixture %s: %w", filePath, readErr)
		}
	} else {
	...
	if err != nil {
		...
		return fmt.Errorf("custom transcription failed after retries: %s", errorMsg)
	}
	if response == nil {
		return fmt.Errorf("custom transcription returned nil response after retries")
	}
	if response.Text == "" {
		return fmt.Errorf("custom transcription returned empty text after retries")
	}
...
// RunTranscriptionAdvancedTest executes advanced transcription test scenarios using multi-model testing framework
// This function now supports testing multiple transcription models - the test passes only if ALL models pass
func RunTranscriptionAdvancedTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("TranscriptionAdvanced", func(t *testing.T) {
		WrapTestScenario(t, client, ctx, testConfig, "TranscriptionAdvanced", ModelTypeTranscription, runTranscriptionAdvancedTestForModel)
	})
}

// runTranscriptionAdvancedTestForModel runs the advanced transcription test for a specific model
// The config passed here will have only ONE model in TranscriptionModels array
func runTranscriptionAdvancedTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	// Test supported response formats (excluding text to avoid JSON parsing issues)
	formats := []string{"json"}

	for _, format := range formats {
		format := format // Capture loop variable
		if err := runFormatTest(t, client, ctx, testConfig, format); err != nil {
			return fmt.Errorf("format test %s failed: %w", format, err)
		}
	}

	// Test with custom parameters
	if err := runCustomParametersTest(t, client, ctx, testConfig); err != nil {
		return fmt.Errorf("custom parameters test failed: %w", err)
	}
	...
	for _, lang := range languages {
		...
		if err := runLanguageTest(t, client, ctx, testConfig, lang); err != nil {
			return fmt.Errorf("language test %s failed: %w", lang, err)
		}
	}
...
// runFormatTest tests a specific response format
func runFormatTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig, format string) error {
	speechSynthesisProvider := testConfig.Provider
	if testConfig.ExternalTTSProvider != "" {
		speechSynthesisProvider = testConfig.ExternalTTSProvider
	}
	...
	if err != nil {
		...
		return fmt.Errorf("transcription failed for format %s after retries: %s", format, errorMsg)
	}
	if response == nil {
		return fmt.Errorf("transcription returned nil response for format %s after retries", format)
	}
	if response.Text == "" {
		return fmt.Errorf("transcription returned empty text for format %s after retries", format)
	}
...
// runCustomParametersTest tests transcription with custom parameters
func runCustomParametersTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	speechSynthesisProvider := testConfig.Provider
	if testConfig.ExternalTTSProvider != "" {
		speechSynthesisProvider = testConfig.ExternalTTSProvider
	}
	...
	if err != nil {
		...
		return fmt.Errorf("advanced transcription failed after retries: %s", errorMsg)
	}
	if response == nil {
		return fmt.Errorf("advanced transcription returned nil response after retries")
	}
	if response.Text == "" {
		return fmt.Errorf("advanced transcription returned empty text after retries")
	}
...
// runLanguageTest tests transcription with a specific language
func runLanguageTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig, lang string) error {
	speechSynthesisProvider := testConfig.Provider
	if testConfig.ExternalTTSProvider != "" {
		speechSynthesisProvider = testConfig.ExternalTTSProvider
	}
	...
	if err != nil {
		...
		return fmt.Errorf("transcription failed for language %s after retries: %s", lang, errorMsg)
	}
	if response == nil {
		return fmt.Errorf("transcription returned nil response for language %s after retries", lang)
	}
	if response.Text == "" {
		return fmt.Errorf("transcription returned empty text for language %s after retries", lang)
	}
...
// validateTranscriptionRoundTripError performs round-trip validation and returns an error if validation fails
// This is the error-returning version used by the multi-model testing framework
func validateTranscriptionRoundTripError(t *testing.T, response *schemas.BifrostTranscriptionResponse, originalText string, testName string, testConfig ComprehensiveTestConfig) error {
	if response == nil || response.Text == "" {
		return fmt.Errorf("transcription response missing transcribed text")
	}
