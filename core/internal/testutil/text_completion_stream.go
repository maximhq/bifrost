...
func RunTextCompletionStreamTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("TextCompletionStream", func(t *testing.T) {
		WrapTestScenario(t, client, ctx, testConfig, "TextCompletionStream", ModelTypeText, runTextCompletionStreamTestForModel)
	})

	// Test text completion streaming with different prompts
	t.Run("TextCompletionStreamVariedPrompts", func(t *testing.T) {
		WrapTestScenario(t, client, ctx, testConfig, "TextCompletionStreamVariedPrompts", ModelTypeText, runTextCompletionStreamVariedPromptsTestForModel)
	})

	// Test text completion streaming with different parameters
	t.Run("TextCompletionStreamParameters", func(t *testing.T) {
		WrapTestScenario(t, client, ctx, testConfig, "TextCompletionStreamParameters", ModelTypeText, runTextCompletionStreamParametersTestForModel)
	})
}

// runTextCompletionStreamTestForModel runs the text completion stream test for a specific model
func runTextCompletionStreamTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	...
	for {
		...
		case <-streamCtx.Done():
			return fmt.Errorf("Timeout waiting for text completion streaming response")
		}
	}
...
streamComplete:
	...
	if lastResponse != nil && lastResponse.BifrostTextCompletionResponse != nil {
		if lastResponse.BifrostTextCompletionResponse.ExtraFields.Latency <= 0 {
			return fmt.Errorf("❌ Last streaming chunk missing latency information (got %d ms)", lastResponse.BifrostTextCompletionResponse.ExtraFields.Latency)
		} else {
	...
	if !validationResult.Passed {
		return fmt.Errorf("❌ Text completion streaming validation failed: %v", validationResult.Errors)
	}
...
// runTextCompletionStreamVariedPromptsTestForModel runs varied prompts test for a specific model
func runTextCompletionStreamVariedPromptsTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	// Get the single model from the config
	model := GetTextModelOrFirst(testConfig)
	if model == "" {
		model = GetChatModelOrFirst(testConfig)
	...
	for _, testCase := range testPrompts {
		t.Logf("Testing varied prompt: %s", testCase.name)
...
// runTextCompletionStreamParametersTestForModel runs parameters test for a specific model
func runTextCompletionStreamParametersTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	// Get the single model from the config
	model := GetTextModelOrFirst(testConfig)
	if model == "" {
		model = GetChatModelOrFirst(testConfig)
	...
	for _, paramTest := range parameterTests {
		t.Logf("Testing parameters: %s", paramTest.name)
