...
func RunResponsesStreamTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ResponsesStream", func(t *testing.T) {
		WrapTestScenario(t, client, ctx, testConfig, "ResponsesStream", ModelTypeChat, runResponsesStreamTest)
	})
	...
	if testConfig.Scenarios.ToolCalls {
		t.Run("ResponsesStreamWithTools", func(t *testing.T) {
			WrapTestScenario(t, client, ctx, testConfig, "ResponsesStreamWithTools", ModelTypeChat, runResponsesStreamWithToolsTest)
		})
	}
	...
	if testConfig.Scenarios.Reasoning && GetReasoningModelOrFirst(testConfig) != "" {
		t.Run("ResponsesStreamWithReasoning", func(t *testing.T) {
			WrapTestScenario(t, client, ctx, testConfig, "ResponsesStreamWithReasoning", ModelTypeReasoning, runResponsesStreamWithReasoningTest)
		})
	}

	// Test responses streaming lifecycle events
	t.Run("ResponsesStreamLifecycle", func(t *testing.T) {
		WrapTestScenario(t, client, ctx, testConfig, "ResponsesStreamLifecycle", ModelTypeChat, runResponsesStreamLifecycleTest)
	})
}

// runResponsesStreamTest is the helper function for the main responses stream test
func runResponsesStreamTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	...
	if !validationResult.Passed {
		...
		return fmt.Errorf("responses streaming validation failed after retries: %s", errorMsg)
	}
...
// runResponsesStreamWithToolsTest is the helper function for responses stream with tools
func runResponsesStreamWithToolsTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	messages := []schemas.ResponsesMessage{
		{
			Role: schemas.Ptr(schemas.ResponsesInputMessageRoleUser),
			Content: &schemas.ResponsesMessageContent{
	...
	if err != nil {
		return fmt.Errorf("responses stream with tools failed: %w", err)
	}
	...
	for {
		...
		case <-streamCtx.Done():
			return fmt.Errorf("timeout waiting for responses streaming response with tools")
		}
	}
...
toolStreamComplete:
	...
	if !toolCallDetected && !functionCallArgsDetected {
		return fmt.Errorf("should detect tool calls or function arguments in responses streaming response")
	}
...
// runResponsesStreamWithReasoningTest is the helper function for responses stream with reasoning
func runResponsesStreamWithReasoningTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	...
	if err != nil {
		return fmt.Errorf("responses stream with reasoning failed: %w", err)
	}
	if responseChannel == nil {
		return fmt.Errorf("response channel should not be nil")
	}
	...
	for {
		...
		case <-streamCtx.Done():
			return fmt.Errorf("timeout waiting for responses streaming response with reasoning")
		}
	}
...
// runResponsesStreamLifecycleTest is the helper function for responses stream lifecycle test
func runResponsesStreamLifecycleTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	messages := []schemas.ResponsesMessage{
		{
			Role: schemas.Ptr(schemas.ResponsesInputMessageRoleUser),
			Content: &schemas.ResponsesMessageContent{
	...
	if !validationResult.Passed {
		...
		return fmt.Errorf("responses streaming lifecycle validation failed after retries: %s", errorMsg)
	}
