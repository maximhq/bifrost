...
func WithImageGenerationRetry(
	...
	for attempt := 1; attempt <= config.MaxAttempts; attempt++ {
		...
		if response == nil {
			...
			if config.OnFinalFail != nil {
				config.OnFinalFail(attempt, fmt.Errorf("❌ image generation response is nil after %d attempts", attempt), t)
			}
			statusCode := 400
...
// WithImageGenerationStreamValidationRetry wraps an image generation streaming operation with retry logic that includes stream content validation
// This function wraps the entire operation (request + stream reading + validation) and retries on validation failures
