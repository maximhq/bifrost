...
func RunBatchRetrieveTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("BatchRetrieve", func(t *testing.T) {
		...
		if createErr != nil {
			// Check if this is an unsupported operation error
			if createErr.Error.Code != nil && *createErr.Error.Code == "unsupported_operation" {
...
`)
		...
		if uploadErr != nil {
			if uploadErr.Error.Code != nil && *uploadErr.Error.Code == "unsupported_operation" {
...
`)
		...
		if batchErr != nil {
			if batchErr.Error.Code != nil && *batchErr.Error.Code == "unsupported_operation" {
