...
func RunSpeechSynthesisStreamTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("SpeechSynthesisStream", func(t *testing.T) {
		...
		// Use WrapTestScenario for multi-model testing
		WrapTestScenario(t, client, ctx, testConfig, "SpeechSynthesisStream", ModelTypeSpeechSynthesis, runSpeechSynthesisStreamTestForModel)
	})
}

// runSpeechSynthesisStreamTestForModel runs the streaming speech synthesis test for a specific model
func runSpeechSynthesisStreamTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	...
	for _, tc := range testCases {
		...
		err := func() error {
			if tc.skip {
				t.Logf("Skipping %s test", tc.name)
				return nil
			}
			...
			if err != nil {
				return fmt.Errorf("speech synthesis stream initiation failed: %v", err)
			}
			...
			for response := range responseChannel {
				...
				if response.BifrostSpeechStreamResponse.Audio != nil {
					...
					if chunkSize == 0 {
						t.Logf("Skipping zero-length audio chunk")
						continue
					}
					...
					if response.BifrostSpeechStreamResponse.Type != "" && (response.BifrostSpeechStreamResponse.Type != schemas.SpeechStreamResponseTypeDelta && response.BifrostSpeechStreamResponse.Type != schemas.SpeechStreamResponseTypeDone) {
						t.Logf("Unexpected object type in stream: %s", response.BifrostSpeechStreamResponse.Type)
					}
					if response.BifrostSpeechStreamResponse.ExtraFields.ModelRequested != "" && response.BifrostSpeechStreamResponse.ExtraFields.ModelRequested != GetSpeechSynthesisModelOrFirst(testConfig) {
						t.Logf("Unexpected model in stream: %s", response.BifrostSpeechStreamResponse.ExtraFields.ModelRequested)
					}
				}
			...
			if len(streamErrors) > 0 {
				t.Logf("Stream errors encountered: %v", streamErrors)
			}

			if chunkCount < tc.expectMinChunks {
				return fmt.Errorf("insufficient chunks received: got %d, expected at least %d", chunkCount, tc.expectMinChunks)
			}

			if totalBytes < tc.expectMinBytes {
				return fmt.Errorf("insufficient audio data: got %d bytes, expected at least %d", totalBytes, tc.expectMinBytes)
			}
			...
			if chunkCount == 0 {
				return fmt.Errorf("no audio chunks received from stream")
			}
			...
			if audioBuffer.Len() > 0 {
				...
				if validationErr != nil {
					return fmt.Errorf("audio codec validation failed: %v", validationErr)
				}
				t.Logf("Audio file validated successfully: %s", filePath)
			} else {
				return fmt.Errorf("no audio data accumulated for codec validation")
			}
...
func RunSpeechSynthesisStreamAdvancedTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("SpeechSynthesisStreamAdvanced", func(t *testing.T) {
		t.Run("LongText_HDModel_Streaming", func(t *testing.T) {
			...
			// Use WrapTestScenario for multi-model testing
			WrapTestScenario(t, client, ctx, testConfig, "LongText_HDModel_Streaming", ModelTypeSpeechSynthesis, runLongTextHDModelStreamingTestForModel)
		})
		...
		t.Run("MultipleVoices_Streaming", func(t *testing.T) {
			...
			// Use WrapTestScenario for multi-model testing
			WrapTestScenario(t, client, ctx, testConfig, "MultipleVoices_Streaming", ModelTypeSpeechSynthesis, runMultipleVoicesStreamingTestForModel)
		})
	})
}

// runLongTextHDModelStreamingTestForModel runs the long text HD model streaming test for a specific model
func runLongTextHDModelStreamingTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	if testConfig.Provider == schemas.Gemini {
		t.Logf("Skipping LongText_HDModel_Streaming test for Gemini")
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("HD streaming speech synthesis failed: %v", err)
	}
	...
	for response := range responseChannel {
		...
		if response.BifrostSpeechStreamResponse != nil && response.BifrostSpeechStreamResponse.Audio != nil {
			chunkSize := len(response.BifrostSpeechStreamResponse.Audio)
			if chunkSize == 0 {
				t.Logf("Skipping zero-length HD audio chunk")
				continue
			}
			// Accumulate audio data for codec validation
			audioBuffer.Write(response.BifrostSpeechStreamResponse.Audio)
		...
		if response.BifrostSpeechStreamResponse != nil && response.BifrostSpeechStreamResponse.ExtraFields.ModelRequested != "" && response.BifrostSpeechStreamResponse.ExtraFields.ModelRequested != GetSpeechSynthesisModelOrFirst(testConfig) {
			t.Logf("Unexpected HD model: %s", response.BifrostSpeechStreamResponse.ExtraFields.ModelRequested)
		}
	}

	if len(streamErrors) > 0 {
		t.Logf("HD stream errors: %v", streamErrors)
	}

	if chunkCount <= 3 {
		return fmt.Errorf("HD model should produce more chunks for long text: got %d, expected > 3", chunkCount)
	}

	if totalBytes <= 10000 {
		return fmt.Errorf("HD model should produce substantial audio data: got %d bytes, expected > 10000", totalBytes)
	}
...
// runMultipleVoicesStreamingTestForModel runs the multiple voices streaming test for a specific model
func runMultipleVoicesStreamingTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	voices := []string{}
	...
	for _, voice := range voices {
		...
		err := func() error {
			...
			if !validationResult.Passed {
				...
				return fmt.Errorf("speech streaming validation failed for voice %s: %s", voiceCopy, strings.Join(allErrors, "; "))
			}
			...
			if accumulatedAudio.Len() > 0 {
				...
				filePath, validationErr := SaveAndValidateAudio(t, audioData)
				if validationErr != nil {
					return fmt.Errorf("audio codec validation failed for voice %s: %v", voiceCopy, validationErr)
				}
				t.Logf("Audio file validated successfully for voice %s: %s", voiceCopy, filePath)
			} else {
				return fmt.Errorf("no audio data accumulated for codec validation (voice: %s)", voiceCopy)
			}
		...
		if err != nil {
			return fmt.Errorf("voice test %s failed: %w", voiceCopy, err)
		}
	}
