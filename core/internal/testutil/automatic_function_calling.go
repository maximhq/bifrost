...
func RunAutomaticFunctionCallingTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("AutomaticFunctionCalling", func(t *testing.T) {
		WrapTestScenario(t, client, ctx, testConfig, "AutomaticFunctionCalling", ModelTypeChat, runAutomaticFunctionCallingTestForModel)
	})
}

// runAutomaticFunctionCallingTestForModel runs the automatic function calling test for a specific model
// The config passed here will have only ONE model in ChatModels array
func runAutomaticFunctionCallingTestForModel(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) error {
	...
	if chatTool == nil {
		return fmt.Errorf("GetSampleChatTool returned nil for SampleToolTypeTime")
	}

	responsesTool := GetSampleResponsesTool(SampleToolTypeTime) // Responses API
	if responsesTool == nil {
		return fmt.Errorf("GetSampleResponsesTool returned nil for SampleToolTypeTime")
	}
	...
	if !result.BothSucceeded {
		...
		return fmt.Errorf("AutomaticFunctionCalling dual API test failed: %v", errors)
	}
	...
	t.Logf("🎉 Both Chat Completions and Responses APIs passed AutomaticFunctionCalling test for model: %s!", GetChatModelOrFirst(testConfig))
	return nil
}
