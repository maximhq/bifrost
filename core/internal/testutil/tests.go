...
func printTestSummary(t *testing.T, testConfig ComprehensiveTestConfig) {
	testScenarios := []struct {
		...
		{"TextCompletion", testConfig.Scenarios.TextCompletion && len(testConfig.TextModels) > 0},
		...
		{"Embedding", testConfig.Scenarios.Embedding && len(testConfig.EmbeddingModels) > 0},
		{"ChatCompletionReasoning", testConfig.Scenarios.Reasoning && len(testConfig.ReasoningModels) > 0},
		{"ResponsesReasoning", testConfig.Scenarios.Reasoning && len(testConfig.ReasoningModels) > 0},
		{"ListModels", testConfig.Scenarios.ListModels},
		{"PromptCaching", testConfig.Scenarios.SimpleChat && len(testConfig.PromptCachingModels) > 0},
		...
		{"ChatAudio", testConfig.Scenarios.ChatAudio && len(testConfig.ChatAudioModels) > 0},
		{"ChatAudioStream", testConfig.Scenarios.ChatAudio && len(testConfig.ChatAudioModels) > 0},
