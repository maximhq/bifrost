...
func RunEmbeddingTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("Embedding", func(t *testing.T) {
		...
		contents := make([]schemas.EmbeddingInputItem, len(testTexts))
		for i, text := range testTexts {
			t := text
			contents[i] = schemas.EmbeddingInputItem{Content: schemas.EmbeddingContent{{Type: schemas.EmbeddingContentPartTypeText, Text: &t}}}
		}
