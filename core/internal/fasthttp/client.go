...
// DoContext is like Do, but additionally accepts a context.Context.
// If the context is cancelled or its deadline expires while the request
// is in-flight, the underlying connection is closed, aborting the
// upstream request immediately. This is especially useful for streaming
// requests where the downstream client may disconnect during the
// provider's prefill phase.
...
func (c *HostClient) Do(req *Request, resp *Response) error {
	return c.doWithChecks(req, resp)
}

// DoContext is like Do, but accepts a context.Context for cancellation.
// If the context is cancelled while the request is in-flight, the
// underlying connection is closed, aborting the upstream request.
func (c *HostClient) DoContext(ctx context.Context, req *Request, resp *Response) error {
	req.ctx = ctx
	return c.doWithChecks(req, resp)
}

func (c *HostClient) doWithChecks(req *Request, resp *Response) error {
	var (
		err          error
		retry        bool
		resetTimeout bool
	...
	// If the context was cancelled during the request, return the context
	// error instead of the raw connection error. This ensures callers
	// consistently see context.Canceled / context.DeadlineExceeded on
	// cancellation, regardless of which I/O operation was interrupted.
	if req.ctx != nil {
...
func (t *transport) RoundTrip(hc *HostClient, req *Request, resp *Response) (retry bool, err error) {
	...
	// Context cancellation during prefill (header read phase).
	// When the context is cancelled, close the underlying connection to
	// abort any in-flight Read. After headers are successfully read, the
	// watcher is stopped so streaming body reads can proceed normally.
	// Streaming body cancellation is handled at the call site via
	// SetupStreamCancellation.
	var prefillCancel func()
	if req.ctx != nil {
		prefillDone := make(chan struct{})
		prefillCancel = func() { close(prefillDone) }
		go func() {
			select {
			case <-req.ctx.Done():
				conn.Close()
	...
	if prefillCancel != nil {
		prefillCancel()
		// Check if context was cancelled during the header read.
		// This handles the race between ReadLimitBody returning success
		// and the watcher goroutine receiving from ctx.Done().
