...
type Request struct {
	...
	// Request context. Used to cancel in-flight Do* calls.
	// Set via SetContext or DoContext.
	ctx context.Context
...
// SetContext sets the context for the request. When set, the context's
// Done channel is monitored during Do* calls. If the context is cancelled
// or its deadline expires while the request is in-flight, the underlying
// connection is closed, aborting the request immediately.
//
// Unlike SetTimeout which sets a fixed deadline at call time, SetContext
// allows dynamic cancellation from another goroutine at any point during
// the request lifecycle.
//
// The context is cleared on Reset.
func (req *Request) SetContext(ctx context.Context) {
	req.ctx = ctx
}

// Context returns the context associated with the request, or nil if
// no context was set via SetContext or DoContext.
