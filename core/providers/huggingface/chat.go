	...
	for _, msg := range bifrostReq.Input {
		...
		if msg.Content != nil {
			...
			if msg.Content.ContentStr != nil {
				// Assign as a string content for HuggingFaceMessageContent
				s := *msg.Content.ContentStr
				hfMsg.Content = HuggingFaceMessageContent{String: &s}
				...
				// Assign as structured content items for HuggingFaceMessageContent
				hfMsg.Content = HuggingFaceMessageContent{Items: contentItems}
			}
		}
...
func (response *HuggingFaceChatStreamResponse) ToBifrostChatStreamResponse() *schemas.BifrostChatResponse {
	...
	for i, choice := range response.Choices {
		...
		// Preserve reasoning separately
		if choice.Delta.Reasoning != nil {
			streamChoice.Delta.Reasoning = choice.Delta.Reasoning
		}

		// Convert tool calls if present
		if len(choice.Delta.ToolCalls) > 0 {
			toolCalls := make([]schemas.ChatAssistantMessageToolCall, 0, len(choice.Delta.ToolCalls))
