...
func (provider *AzureProvider) Responses(ctx context.Context, key schemas.Key, request *schemas.BifrostResponsesRequest) (*schemas.BifrostResponse, *schemas.BifrostError) {
	...
	if reqBody == nil {
		return nil, newBifrostOperationError("responses input is not provided", nil, schemas.Azure)
	}

	responseBody, err := provider.completeRequest(ctx, reqBody, "responses", key, request.Model)
	if err != nil {
		return nil, err
	}
