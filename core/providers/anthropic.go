...
func buildAnthropicTextRequest(model string, text string, params *schemas.ModelParameters) *api.AnthropicTextRequest {
	...
	if params != nil {
		request.MaxTokensToSample = DEFAULT_MAX_TOKENS
...
func buildAnthropicChatRequest(model string, messages []schemas.BifrostMessage, params *schemas.ModelParameters) *api.AnthropicMessageRequest {
	...
	// Add parameters if provided
	if params != nil {
		request.MaxTokens = DEFAULT_MAX_TOKENS // Default value, will be set below if provided
		if params.MaxTokens != nil {
		...
		if params.ExtraParams != nil {
			// Extract known fields and set them on the typed struct
			if anthropicVersion, ok := params.ExtraParams["anthropic_version"].(string); ok {
				request.AnthropicVersion = &anthropicVersion
				delete(params.ExtraParams, "anthropic_version")
			}
			if region, ok := params.ExtraParams["region"].(string); ok {
				request.Region = &region
				delete(params.ExtraParams, "region")
			}

			// Pass all ExtraParams to the request - MarshalJSON will handle conflict detection
			// This ensures unknown fields can still be passed through to the API
			request.ExtraParams = params.ExtraParams
		}
	}
