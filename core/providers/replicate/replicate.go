...
func (provider *ReplicateProvider) FileContent(ctx *schemas.BifrostContext, keys []schemas.Key, request *schemas.BifrostFileContentRequest) (*schemas.BifrostFileContentResponse, *schemas.BifrostError) {
	...
	var lastErr *schemas.BifrostError
	for _, key := range keys {
		// Check if we have the required download parameters from ExtraParams
		var owner string
		var expiry int64
		...
		if request.ExtraParams != nil {
			...
			if e, ok := request.ExtraParams["expiry"].(int64); ok {
				expiry = e
			} else if e, ok := request.ExtraParams["expiry"].(int); ok {
				expiry = int64(e)
		...
		// Validate that owner and expiry are provided
		if owner == "" || expiry == 0 {
			return nil, providerUtils.NewBifrostOperationError(
				"Replicate file download requires 'owner' and 'expiry' parameters.",
				nil,
				providerName,
			)
		}

		// Check if key has signing secret configured
		if key.ReplicateKeyConfig == nil || key.ReplicateKeyConfig.SigningSecret == nil {
			return nil, providerUtils.NewBifrostOperationError(
				"Replicate file download requires 'signing_secret' to be configured in the Replicate key configuration",
				nil,
				providerName,
			)
		}

		signingSecret := key.ReplicateKeyConfig.SigningSecret.GetValue()
		if signingSecret == "" {
			return nil, providerUtils.NewBifrostOperationError(
				"Replicate file download requires 'signing_secret' to be configured in the Replicate key configuration",
				nil,
		...
		signature := generateFileDownloadSignature(owner, request.FileID, expiry, signingSecret)
