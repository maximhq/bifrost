...
func (provider *ReplicateProvider) TextCompletion(ctx *schemas.BifrostContext, key schemas.Key, request *schemas.BifrostTextCompletionRequest) (*schemas.BifrostTextCompletionResponse, *schemas.BifrostError) {
	...
	jsonData, bifrostErr := providerUtils.CheckContextAndGetRequestBody(
		ctx,
		request,
		func() (any, error) { return ToReplicateTextRequest(request) },
...
func (provider *ReplicateProvider) ChatCompletion(ctx *schemas.BifrostContext, key schemas.Key, request *schemas.BifrostChatRequest) (*schemas.BifrostChatResponse, *schemas.BifrostError) {
	...
	jsonData, bifrostErr := providerUtils.CheckContextAndGetRequestBody(
		ctx,
		request,
		func() (any, error) { return ToReplicateChatRequest(request) },
		provider.GetProviderKey())
	if bifrostErr != nil {
		return nil, bifrostErr
	}
...
func (provider *ReplicateProvider) Responses(ctx *schemas.BifrostContext, key schemas.Key, request *schemas.BifrostResponsesRequest) (*schemas.BifrostResponsesResponse, *schemas.BifrostError) {
	...
	jsonData, bifrostErr := providerUtils.CheckContextAndGetRequestBody(
		ctx,
		request,
		func() (any, error) { return ToReplicateResponsesRequest(request) },
		provider.GetProviderKey())
	if bifrostErr != nil {
		return nil, bifrostErr
	}
...
func (provider *ReplicateProvider) ImageGeneration(ctx *schemas.BifrostContext, key schemas.Key, request *schemas.BifrostImageGenerationRequest) (*schemas.BifrostImageGenerationResponse, *schemas.BifrostError) {
	...
	jsonData, bifrostErr := providerUtils.CheckContextAndGetRequestBody(
		ctx,
		request,
		func() (any, error) { return ToReplicateImageGenerationInput(request), nil },
		provider.GetProviderKey())
	if bifrostErr != nil {
		return nil, bifrostErr
	}
...
// FileContent downloads file content from Replicate's Files API.
// Replicate's download endpoint requires owner, expiry, and signature query parameters.
// These can be provided via the request's ExtraParams or will be returned in an error.
func (provider *ReplicateProvider) FileContent(ctx *schemas.BifrostContext, keys []schemas.Key, request *schemas.BifrostFileContentRequest) (*schemas.BifrostFileContentResponse, *schemas.BifrostError) {
	...
	var lastErr *schemas.BifrostError
	for _, key := range keys {
		// Check if we have the required download parameters
		var owner, signature string
		var expiry int64

		if request.ExtraParams != nil {
			if o, ok := request.ExtraParams["owner"].(string); ok {
				owner = o
			}
			if s, ok := request.ExtraParams["signature"].(string); ok {
				signature = s
			}
			if e, ok := request.ExtraParams["expiry"].(int64); ok {
				expiry = e
			} else if e, ok := request.ExtraParams["expiry"].(float64); ok {
				expiry = int64(e)
			}
		}

		// If we don't have the required parameters, return an error with instructions
		if owner == "" || signature == "" || expiry == 0 {
			return nil, providerUtils.NewBifrostOperationError(
				"Replicate file download requires 'owner', 'expiry', and 'signature' parameters. "+
					"Provide these in ExtraParams: owner (string), expiry (int64 Unix timestamp), "+
					"signature (base64-encoded HMAC-SHA256 of '{owner} {file_id} {expiry}' using Files API signing secret)",
				nil,
				providerName,
			)
		}
		...
		downloadURL := fmt.Sprintf("%s/v1/files/%s/download?owner=%s&expiry=%d&signature=%s",
			provider.networkConfig.BaseURL,
			url.PathEscape(request.FileID),
