...
func ToAnthropicChatRequest(bifrostReq *schemas.BifrostChatRequest) (*AnthropicMessageRequest, error) {
	...
	} else if bifrostReq.Params.Reasoning != nil && bifrostReq.Params.Reasoning.Effort != nil && *bifrostReq.Params.Reasoning.Effort == "none" {
		anthropicReq.Thinking = &AnthropicThinking{
			Type: "disabled",
		}
	}
			...
			for _, c := range response.Content {
				...
				case AnthropicContentBlockTypeThinking:
					reasoningDetails = append(reasoningDetails, schemas.ChatReasoningDetails{
						...
						Type:      schemas.ChatReasoningDetailsTypeText,
						Text:      c.Thinking,
...
func ToAnthropicChatResponse(bifrostResp *schemas.BifrostChatResponse) *AnthropicMessageResponse {
	...
	if len(bifrostResp.Choices) > 0 {
		...
		if choice.Message.ChatAssistantMessage != nil && choice.Message.ChatAssistantMessage.ReasoningDetails != nil {
			for _, reasoningDetail := range choice.Message.ChatAssistantMessage.ReasoningDetails {
				if reasoningDetail.Type == schemas.ChatReasoningDetailsTypeText && reasoningDetail.Text != nil {
					content = append(content, AnthropicContentBlock{
						Type:      AnthropicContentBlockTypeThinking,
						Thinking:  reasoningDetail.Text,
						Signature: reasoningDetail.Signature,
