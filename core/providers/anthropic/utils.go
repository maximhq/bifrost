...
func IsAdaptiveOnlyThinkingModel(provider schemas.ModelProvider, model string) bool {
	if ov := providerUtils.CapabilitiesFor(provider, model); ov != nil && ov.SupportsSamplingParams != nil {
...
func SupportsEffortParameter(provider schemas.ModelProvider, model string) bool {
	if ov := providerUtils.CapabilitiesFor(provider, model); ov != nil && ov.SupportsNativeEffort != nil {
...
func SupportsMidConversationSystem(provider schemas.ModelProvider, model string) bool {
	...
	if ov := providerUtils.CapabilitiesFor(provider, model); ov != nil && ov.SupportsMidConversationSystem != nil {
...
func SupportsFastMode(provider schemas.ModelProvider, model string) bool {
	if ov := providerUtils.CapabilitiesFor(provider, model); ov != nil && ov.SupportsFastMode != nil {
...
func SupportsAdaptiveThinking(provider schemas.ModelProvider, model string) bool {
	if ov := providerUtils.CapabilitiesFor(provider, model); ov != nil && ov.SupportsAdaptiveThinking != nil {
...
func ComputerUseGeneration(provider schemas.ModelProvider, model string) string {
	if ov := providerUtils.CapabilitiesFor(provider, model); ov != nil {
...
func AddMissingBetaHeadersToContext(ctx *schemas.BifrostContext, req *AnthropicMessageRequest, provider schemas.ModelProvider) error {
	...
	if !hasProvider || features.FastMode {
		for _, fb := range req.nativeFallbacks() {
			if fb.Speed != nil && SupportsFastMode(provider, fb.Model) {
				headers = appendUniqueHeader(headers, AnthropicFastModeBetaHeader)
				break
			}
		}
