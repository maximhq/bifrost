...
func setOverride(t *testing.T, model string, ov schemas.ModelCapabilities) {
	t.Helper()
	// Helpers read overrides via CapabilitiesFor(Anthropic, model),
	// which resolves the CapabilityCacheKey composite; seed under the same key.
	key := providerUtils.CapabilityCacheKey(model, schemas.Anthropic)
	providerUtils.SetModelCapability(key, &ov)
	t.Cleanup(func() { providerUtils.DeleteModelCapability(key) })
}
...
func TestSupportsFastMode_OverrideAbsent_FallbackTakesOver(t *testing.T) {
	...
	assert.True(t, SupportsFastMode(schemas.Anthropic, "claude-opus-4-7-20260401"))
	assert.False(t, SupportsFastMode(schemas.Anthropic, "claude-sonnet-4-5-20250929"))
}
