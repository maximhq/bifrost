...
// NewSarvamProvider creates a new Sarvam AI provider instance.
...
// AuthHeaders returns Sarvam's auth headers for a key: the native
// api-subscription-key header (required by every Sarvam endpoint) plus
// Authorization: Bearer, which Sarvam's chat/completions endpoint also
// accepts for OpenAI-tooling compatibility.
func AuthHeaders(key schemas.Key) map[string]string {
	headers := openai.BearerAuthHeader(key)
	if key.Value.GetValue() != "" {
		headers["api-subscription-key"] = key.Value.GetValue()
	}
	return headers
}

// ListModels performs a list models request to Sarvam's API.
// GET /v1/models is undocumented in Sarvam's published OpenAPI spec, but is
// live and OpenAI-shaped (verified: {"object":"list","data":[{"id",...}]});
...
// Responses performs a responses request to the Sarvam API via chat completion fallback.
...
// ResponsesStream performs a streaming responses request to the Sarvam API via chat completion fallback.
...
// VideoDelete is not supported by the Sarvam provider.
...
// VideoList is not supported by the Sarvam provider.
...
// VideoRemix is not supported by the Sarvam provider.
...
// BatchCreate is not supported by the Sarvam provider.
...
// BatchList is not supported by the Sarvam provider.
...
// BatchRetrieve is not supported by the Sarvam provider.
...
// BatchCancel is not supported by the Sarvam provider.
...
// BatchDelete is not supported by the Sarvam provider.
...
// BatchResults is not supported by the Sarvam provider.
...
// FileUpload is not supported by the Sarvam provider.
...
// FileList is not supported by the Sarvam provider.
...
// FileRetrieve is not supported by the Sarvam provider.
...
// FileDelete is not supported by the Sarvam provider.
...
// FileContent is not supported by the Sarvam provider.
...
// Passthrough forwards a request byte-for-byte to Sarvam's API (no OpenAI-shape
// translation). Unlike the openai provider's Passthrough, Sarvam's non-chat
// endpoints (/text-to-speech, /speech-to-text, translation, etc.) are not
// versioned under /v1, so the request path is forwarded as-is.
func (provider *SarvamProvider) Passthrough(
	ctx *schemas.BifrostContext,
	key schemas.Key,
	req *schemas.BifrostPassthroughRequest,
) (*schemas.BifrostPassthroughResponse, *schemas.BifrostError) {
	if err := providerUtils.CheckOperationAllowed(schemas.Sarvam, nil, schemas.PassthroughRequest); err != nil {
		return nil, err
	}
...
// PassthroughStream forwards a streaming request byte-for-byte to Sarvam's API.
// Usage extraction is not attempted (Sarvam's usage shape is undocumented for
// passthrough-only endpoints); only raw bytes are forwarded to the client.
func (provider *SarvamProvider) PassthroughStream(
	ctx *schemas.BifrostContext,
	...
	if err := providerUtils.CheckOperationAllowed(schemas.Sarvam, nil, schemas.PassthroughStreamRequest); err != nil {
		return nil, err
	}
