	...
	go func() {
		...
		for {
			...
			if response.Type == schemas.ResponsesStreamResponseTypeError {
				...
				ctx.SetValue(schemas.BifrostContextKeyStreamEndIndicator, true)
				providerUtils.ProcessAndSendBifrostError(ctx, postHookRunner, preserveCustomRawErrorFields(providerUtils.EnrichError(ctx, bifrostErr, jsonBody, []byte(jsonData), sendBackRawRequest, sendBackRawResponse)), responseChan, logger, postHookSpanFinalizer)
				return
			}
