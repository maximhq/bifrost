...
func (provider *OpenAIProvider) exchangeWebRTCSDP(
	...
	if resp.StatusCode() < fasthttp.StatusOK || resp.StatusCode() >= fasthttp.StatusMultipleChoices {
		return "", provider.realtimeWebRTCUpstreamError(resp.StatusCode(), answerBody)
	}

	return string(answerBody), nil
}

func (provider *OpenAIProvider) realtimeWebRTCUpstreamError(statusCode int, body []byte) *schemas.BifrostError {
