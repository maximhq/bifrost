...
func mantleOpenAIURL(baseURL, model, path string) string {
	base := "v1"
	if strings.Contains(model, "gpt-5") || strings.Contains(model, "gemma-4") {
		base = "openai/v1"
	}
	return fmt.Sprintf("%s/%s/%s", baseURL, base, path)
}
...
func (provider *BedrockProvider) mantleChatCompletions(
	...
	url := mantleOpenAIURL(provider.endpointBase(key, bedrockServiceMantle, region), schemas.ResolveCanonicalModel(ctx, request.Model), "chat/completions")
...
func (provider *BedrockProvider) mantleResponses(
	...
	region := resolveBedrockRegion(ctx, key, request.Model)
	url := mantleOpenAIURL(provider.endpointBase(key, bedrockServiceMantle, region), schemas.ResolveCanonicalModel(ctx, request.Model), "responses")
