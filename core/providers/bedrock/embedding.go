...
func (r *BedrockCohereEmbeddingResponse) ToBifrostEmbeddingResponse() (*schemas.BifrostEmbeddingResponse, error) {
	...
	case "embeddings_by_type":
		...
		if typed.Float != nil {
			for i, emb := range typed.Float {
				bifrostResponse.Data = append(bifrostResponse.Data, schemas.EmbeddingData{
					Object:    "embedding",
					Index:     i,
					Embedding: schemas.EmbeddingStruct{EmbeddingArray: emb},
				})
			}
		}
		if typed.Base64 != nil {
		...
		for typeName, rows := range map[string][][]int8{"int8": typed.Int8, "binary": typed.Binary} {
			for i, emb := range rows {
				b, err := json.Marshal(emb)
				if err != nil {
					return nil, fmt.Errorf("error serializing %s embedding: %w", typeName, err)
				}
				s := string(b)
				bifrostResponse.Data = append(bifrostResponse.Data, schemas.EmbeddingData{
					Object:    "embedding",
					Index:     i,
					Embedding: schemas.EmbeddingStruct{EmbeddingStr: &s},
				})
			}
		}
		for typeName, rows := range map[string][][]int32{"uint8": typed.Uint8, "ubinary": typed.Ubinary} {
			for i, emb := range rows {
				b, err := json.Marshal(emb)
				if err != nil {
					return nil, fmt.Errorf("error serializing %s embedding: %w", typeName, err)
