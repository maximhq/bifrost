...
// Names are tracked by their emitted form, and suffix candidates are probed
// until one is unused: emitting "document-2" must reserve it, otherwise a
// later explicitly-named "document-2" collides again.
...
func convertContentBlock(ctx context.Context, model string, block schemas.ChatContentBlock, docNamer *bedrockDocNamer) ([]BedrockContentBlock, error) {
	...
	case schemas.ChatContentBlockTypeFile:
		...
		// Set filename (normalized for Bedrock). Untitled documents share the
		// default name, and the Converse API rejects duplicate names within a
		// request (#7003), so the namer disambiguates with numeric suffixes.
		if block.File.Filename != nil {
			documentSource.Name = docNamer.name(*block.File.Filename)
		} else {
			documentSource.Name = docNamer.name("")
		}
