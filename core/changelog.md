- fix: add GLM-5.3 to models supporting max reasoning effort and clamp its enum to max/high/low (medium→high, none/minimal→low, xhigh→max), mirroring Z.ai's Coding Plan coercion — the GLM-5.3 API errors on every other value [@is911](https://github.com/is911)
