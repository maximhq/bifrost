- fix: Anthropic provider - a chat completion user turn that converts to no content blocks (OpenAI-compatible clients such as Cursor send `content: ""` or `[]` for an empty turn) is sent as the provider's existing `.` placeholder text block instead of `content: []`, which Anthropic rejects (`user messages must have non-empty content`). The turn is filled rather than dropped so the request still ends on a user message; models without assistant-prefill support (Claude Fable 5.1) reject a request that ends on the assistant turn. Assistant turns are unchanged (#7276)
