- fix: Anthropic provider - an empty or whitespace-only chat completion user turn (OpenAI-compatible clients such as Cursor send `content: ""`, `[]` or blank text) is sent as the provider's existing `.` placeholder text block instead of `content: []`, which Anthropic rejects (`user messages must have non-empty content`). The turn is filled rather than dropped so the request still ends on a user message; models without assistant-prefill support (Claude Fable 5.1) reject a request that ends on the assistant turn. Assistant turns, and user turns whose real content cannot be converted (such as `input_audio`), are unchanged (#7276)
