...
const (
	...
	// FailureClassRegionBlocked: the provider refuses the caller's location. The location is
	// the gateway's, which every key shares, so no other key would fare better: the request
	// ends without retry or rotation.
...
func (c FailureClass) IsPermanentPerKey() bool {
	switch c {
	case FailureClassCredential, FailureClassModelAccess, FailureClassQuota, FailureClassModelGone:
		return true
	}
	return false
}
