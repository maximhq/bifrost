...
// ToCohereResponsesAPIRequest converts a BifrostRequest (ResponsesAPI structure) to CohereChatRequest
func ToCohereResponsesAPIRequest(bifrostReq *schemas.BifrostRequest) *CohereChatRequest {
	if bifrostReq == nil {
		return nil
	}
	...
	if bifrostReq.Params.Tools != nil {
		...
		for _, tool := range bifrostReq.Params.Tools.ResponsesTools {
	...
	// Process ResponsesInput (which contains the ResponsesAPI items)
	if bifrostReq.Input.ResponsesInput != nil {
		cohereReq.Messages = convertResponsesMessagesToCohereMessages(*bifrostReq.Input.ResponsesInput)
	}

	return cohereReq
}

// ToResponsesAPIBifrostResponse converts CohereChatResponse to BifrostResponse (ResponsesAPI structure)
func (cohereResp *CohereChatResponse) ToResponsesAPIBifrostResponse() *schemas.BifrostResponse {
	if cohereResp == nil {
		return nil
	}
...
func convertBifrostToolChoiceToCohereToolChoice(toolChoice schemas.BifrostToolChoice) *CohereToolChoice {
	toolChoiceString := toolChoice.ToolChoiceStr
...
// convertResponsesMessagesToCohereMessages converts ResponsesAPI items to Cohere messages
...
// convertCohereMessageToResponsesAPIOutput converts Cohere message to ResponsesAPI output format
func convertCohereMessageToResponsesOutput(cohereMsg CohereMessage) []schemas.ResponsesMessage {
	...
	if cohereMsg.Content != nil {
		...
		var thinkingContent []schemas.ResponsesReasoningContent
		...
		if cohereMsg.Content.StringContent != nil {
			...
			for _, block := range *cohereMsg.Content.BlocksContent {
				...
				if block.Type == CohereContentBlockTypeThinking {
					thinkingContent = append(thinkingContent, schemas.ResponsesReasoningContent{
						Type: schemas.ResponsesReasoningContentBlockTypeReasoningText,
						Text: *block.Thinking,
					})
				}
			}
...
// convertCohereContentBlockToBifrost converts CohereContentBlock to schemas.ContentBlock for ResponsesAPI
