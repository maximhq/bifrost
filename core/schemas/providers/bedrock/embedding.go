...
func ToBedrockTitanEmbeddingRequest(bifrostReq *schemas.BifrostEmbeddingRequest) (*BedrockTitanEmbeddingRequest, error) {
	...
	if bifrostReq.Input.Text == nil && len(bifrostReq.Input.Texts) > 1 {
		return nil, fmt.Errorf("amazon Titan embedding models only support single text input, but %d texts were provided", len(bifrostReq.Input.Texts))
	}
