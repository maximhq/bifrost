...
type ResponsesResponseInputTokens struct {
	...
	ToolUseTokens int `json:"tool_use_tokens,omitempty"` // Tokens for tool use input (set by gemini)

	ModalityTokenCount []ModalityTokenCount `json:"modality_token_count,omitempty"` // Detailed breakdown of input tokens by modality (set by gemini)
...
type ResponsesCodeInterpreterToolCall struct {
	...
	Language    *string                          `json:"language,omitempty"` // The language of the code (sent by Gemini)
}
...
type ResponsesCodeInterpreterOutputLogs struct {
	...
	ReturnCode *int   `json:"return_code,omitempty"` // sent by Anthropic
}
...
type ResponsesToolCodeInterpreter struct {
	Container ResponsesToolCodeInterpreterContainer `json:"container"` // Either a string or a ResponsesToolCodeInterpreterContainerStruct object
}

type ResponsesToolCodeInterpreterContainer struct {
	ResponsesToolCodeInterpreterContainerString *string
	ResponsesToolCodeInterpreterContainerStruct *ResponsesToolCodeInterpreterContainerStruct
}

func (c *ResponsesToolCodeInterpreterContainer) MarshalJSON() ([]byte, error) {
	if c.ResponsesToolCodeInterpreterContainerString != nil {
		return Marshal(*c.ResponsesToolCodeInterpreterContainerString)
	}
	if c.ResponsesToolCodeInterpreterContainerStruct != nil {
		return Marshal(*c.ResponsesToolCodeInterpreterContainerStruct)
	}
	return nil, fmt.Errorf("container field is neither a string nor a ResponsesToolCodeInterpreterContainerStruct object")
}

func (c *ResponsesToolCodeInterpreterContainer) UnmarshalJSON(data []byte) error {
	// First, try to unmarshal as a direct string
	var containerString string
	if err := Unmarshal(data, &containerString); err == nil {
		c.ResponsesToolCodeInterpreterContainerString = &containerString
		return nil
	}
	// Try to unmarshal as a ResponsesToolCodeInterpreterContainerStruct object
	var containerStruct ResponsesToolCodeInterpreterContainerStruct
	if err := Unmarshal(data, &containerStruct); err == nil {
		c.ResponsesToolCodeInterpreterContainerStruct = &containerStruct
		return nil
	}
	return fmt.Errorf("container field is neither a string nor a ResponsesToolCodeInterpreterContainerStruct object")
}

type ResponsesToolCodeInterpreterContainerStruct struct {
