...
const (
	...
	BifrostContextKeyGovernanceEffectiveAccess           BifrostContextKey = "bf-governance-effective-access"         // *grants.EffectiveAccess (what the request may reach, resolved once from the grants it carries; every consumer reads this one answer instead of deriving its own - reserved, writable during a hook only by the plugin a deployment named for it - set by bifrost governance plugin - DO NOT SET THIS MANUALLY)
