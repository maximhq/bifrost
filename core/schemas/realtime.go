...
type RealtimeProvider interface {
	...
	RealtimeWebRTCHeaders(authorization string) map[string]string
...
func ParseRealtimeEvent(raw []byte) (*BifrostRealtimeEvent, error) {
	...
	if event.Session != nil {
		...
		if rawSession, ok := sessionRaw(root, raw); ok && json.Unmarshal(rawSession, &sessionRoot) == nil {
	...
	if event.Item != nil {
		...
		if rawItem, ok := itemRaw(root, raw); ok && json.Unmarshal(rawItem, &itemRoot) == nil {
			for _, key := range []string{
	...
	if event.Error != nil {
		...
		if rawError, ok := errorRaw(root, raw); ok && json.Unmarshal(rawError, &errorRoot) == nil {
...
func sessionRaw(root map[string]json.RawMessage, raw []byte) (json.RawMessage, bool) {
	if session, ok := root["session"]; ok && len(session) > 0 {
		return session, true
	}
	...
	session, ok := all["session"]
	return session, ok && len(session) > 0
}

func itemRaw(root map[string]json.RawMessage, raw []byte) (json.RawMessage, bool) {
	if item, ok := root["item"]; ok && len(item) > 0 {
		return item, true
	}
	var all map[string]json.RawMessage
	if err := json.Unmarshal(raw, &all); err != nil {
	...
	item, ok := all["item"]
	return item, ok && len(item) > 0
}

func errorRaw(root map[string]json.RawMessage, raw []byte) (json.RawMessage, bool) {
	if rtErr, ok := root["error"]; ok && len(rtErr) > 0 {
		return rtErr, true
	}
	var all map[string]json.RawMessage
	if err := json.Unmarshal(raw, &all); err != nil {
