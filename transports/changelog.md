...
- **LLM Gateway Provider** - `config.json` now accepts `llmgateway` as a provider and base provider type, for routing through the new LLM Gateway (llmgateway.io) integration.
