...
func main() {
	...
	store, err := lib.NewConfigStore(logger, configDB)
	if err != nil {
		log.Fatalf("failed to initialize config store: %v", err)
	}

	// Load configuration from database into the store with full preprocessing
