		...
		if config.ConfigStore != nil && config.MCPConfig != nil {
			...
			for _, clientConfig := range config.MCPConfig.ClientConfigs {
				...
				if schemasClientConfig != nil && len(schemasClientConfig.ClientConfigs) > 0 {
					if err := config.ConfigStore.CreateMCPClientConfig(ctx, schemasClientConfig.ClientConfigs[0], config.EnvKeys); err != nil {
	...
	if mcpConfig != nil {
		for _, clientConfig := range mcpConfig.ClientConfigs {
			...
			for toolName, costPerExecution := range dbClientConfig.ToolPricing {
				// toolName in DB already includes the client prefix (e.g., "web_search_exa")
				// So we use it directly as the key
		...
		if clientConfig.ConnectionString != nil {
			...
			if envVar != "" {
				c.EnvKeys[envVar] = append(c.EnvKeys[envVar], configstore.EnvKeyInfo{
					...
					ConfigPath: fmt.Sprintf("mcp.client_configs.%s.connection_string", clientConfig.ClientID),
			...
			for header, value := range clientConfig.Headers {
				...
				if envVar != "" {
					c.EnvKeys[envVar] = append(c.EnvKeys[envVar], configstore.EnvKeyInfo{
						...
						Provider:   "",
						KeyType:    "mcp_header",
						ConfigPath: fmt.Sprintf("mcp.client_configs.%s.headers.%s", clientConfig.ClientID, header),
...
func (c *Config) AddMCPClient(ctx context.Context, clientConfig schemas.MCPClientConfig) error {
	...
	newEnvKeys := make(map[string]struct{})
	tableClientConfig := convertSchemasMCPClientConfigToTable(clientConfig)
	c.MCPConfig.ClientConfigs = append(c.MCPConfig.ClientConfigs, tableClientConfig)
	...
	// Convert to schemas format for Bifrost client
	schemasConfig := configstore.ConvertTableMCPConfigToSchemas(&configstoreTables.MCPConfig{
		ClientConfigs: []configstoreTables.TableMCPClient{c.MCPConfig.ClientConfigs[len(c.MCPConfig.ClientConfigs)-1]},
	})
	if schemasConfig == nil || len(schemasConfig.ClientConfigs) == 0 {
		c.MCPConfig.ClientConfigs = c.MCPConfig.ClientConfigs[:len(c.MCPConfig.ClientConfigs)-1]
		c.cleanupEnvKeys("", clientConfig.ID, newEnvKeys)
		return fmt.Errorf("failed to convert client config")
	}
	// Config with processed env vars
	if err := c.client.AddMCPClient(schemasConfig.ClientConfigs[0]); err != nil {
		c.MCPConfig.ClientConfigs = c.MCPConfig.ClientConfigs[:len(c.MCPConfig.ClientConfigs)-1]
		c.cleanupEnvKeys("", clientConfig.ID, newEnvKeys)
...
func (c *Config) EditMCPClient(ctx context.Context, id string, updatedConfig configstoreTables.TableMCPClient) error {
	...
	// Note: Tool names in configs (tools_to_execute, tools_to_auto_execute, tool_pricing)
	// are stored WITHOUT server prefixes. The server prefix is only added internally
	// when tools are loaded into memory (format: server/toolname).
	// Therefore, we don't need to update tool names when the server name changes.

	// Preserve read-only fields from old config
	processedConfig.ID = oldConfig.ID
	processedConfig.ClientID = oldConfig.ClientID
	processedConfig.ConnectionType = oldConfig.ConnectionType
	processedConfig.ConnectionString = oldConfig.ConnectionString
	processedConfig.StdioConfig = oldConfig.StdioConfig

	// Update the in-memory config with the processed values
	c.MCPConfig.ClientConfigs[configIndex] = processedConfig

	// Convert to schemas.MCPClientConfig for runtime bifrost client (without tool_pricing)
	schemasConfig := schemas.MCPClientConfig{
		ID:                 processedConfig.ClientID,
		Name:               processedConfig.Name,
		IsCodeModeClient:   processedConfig.IsCodeModeClient,
		ConnectionType:     schemas.MCPConnectionType(processedConfig.ConnectionType),
		ConnectionString:   processedConfig.ConnectionString,
		StdioConfig:        processedConfig.StdioConfig,
		ToolsToExecute:     processedConfig.ToolsToExecute,
		ToolsToAutoExecute: processedConfig.ToolsToAutoExecute,
	...
	if c.ConfigStore != nil {
		if err := c.ConfigStore.UpdateMCPClientConfig(ctx, id, processedConfig, c.EnvKeys); err != nil {
		...
		if c.MCPCatalog != nil {
			...
			if processedConfig.Name != oldConfig.Name {
				for toolName := range oldConfig.ToolPricing {
					c.MCPCatalog.DeletePricingData(oldConfig.Name, toolName)
				}
				logger.Debug("deleted old MCP catalog pricing for renamed client: %s -> %s (%d tools)", oldConfig.Name, processedConfig.Name, len(oldConfig.ToolPricing))
			} else {
				// If name hasn't changed, remove pricing entries that were deleted
				for toolName := range oldConfig.ToolPricing {
					if _, exists := processedConfig.ToolPricing[toolName]; !exists {
						c.MCPCatalog.DeletePricingData(processedConfig.Name, toolName)
					}
				}
			}
			// Then, add or update pricing entries from the new config (with new name if changed)
			for toolName, costPerExecution := range processedConfig.ToolPricing {
				c.MCPCatalog.UpdatePricingData(processedConfig.Name, toolName, costPerExecution)
			}
			logger.Debug("updated MCP catalog pricing for client: %s (%d tools)", processedConfig.Name, len(processedConfig.ToolPricing))
		}
	}
	return nil
}
