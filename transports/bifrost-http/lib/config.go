...
func LoadConfig(ctx context.Context, configDirPath string) (*Config, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			config.ConfigStore, err = configstore.NewConfigStore(&configstore.Config{
				Enabled: true,
				Type:    string(configstore.ConfigStoreTypeSqlite),
	...
	if configData.LogsStoreConfig != nil && configData.LogsStoreConfig.Enabled {
		...
		config.LogsStore, err = logstore.NewLogStore(configData.LogsStoreConfig)
		if err != nil {
			return nil, err
		}
	...
	if configData.VectorStoreConfig != nil && configData.VectorStoreConfig.Enabled {
		...
		if err != nil {
			logger.Warn("Failed to create vector store: %v", err)
		}
		if config.ConfigStore != nil {
...
func (s *Config) GetCacheConfigRedacted() (*vectorstore.Config, error) {
	...
	if vectorStoreConfig.Type == vectorstore.VectorStoreTypeRedisCluster {
		...
		if redisClusterConfig.Password != "" {
			redisClusterConfig.Password = RedactKey(redisClusterConfig.Password)
		}
		return &redactedConfig, nil
	}
	return nil, nil
