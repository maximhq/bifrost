...
func LoadConfig(configDirPath string) (*Config, error) {
	...
	logger.Debug("config file contents: %s", string(data))
	...
	if processedProviders == nil {
			...
			for providerName, cfg := range configData.Providers {
				...
				for i, key := range cfg.Keys {
					...
					if err != nil {
						...
						logger.Warn(fmt.Sprintf("failed to process env vars in keys for %s: %v", provider, err))
						continue
					}
					...
					if key.AzureKeyConfig != nil {
						if err := config.processAzureKeyConfigEnvVars(&cfg.Keys[i], provider, i, newEnvKeys); err != nil {
							config.cleanupEnvKeys(string(provider), "", newEnvKeys)
							logger.Warn(fmt.Sprintf("failed to process Azure key config env vars for %s: %v", provider, err))
							continue
						}
					}
					...
					if key.VertexKeyConfig != nil {
						if err := config.processVertexKeyConfigEnvVars(&cfg.Keys[i], provider, i, newEnvKeys); err != nil {
							config.cleanupEnvKeys(string(provider), "", newEnvKeys)
							logger.Warn(fmt.Sprintf("failed to process Vertex key config env vars for %s: %v", provider, err))
							continue
						}
					}
					...
					if key.BedrockKeyConfig != nil {
						if err := config.processBedrockKeyConfigEnvVars(&cfg.Keys[i], provider, i, newEnvKeys); err != nil {
							config.cleanupEnvKeys(string(provider), "", newEnvKeys)
							logger.Warn(fmt.Sprintf("failed to process Bedrock key config env vars for %s: %v", provider, err))
							continue
						}
					}
				}
		...
		if config.ConfigStore != nil {
			...
			if err != nil {
				logger.Warn(fmt.Sprintf("Failed to update providers config: %v", err))
			}
		}
	}
	...
	if config.MCPConfig == nil && configData.MCP != nil {
		...
		config.processMCPEnvVars()
		if config.ConfigStore != nil {
			err = config.ConfigStore.UpdateMCPConfig(config.MCPConfig)
			if err != nil {
				logger.Warn(fmt.Sprintf("Failed to update MCP config: %v", err))
			}
		}
	}
	...
	if configData.VectorStoreConfig != nil && configData.VectorStoreConfig.Enabled {
		...
		config.VectorStore, err = vectorstore.NewVectorStore(configData.VectorStoreConfig)
		if err != nil {
			logger.Warn(fmt.Sprintf("Failed to create vector store: %v", err))
		}
		if config.ConfigStore != nil {
			err = config.ConfigStore.UpdateVectorStoreConfig(configData.VectorStoreConfig)
			if err != nil {
				logger.Warn(fmt.Sprintf("Failed to update cache config: %v", err))
			}
		}
	}
...
func (s *Config) AddProvider(provider schemas.ModelProvider, config configstore.ProviderConfig) error {
	...
	logger.Info(fmt.Sprintf("Added provider: %s", provider))
	return nil
}
...
func (s *Config) UpdateProviderConfig(provider schemas.ModelProvider, config configstore.ProviderConfig) error {
	...
	logger.Info(fmt.Sprintf("Updated configuration for provider: %s", provider))
	return nil
}
...
func (s *Config) processMCPEnvVars() error {
	...
	for i, clientConfig := range s.MCPConfig.ClientConfigs {
		...
		if clientConfig.ConnectionString != nil {
			...
			if err != nil {
				logger.Warn(fmt.Sprintf("failed to process env vars in MCP client %s: %v", clientConfig.Name, err))
...
func (s *Config) autoDetectProviders() {
	...
	for provider, envVars := range providerEnvVars {
		for _, envVar := range envVars {
			if apiKey := os.Getenv(envVar); apiKey != "" {
				...
				logger.Info(fmt.Sprintf("Auto-detected %s provider from environment variable %s", provider, envVar))
				detectedCount++
	...
	if detectedCount > 0 {
		logger.Info(fmt.Sprintf("Auto-configured %d provider(s) from environment variables", detectedCount))
		if s.ConfigStore != nil {
			if err := s.ConfigStore.UpdateProvidersConfig(s.Providers); err != nil {
				logger.Error(fmt.Errorf("failed to update providers in store: %v", err))
			}
		}
	}
}
...
func (s *Config) GetCacheConfigRedacted() (*vectorstore.Config, error) {
	var err error
	...
	if vectorStoreConfig.Type == vectorstore.VectorStoreTypeRedis {
		redisConfig, ok := vectorStoreConfig.Config.(*vectorstore.RedisVectorStoreConfig)
		if !ok {
		...
		if redisConfig.Password != "" {
			redisConfig.Password = RedactKey(redisConfig.Password)
		}
		return &redactedConfig, nil
	}
	return nil, nil
