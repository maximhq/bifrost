...
func (s *ConfigStore) loadDefaultConfig() error {
	...
	s.writeConfigToFile(s.configPath)
	...
	if err := s.db.Create(&newHash).Error; err != nil {
...
func (s *ConfigStore) loadWithFileCheck(configFile string) error {
	...
	err = s.db.Order("created_at DESC").First(&latestHash).Error
