...
func createBedrockBatchRouteConfigs(pathPrefix string, handlerStore lib.HandlerStore) []RouteConfig {
	...
	// Create batch job endpoint - POST /model-invocation-jobs
	routes = append(routes, RouteConfig{
		...
		BatchCreateRequestConverter: func(ctx *context.Context, req interface{}) (*BatchRequest, error) {
			if bedrockReq, ok := req.(*bedrock.BedrockBatchJobRequest); ok {
				...
				// Note: Bedrock uses S3 URIs for input/output
				...
				// Store Bedrock-specific config in metadata or extra params
				if bedrockReq.JobName != "" {
					createReq.Metadata["job_name"] = bedrockReq.JobName
				}

				createReq.ExtraParams = map[string]interface{}{
					"roleArn":          bedrockReq.RoleArn,
					"inputDataConfig":  bedrockReq.InputDataConfig,
					"outputDataConfig": bedrockReq.OutputDataConfig,
				}

				if bedrockReq.TimeoutDurationInHours > 0 {
					createReq.ExtraParams["timeoutDurationInHours"] = bedrockReq.TimeoutDurationInHours
				}
		...
		PreCallback: func(ctx *fasthttp.RequestCtx, bifrostCtx *context.Context, req interface{}) error {
			...
			if bedrockReq, ok := req.(*bedrock.BedrockBatchJobRequest); ok {
				if provider != nil {
					bedrockReq.Provider = schemas.ModelProvider(provider)
				} else {
	...
	routes = append(routes, RouteConfig{
		...
		BatchCreateRequestConverter: func(ctx *context.Context, req interface{}) (*BatchRequest, error) {
			if listReq, ok := req.(*schemas.BifrostBatchListRequest); ok {
				if listReq.Provider == "" {
					listReq.Provider = schemas.Bedrock
				}
				return &BatchRequest{
	...
	// Get batch job endpoint - GET /model-invocation-jobs/{job_arn}
	routes = append(routes, RouteConfig{
		Type:   RouteConfigTypeBedrock,
		Path:   pathPrefix + "/model-invocation-jobs/{job_arn}",
		Method: "GET",
		GetRequestTypeInstance: func() interface{} {
		...
		BatchCreateRequestConverter: func(ctx *context.Context, req interface{}) (*BatchRequest, error) {
			if retrieveReq, ok := req.(*schemas.BifrostBatchRetrieveRequest); ok {
				if retrieveReq.Provider == "" {
					retrieveReq.Provider = schemas.Bedrock
				}
				return &BatchRequest{
	...
	// Stop batch job endpoint - POST /model-invocation-jobs/{job_arn}/stop
	routes = append(routes, RouteConfig{
		Type:   RouteConfigTypeBedrock,
		Path:   pathPrefix + "/model-invocation-jobs/{job_arn}/stop",
		Method: "POST",
		GetRequestTypeInstance: func() interface{} {
		...
		BatchCreateRequestConverter: func(ctx *context.Context, req interface{}) (*BatchRequest, error) {
			if cancelReq, ok := req.(*schemas.BifrostBatchCancelRequest); ok {
				if cancelReq.Provider == "" {
					cancelReq.Provider = schemas.Bedrock
				}
				return &BatchRequest{
	...
	return func(ctx *fasthttp.RequestCtx, bifrostCtx *context.Context, req interface{}) error {
		...
		if listReq, ok := req.(*schemas.BifrostBatchListRequest); ok {
			...
			if nextToken := string(ctx.QueryArgs().Peek("nextToken")); nextToken != "" {
				listReq.PageToken = &nextToken
			}

			// Extract status filter
			if status := string(ctx.QueryArgs().Peek("statusEquals")); status != "" {
				if listReq.ExtraParams == nil {
					listReq.ExtraParams = make(map[string]interface{})
				}
				listReq.ExtraParams["statusEquals"] = status
			}
			...
			if name := string(ctx.QueryArgs().Peek("nameContains")); name != "" {
				...
				listReq.ExtraParams["nameContains"] = name
			}
		}
...
func extractBedrockJobArnFromPath(handlerStore lib.HandlerStore) PreRequestCallback {
	return func(ctx *fasthttp.RequestCtx, bifrostCtx *context.Context, req interface{}) error {
		...
		case *schemas.BifrostBatchRetrieveRequest:
			r.BatchID = decodedJobArn
			r.Provider = schemas.Bedrock
...
func createBedrockFilesRouteConfigs(pathPrefix string, handlerStore lib.HandlerStore) []RouteConfig {
	...
	routes = append(routes, RouteConfig{
		...
		FileListResponseConverter: func(ctx *context.Context, resp *schemas.BifrostFileListResponse) (interface{}, error) {
			// Get bucket and prefix from context
			bucket := ""
			prefix := ""
			maxKeys := 1000
...
func parseS3PutObjectRequest(ctx *fasthttp.RequestCtx, req interface{}) error {
	...
	if !ok {
		fmt.Println("invalid request type for S3 PutObject")
...
func extractS3ListObjectsV2Params(handlerStore lib.HandlerStore) PreRequestCallback {
	return func(ctx *fasthttp.RequestCtx, bifrostCtx *context.Context, req interface{}) error {
		...
		*bifrostCtx = context.WithValue(*bifrostCtx, s3ContextKeyPrefix, key)
