...
func CreateOpenAIRouteConfigs(pathPrefix string, handlerStore lib.HandlerStore) []RouteConfig {
	...
	for _, path := range []string{
		...
		routes = append(routes, RouteConfig{
			...
			RequestConverter: func(ctx *schemas.BifrostContext, req interface{}) (*schemas.BifrostRequest, error) {
				if videoRetrieveReq, ok := req.(*schemas.BifrostVideoRetrieveRequest); ok {
					if videoRetrieveReq.Provider == schemas.OpenAI {
						return &schemas.BifrostRequest{
							VideoDownloadRequest: videoRetrieveReq,
						}, nil
					} else {
						return &schemas.BifrostRequest{
							VideoRetrieveRequest: videoRetrieveReq,
				...
				return nil, errors.New("invalid video download request type")
			},
			...
			VideoRetrieveAsDownloadConverter: func(ctx *schemas.BifrostContext, resp *schemas.BifrostVideoGenerationResponse) (interface{}, error) {
				// here convert the videos output, url or base64 encoded video to openai format
				return openai.ToOpenaiVideoDownloadResponse(resp)
			},
			ErrorConverter: func(ctx *schemas.BifrostContext, err *schemas.BifrostError) interface{} {
				return err
			},
