...
type RouteConfig struct {
	...
	VideoRetrieveAsDownloadConverter       VideoRetrieveAsDownloadConverter       // Function to convert BifrostVideoGenerationResponse to integration format (SHOULD NOT BE NIL)
...
func (g *GenericRouter) handleNonStreamingRequest(ctx *fasthttp.RequestCtx, config RouteConfig, req interface{}, bifrostReq *schemas.BifrostRequest, bifrostCtx *schemas.BifrostContext) {
	...
	case bifrostReq.VideoRetrieveRequest != nil:
		...
		// If this route is acting as a "download via retrieve" endpoint, use the dedicated converter
		if config.VideoRetrieveAsDownloadConverter != nil {
			convertedResp, err := config.VideoRetrieveAsDownloadConverter(bifrostCtx, videoRetrieveResponse)
			if err != nil {
				if errors.Is(err, openai.ErrVideoNotReady) {
					g.sendError(ctx, bifrostCtx, config.ErrorConverter, &schemas.BifrostError{
						IsBifrostError: false,
						StatusCode:     schemas.Ptr(int(fasthttp.StatusNotFound)),
						Error: &schemas.ErrorField{
							Type:    schemas.Ptr("invalid_request_error"),
							Message: "Video is not ready yet, use GET /v1/videos/{video_id} to check status",
						},
					})
					return
				}
				g.sendError(ctx, bifrostCtx, config.ErrorConverter, newBifrostError(err, "failed to convert video retrieve response to download"))
				return
			}
			// Reuse the same binary-write logic as VideoDownloadRequest
			if rawBytes, ok := convertedResp.([]byte); ok {
				// detect content type or default
