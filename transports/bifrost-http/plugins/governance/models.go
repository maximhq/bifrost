...
type Budget struct {
	...
	MaxLimit      int64     `gorm:"not null" json:"max_limit"`                       // Maximum budget in cents
	...
	CurrentUsage  int64     `gorm:"default:0" json:"current_usage"`                  // Current usage in cents
...
func (b *Budget) BeforeSave(tx *gorm.DB) error {
	...
	if b.MaxLimit < 0 {
		return fmt.Errorf("budget max_limit cannot be negative: %d", b.MaxLimit)
	}
