...
func (p *LoggerPlugin) createFTSTable() error {
	...
	triggerSQL := []string{
		`CREATE TRIGGER IF NOT EXISTS logs_fts_insert AFTER INSERT ON logs BEGIN
			INSERT INTO logs_fts(rowid, content_summary) VALUES (new.id, new.content_summary);
		END`,
		`CREATE TRIGGER IF NOT EXISTS logs_fts_delete AFTER DELETE ON logs BEGIN
			INSERT INTO logs_fts(logs_fts, rowid, content_summary) VALUES('delete', old.id, old.content_summary);
		END`,
