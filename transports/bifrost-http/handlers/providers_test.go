...
type mockModelsManager struct {
	...
	refreshErr           error
	// access is what the request may reach; nil stands for a request with nothing resolved —
	// no key presented, or a deployment without governance.
	access       *grants.EffectiveAccess
	resolveCalls int
}

func (m *mockModelsManager) ResolveEffectiveAccess(_ *schemas.BifrostContext) *grants.EffectiveAccess {
	m.resolveCalls++
	return m.access
}
...
// accessForProviderGrants builds the access a key-authenticated caller carries, so these tests
// express what the caller may reach rather than a copy of the key's rows.
func accessForProviderGrants(configs ...grants.ProviderConfigGrant) *grants.EffectiveAccess {
	grant := &grants.Grant{
		Type:                 grants.GrantTypeVirtualKey,
		ID:                   "vk-test",
		Name:                 "Test VK",
		ProviderConfigGrants: configs,
	}
	return grants.NewEffectiveAccess(grant, nil, "", nil, nil)
}
...
func TestListModels_VKFilterHidesBlacklistedModel(t *testing.T) {
	...
	query := modelListQuery{
		Limit:       100,
		HasVKFilter: true,
		Access: accessForProviderGrants(grants.ProviderConfigGrant{
			Provider:          "openai",
...
func TestListModels_VKFilterUnionsDuplicateProviderConfigs(t *testing.T) {
	...
	query := modelListQuery{
		Limit:       100,
		HasVKFilter: true,
		Access: accessForProviderGrants(
			grants.ProviderConfigGrant{Provider: "openai", AllowedModels: schemas.WhiteList{"gpt-4o"}},
			grants.ProviderConfigGrant{Provider: "openai", AllowedModels: schemas.WhiteList{"gpt-4o-mini"}},
		),
	}
...
func TestListModels_VKFilterRestrictsToAllowedProviderAndModels(t *testing.T) {
	...
	query := modelListQuery{
		Limit:       100,
		HasVKFilter: true,
		Access:      accessForProviderGrants(grants.ProviderConfigGrant{Provider: "openai", AllowedModels: schemas.WhiteList{"gpt-4o", "gpt-4o-mini"}}),
	}
...
func TestListModels_VKFilterAllowsAllModelsWithWildcard(t *testing.T) {
	...
	query := modelListQuery{
		Limit:       100,
		HasVKFilter: true,
		Access:      accessForProviderGrants(grants.ProviderConfigGrant{Provider: "openai", AllowedModels: schemas.WhiteList{"*"}}),
	}
...
func TestListModels_VKFilterNoProviderConfigsDeniesAll(t *testing.T) {
	...
	query := modelListQuery{
		Limit:       100,
		HasVKFilter: true,
		Access:      accessForProviderGrants(), // a caller granted no provider
	}
...
func TestListModels_VKFilterBlockedExplicitProviderReturnsEmptyResult(t *testing.T) {
	...
	query.Access = accessForProviderGrants(grants.ProviderConfigGrant{Provider: "openai", AllowedModels: schemas.WhiteList{"*"}})
...
// (unknown key, or a deployment without governance) are the same case — the listing stays
