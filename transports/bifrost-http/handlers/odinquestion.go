...
//
// Two things make a metric question answerable: when, and whose. Odin can guess
...
//
// Rather than a free-text "which period did you mean?", Odin poses a structured
...
// form Odin can use directly instead of one it has to re-interpret.
//
...
const (
	// odinAskUserTool is the tool name the model calls to pose a question.
	odinAskUserTool = "ask_user"

	// odinMaxQuestionOptions bounds an option list. Past this it stops being a
	// picker and becomes a list to read, which is what the question was meant to
	// avoid.
	odinMaxQuestionOptions = 8
)

// odinQuestion is a structured question posed to the person asking.
type odinQuestion struct {
	Question string            `json:"question"`
	Options  []odinQuestionOpt `json:"options"`
...
type odinQuestionOpt struct {
	Label string `json:"label"`
	// Hint is the shorter form Odin should receive back, so the answer needs no
	// re-interpretation: "-7d" rather than "Last 7 days".
	Hint string `json:"hint,omitempty"`
}

// odinAskUserSchema is the tool's argument schema.
//
...
// odinAskUserToolDef builds the tool.
//
...
func odinAskUserToolDef() odinTool {
	return odinTool{
		name: odinAskUserTool,
		...
		schemaJSON: odinAskUserSchema,
		execute: func(_ context.Context, _ *odinToolDeps, args map[string]any) (any, error) {
			question, err := parseOdinQuestion(args)
			if err != nil {
				return nil, err
			}
...
// parseOdinQuestion validates the model's question.
func parseOdinQuestion(args map[string]any) (*odinQuestion, error) {
	...
	if len(rawOptions) > odinMaxQuestionOptions {
		rawOptions = rawOptions[:odinMaxQuestionOptions]
	}

	options := make([]odinQuestionOpt, 0, len(rawOptions))
	for _, raw := range rawOptions {
		...
		options = append(options, odinQuestionOpt{Label: strings.TrimSpace(label), Hint: strings.TrimSpace(hint)})
	}
	if len(options) < 2 {
	...
	return &odinQuestion{Question: text, Options: options, AllowOther: allowOther, Kind: kind}, nil
}

// odinQuestionFromToolCall extracts a question from a tool call the agent is
// about to execute, or nil when that call is something else.
func odinQuestionFromToolCall(name string, arguments string) *odinQuestion {
	if name != odinAskUserTool {
		return nil
	}
	args := map[string]any{}
...
// odinQuestionGuidance is appended to the system prompt. It lives here beside
// the tool so the rules and the thing they describe cannot drift apart.
const odinQuestionGuidance = `

Asking before you answer:

- Two things decide a metric answer: which time range, and whose traffic. If the question does not say, ask with ` + odinAskUserTool + ` rather than choosing for them. A number computed over the wrong window or the wrong scope is not a smaller answer, it is a different one.
