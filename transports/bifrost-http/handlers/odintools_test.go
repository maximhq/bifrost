...
// fakeOdinLogManager records what the tools asked for. Only the methods Odin's
...
// with a nil-pointer panic in tests rather than silently widening Odin's reach.
type fakeOdinLogManager struct {
	odinLogManagerStub
...
func (f *fakeOdinLogManager) GetDimensionRankings(ctx context.Context, filters *logstore.SearchFilters, dimension logstore.RankingDimension) (*logstore.DimensionRankingResult, error) {
	f.sawContext = ctx
	f.rankingFilters, f.rankingDimension = filters, dimension
	return &logstore.DimensionRankingResult{}, nil
}

func (f *fakeOdinLogManager) GetModelRankings(ctx context.Context, filters *logstore.SearchFilters) (*logstore.ModelRankingResult, error) {
	f.sawContext = ctx
...
func (f *fakeOdinLogManager) GetCostHistogram(ctx context.Context, filters *logstore.SearchFilters, bucketSizeSeconds int64) (*logstore.CostHistogramResult, error) {
	f.sawContext = ctx
	f.histogramBucket = bucketSizeSeconds
	return &logstore.CostHistogramResult{}, nil
}

func runOdinTool(t *testing.T, name string, deps *odinToolDeps, args map[string]any) (any, error) {
	t.Helper()
	tool, ok := odinToolByName(buildOdinTools(), name)
...
func TestOdinToolSchemasAreValid(t *testing.T) {
...
// model asks for.
func TestOdinQueryLogsClampsLimit(t *testing.T) {
	fake := &fakeOdinLogManager{}
	deps := &odinToolDeps{logManager: fake}

	_, err := runOdinTool(t, "query_logs", deps, map[string]any{
		"filters": map[string]any{},
		"limit":   float64(5000),
	})
	require.NoError(t, err)
	require.Equal(t, odinMaxLogRows, fake.searchPagination.Limit)
}

func TestOdinRankingClampsLimit(t *testing.T) {
	fake := &fakeOdinLogManager{}
	deps := &odinToolDeps{logManager: fake}

	_, err := runOdinTool(t, "query_virtual_key_usage", deps, map[string]any{
		"filters": map[string]any{},
		"limit":   float64(9999),
	})
	require.NoError(t, err)
	require.NotNil(t, fake.rankingFilters.RankingLimit)
	require.Equal(t, odinMaxRankingRows, *fake.rankingFilters.RankingLimit)
	require.Equal(t, logstore.RankingDimensionVirtualKey, fake.rankingDimension)
}

func TestOdinUserFlowUsesUserDimension(t *testing.T) {
	fake := &fakeOdinLogManager{}
	_, err := runOdinTool(t, "query_user_usage", &odinToolDeps{logManager: fake}, map[string]any{
		"filters": map[string]any{},
	})
	require.NoError(t, err)
...
func TestOdinRejectsUnknownFilterField(t *testing.T) {
	fake := &fakeOdinLogManager{}
	_, err := runOdinTool(t, "query_logs", &odinToolDeps{logManager: fake}, map[string]any{
...
func TestOdinFilterTimeParsing(t *testing.T) {
	now := time.Date(2026, 8, 17, 12, 0, 0, 0, time.UTC)

	t.Run("relative days", func(t *testing.T) {
		filters, err := parseOdinFilters(map[string]any{"start_time": "-7d"}, now)
		require.NoError(t, err)
	...
	t.Run("relative hours", func(t *testing.T) {
		filters, err := parseOdinFilters(map[string]any{"start_time": "-30m"}, now)
		require.NoError(t, err)
		require.Equal(t, now.Add(-30*time.Minute), *filters.StartTime)
	})

	t.Run("absolute rfc3339", func(t *testing.T) {
		filters, err := parseOdinFilters(map[string]any{"start_time": "2026-08-01T00:00:00Z"}, now)
		require.NoError(t, err)
	...
	t.Run("defaults to last 24h", func(t *testing.T) {
		filters, err := parseOdinFilters(nil, now)
		require.NoError(t, err)
		require.Equal(t, now.Add(-odinDefaultLookback), *filters.StartTime)
	})

	t.Run("rejects inverted range", func(t *testing.T) {
		_, err := parseOdinFilters(map[string]any{
	...
	t.Run("rejects unparseable offset", func(t *testing.T) {
		_, err := parseOdinFilters(map[string]any{"start_time": "last tuesday"}, now)
...
// without hedging.
func TestOdinBoundToolResultReplacesRatherThanTruncates(t *testing.T) {
	huge := make([]string, 4000)
	for i := range huge {
		huge[i] = fmt.Sprintf("row-%d-with-some-padding-to-make-this-large", i)
	}
	bounded := boundOdinToolResult(map[string]any{"rows": huge})
	...
	require.Less(t, len(bounded), odinMaxToolResultBytes)
}

func TestOdinBoundToolResultPassesSmallPayloads(t *testing.T) {
	bounded := boundOdinToolResult(map[string]any{"total": 42})
...
// Odin is an API like any other and must not be the place it resurfaces.
func TestOdinNeverReturnsHiddenContent(t *testing.T) {
	entry := &logstore.Log{
		ID:             "hidden-row",
	...
	row := projectOdinLog(entry, true, odinDetailContentChars)
	require.Empty(t, row.Content)
	require.Equal(t, "hidden-row", row.ID)
}

func TestOdinIncludesContentOnlyWhenAsked(t *testing.T) {
	entry := &logstore.Log{
		ID:             "visible-row",
		Timestamp:      time.Now().UTC(),
		Provider:       "openai",
	...
	require.Empty(t, projectOdinLog(entry, false, odinLogContentChars).Content)
	require.Equal(t, "what is the weather", projectOdinLog(entry, true, odinLogContentChars).Content)
}

func TestOdinTruncatesLongContent(t *testing.T) {
	entry := &logstore.Log{
		ID:             "long-row",
		Timestamp:      time.Now().UTC(),
		ContentSummary: strings.Repeat("x", odinLogContentChars*3),
	}
	row := projectOdinLog(entry, true, odinLogContentChars)
	require.Contains(t, row.Content, "[truncated]")
	require.Less(t, len(row.Content), odinLogContentChars*2)
}
...
func TestOdinUsesDenormalizedTokenColumns(t *testing.T) {
	entry := &logstore.Log{
		ID:               "tokens",
		Timestamp:        time.Now().UTC(),
		ContentHidden:    true,
	...
	row := projectOdinLog(entry, false, odinLogContentChars)
	require.Equal(t, 120, row.InputTokens)
	require.Equal(t, 45, row.OutputTokens)
}

func TestOdinQueryLogsReportsTotalSeparately(t *testing.T) {
	fake := &fakeOdinLogManager{searchResult: &logstore.SearchResult{
	...
	result, err := runOdinTool(t, "query_logs", &odinToolDeps{logManager: fake}, map[string]any{
		"filters": map[string]any{},
	})
	require.NoError(t, err)
...
func TestOdinMetricsRequiresAtLeastOneMetric(t *testing.T) {
...
func TestOdinMetricsRejectsUnknownMetric(t *testing.T) {
	_, err := runOdinTool(t, "query_metrics", &odinToolDeps{logManager: &fakeOdinLogManager{}}, map[string]any{
		"filters": map[string]any{},
		"metrics": []any{"vibes"},
	})
...
func TestOdinMetricsSummaryUsesStats(t *testing.T) {
	fake := &fakeOdinLogManager{}
	_, err := runOdinTool(t, "query_metrics", &odinToolDeps{logManager: fake}, map[string]any{
		"filters": map[string]any{},
		"metrics": []any{"summary"},
	})
	require.NoError(t, err)
...
func TestOdinToolsPassCallerContextToStore(t *testing.T) {
	type scopeKey struct{}
	fake := &fakeOdinLogManager{}
	tool, ok := odinToolByName(buildOdinTools(), "query_logs")
	require.True(t, ok)

	ctx := context.WithValue(context.Background(), scopeKey{}, "caller-scope")
	_, err := tool.execute(ctx, &odinToolDeps{logManager: fake}, map[string]any{"filters": map[string]any{}})
	require.NoError(t, err)
...
func TestOdinGetLogDetailRequiresID(t *testing.T) {
	_, err := runOdinTool(t, "get_log_detail", &odinToolDeps{logManager: &fakeOdinLogManager{}}, map[string]any{})
...
func TestOdinLogContentHandlesNilMessageContent(t *testing.T) {
	entry := &logstore.Log{
		ID:        "nil-content",
		Timestamp: time.Now().UTC(),
