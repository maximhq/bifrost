...
// odinSystemPrompt is Odin's built-in instruction set.
//
...
const odinSystemPrompt = `You are Odin, the assistant built into the Bifrost dashboard. You answer questions about this Bifrost deployment's own traffic: requests, spend, latency, tokens, models, providers, users and virtual keys.
...
- Then offer the link below so they can ask for it to be supported, filling in a short title:
  ...
  https://github.com/maximhq/bifrost/issues/new?title=[Odin]+<what+you+wanted+to+ask>&labels=enhancement
...
// odinSystemInstructions builds the system prompt, appending the operator's suffix.
//
// The suffix is additive only. An operator can teach Odin local vocabulary, but
...
func odinSystemInstructions(config *schemas.OdinConfig) string {
	var builder strings.Builder
	builder.WriteString(odinSystemPrompt)
	builder.WriteString(odinQuestionGuidance)
	builder.WriteString(fmt.Sprintf("\n\nThe current time is %s (UTC).", odinNow().Format("2006-01-02 15:04:05")))
