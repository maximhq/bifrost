...
func (h *GovernanceHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	r.GET("/api/governance/complexity-router", lib.ChainMiddlewares(h.getComplexityRouterConfig, middlewares...))
	r.PUT("/api/governance/complexity-router", lib.ChainMiddlewares(h.updateComplexityRouterConfig, middlewares...))
...
var complexityRouterTierKeys = []string{"SIMPLE", "MEDIUM", "COMPLEX", "REASONING"}

func validateComplexityRouterConfig(cfg *configstore.ComplexityRouterConfig) error {
	if cfg == nil {
		return fmt.Errorf("complexity router config is required")
	}

	if len(cfg.Models) != len(complexityRouterTierKeys) {
		return fmt.Errorf("models must contain exactly %d tier mappings", len(complexityRouterTierKeys))
	}

	allowedTiers := make(map[string]struct{}, len(complexityRouterTierKeys))
	for _, tier := range complexityRouterTierKeys {
		allowedTiers[tier] = struct{}{}
		modelMapping, ok := cfg.Models[tier]
		if !ok {
			return fmt.Errorf("models.%s is required", tier)
		}
		if err := validateComplexityRouterModelMapping(modelMapping); err != nil {
			return fmt.Errorf("models.%s %w", tier, err)
		}
	}

	for tier := range cfg.Models {
		if _, ok := allowedTiers[tier]; !ok {
			return fmt.Errorf("invalid models tier key %q", tier)
		}
	}

	if cfg.TierBoundaries != nil {
		boundaries := cfg.TierBoundaries
		if !(0 < boundaries.SimpleMedium &&
			boundaries.SimpleMedium < boundaries.MediumComplex &&
			boundaries.MediumComplex < boundaries.ComplexReasoning &&
			boundaries.ComplexReasoning < 1) {
			return fmt.Errorf("tier_boundaries must satisfy 0 < simple_medium < medium_complex < complex_reasoning < 1")
		}
	}
...
func validateComplexityRouterModelMapping(modelMapping string) error {
	modelMapping = strings.TrimSpace(modelMapping)
	parts := strings.SplitN(modelMapping, "/", 2)
	if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
		return fmt.Errorf("must be in provider/model format")
	}
	if !schemas.IsKnownProvider(parts[0]) {
		return fmt.Errorf("contains unknown provider %q", parts[0])
	}
	return nil
}

func (h *GovernanceHandler) getComplexityRouterConfig(ctx *fasthttp.RequestCtx) {
	if h.configStore == nil {
		SendError(ctx, fasthttp.StatusServiceUnavailable, "config store not available")
		return
	}

	persistedCfg, err := h.configStore.GetComplexityRouterConfig(ctx)
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to get complexity router config: %v", err))
		return
	}
	...
	SendJSON(ctx, configstore.ComplexityRouterConfigFromPersisted(persistedCfg))
}

func (h *GovernanceHandler) updateComplexityRouterConfig(ctx *fasthttp.RequestCtx) {
	if h.configStore == nil {
		SendError(ctx, fasthttp.StatusServiceUnavailable, "config store not available")
		return
	}

	var payload configstore.ComplexityRouterConfig
	if err := json.Unmarshal(ctx.PostBody(), &payload); err != nil {
		SendError(ctx, fasthttp.StatusBadRequest, fmt.Sprintf("invalid request format: %v", err))
		return
	}

	if err := validateComplexityRouterConfig(&payload); err != nil {
		SendError(ctx, fasthttp.StatusBadRequest, err.Error())
		return
	}

	existingCfg, err := h.configStore.GetComplexityRouterConfig(ctx)
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to load existing complexity router config: %v", err))
		return
	}

	persistedCfg := configstore.PersistedComplexityRouterConfigFromPublic(&payload, "")
	if existingCfg != nil {
		persistedCfg.ConfigHash = existingCfg.ConfigHash
	}

	if err := h.configStore.UpdateComplexityRouterConfig(ctx, persistedCfg); err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to update complexity router config: %v", err))
		return
	}

	if err := h.governanceManager.ReloadComplexityRouterConfig(ctx); err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("failed to reload complexity router config: %v", err))
		return
	}
