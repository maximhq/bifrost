...
func (h *MCPHandler) getMCPClientsPaginated(ctx *fasthttp.RequestCtx, limitStr, offsetStr, searchStr string) {
	...
	for _, dbClient := range dbClients {
		...
		clientConfig := &schemas.MCPClientConfig{
			...
			OauthConfigID: func() *string {
				if dbClient.OauthConfig != nil {
					id := dbClient.OauthConfig.ID
					return &id
				}
				return nil
			}(),
...
func (h *MCPHandler) updateMCPClient(ctx *fasthttp.RequestCtx) {
	...
	if req.OauthConfig != nil {
		...
		// Embed the updated client config (with the new OauthConfigID) so completeMCPClientOAuth can reconstruct it.
		mergedReq := mergeMCPRedactedValues(&req.TableMCPClient, existingConfig, h.store.RedactMCPClientConfig(existingConfig))
		pendingConfig := *existingConfig
		pendingConfig.OauthConfigID = &prepared.Initiation.OauthConfigID
		pendingConfig.Name = mergedReq.Name
		pendingConfig.IsCodeModeClient = mergedReq.IsCodeModeClient
		pendingConfig.IsPingAvailable = mergedReq.IsPingAvailable
		pendingConfig.Headers = mergedReq.Headers
		pendingConfig.AllowedExtraHeaders = mergedReq.AllowedExtraHeaders
		pendingConfig.ToolsToExecute = resolvedToolsToExecute
		pendingConfig.ToolsToAutoExecute = resolvedToolsToAutoExecute
		pendingConfig.ToolPricing = mergedReq.ToolPricing
		pendingConfig.AllowOnAllVirtualKeys = mergedReq.AllowOnAllVirtualKeys
