...
// The five named query flows Odin exposes, plus a drill-down and a discovery
...
func odinQueryLogsTool() odinTool {
  ...
  "required": ["filters"]
		...
		execute: func(ctx context.Context, deps *odinToolDeps, args map[string]any) (any, error) {
			now := odinNow()
			filters, err := odinFilterArg(args, now, deps.scope)
			if err != nil {
				return nil, err
			}
			limit := odinIntArg(args, "limit", 10, odinMaxLogRows)
			...
			includeContent := odinBoolArg(args, "include_content")
			rows := make([]odinLogRow, 0, len(result.Logs))
			for i := range result.Logs {
				rows = append(rows, projectOdinLog(&result.Logs[i], includeContent, odinLogContentChars))
			}
			...
			return map[string]any{
				...
				"scope":          odinScopeNote(filters, deps.scope),
			}, nil
		},
	}
}
...
func odinGetLogDetailTool() odinTool {
  ...
  "required": ["log_id"]
		...
		execute: func(ctx context.Context, deps *odinToolDeps, args map[string]any) (any, error) {
			...
			return projectOdinLog(entry, true, odinDetailContentChars), nil
		},
	}
}

// odinCountLogsTool answers "how much is there?" before anything answers "what
// is it?".
//
...
func odinCountLogsTool() odinTool {
	return odinTool{
		name: "count_logs",
  ...
  "required": ["filters"]
		...
		execute: func(ctx context.Context, deps *odinToolDeps, args map[string]any) (any, error) {
			...
			case stats.TotalRequests > odinLargeResultThreshold:
...
func odinQueryMetricsTool() odinTool {
  ...
  "required": ["filters", "metrics"]
		...
		execute: func(ctx context.Context, deps *odinToolDeps, args map[string]any) (any, error) {
			...
			metrics := odinStringSlice(args["metrics"])
			if len(metrics) == 0 {
			...
			bucket, err := odinBucketSize(filters)
			if err != nil {
				return nil, err
			}
...
func odinQueryUsersTool() odinTool {
  ...
  "required": ["filters"]
		...
		execute: func(ctx context.Context, deps *odinToolDeps, args map[string]any) (any, error) {
			return odinRankByDimension(ctx, deps, args, logstore.RankingDimensionUser)
		},
	}
}
...
func odinQueryVirtualKeysTool() odinTool {
  ...
  "required": ["filters"]
		...
		execute: func(ctx context.Context, deps *odinToolDeps, args map[string]any) (any, error) {
			return odinRankByDimension(ctx, deps, args, logstore.RankingDimensionVirtualKey)
		},
	}
}

// odinRankByDimension backs the user and virtual-key flows. RankingLimit pushes
// the cap into SQL, so a large deployment never materializes more rows than the
// answer needs.
func odinRankByDimension(ctx context.Context, deps *odinToolDeps, args map[string]any, dimension logstore.RankingDimension) (any, error) {
	now := odinNow()
	filters, err := odinFilterArg(args, now, deps.scope)
	if err != nil {
		return nil, err
	}
	limit := odinIntArg(args, "limit", 10, odinMaxRankingRows)
	...
	return map[string]any{"rankings": result, "scope": odinScopeNote(filters, deps.scope)}, nil
}
...
func odinQueryModelsTool() odinTool {
  ...
  "required": ["filters"]
		...
		execute: func(ctx context.Context, deps *odinToolDeps, args map[string]any) (any, error) {
			...
			out := map[string]any{"models": rankings, "scope": odinScopeNote(filters, deps.scope)}

			if odinBoolArg(args, "include_performance") {
				...
				// provider, was overflowing the tool-result budget and sending Odin
				// round the retry loop. A dozen buckets carry the shape of a latency
				// trend, which is all the answer needs.
				bucket, err := odinCoarseBucketSize(filters)
				if err != nil {
					return nil, err
				}
