...
type ModelsManager interface {
	...
	// ResolveEffectiveAccess answers what a key-authenticated request may reach. Listing routes
	// run outside the request pipeline, so they ask for the answer here instead of reading one
	// the pipeline would have left on the request. Nil means nothing was resolved — no key was
	// presented, or the deployment has no governance at all.
...
func (h *ProviderHandler) parseModelListQuery(ctx *fasthttp.RequestCtx, bifrostCtx *schemas.BifrostContext, defaultLimit int) (modelListQuery, bool) {
	...
	// reading an answer some hook left behind.
	//
	// Nothing resolved leaves the listing unfiltered: an unknown key, or a deployment with no
	// governance at all, has nothing to narrow the listing by.
	if access := h.modelsManager.ResolveEffectiveAccess(bifrostCtx); access != nil {
...
func (h *ProviderHandler) listManagementModels(query modelListQuery) ([]listedModel, int, error) {
	...
	if query.HasVKFilter {
		granted := query.Access.GrantedProvidersFor("")
		providers = slices.DeleteFunc(providers, func(p schemas.ModelProvider) bool {
			return !slices.ContainsFunc(granted, func(grantedProvider schemas.ModelProvider) bool {
				return strings.EqualFold(string(grantedProvider), string(p))
			})
		})
	}
...
func (h *ProviderHandler) listManagementModelsForProvider(
	...
	if query.HasVKFilter {
		models = slices.DeleteFunc(models, func(m string) bool {
			return !query.Access.IsModelAllowed(provider, m)
		})
	}
