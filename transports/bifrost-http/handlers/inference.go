...
// governance wired — keeps the fan-out it already had rather than being narrowed to nothing.
func (h *CompletionHandler) applyListModelsProviderFilter(bifrostCtx *schemas.BifrostContext) {
	if h.modelsManager == nil {
		return
	}
	access := h.modelsManager.ResolveEffectiveAccess(bifrostCtx)
	if access == nil || access.IsUnrestricted() {
		// Access that permits everything has nothing to narrow to. Publishing what it enumerates would
		// publish an empty list, which reads downstream as no provider being available at all.
		return
	}
	bifrostCtx.SetValue(schemas.BifrostContextKeyAvailableProviders, access.GrantedProvidersFor(""))
}
