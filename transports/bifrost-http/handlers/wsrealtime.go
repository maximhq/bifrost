...
type WSRealtimeHandler struct {
	...
	turnLogger   *realtimeTurnLogger
}

// NewWSRealtimeHandler creates a new Realtime WebSocket handler.
func NewWSRealtimeHandler(client *bifrost.Bifrost, config *lib.Config, pool *bfws.Pool, notifyLog func(*logstore.Log)) *WSRealtimeHandler {
	...
	return &WSRealtimeHandler{
		...
		turnLogger:   newRealtimeTurnLogger(config, notifyLog),
	}
}
