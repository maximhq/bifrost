...
// read regardless of what Odin asks for - so widening a query can never surface
...
//   - When there is no identity, there is no sensible default, so Odin is told
...
//     answer is allowed.
type odinScope struct {
	// HasIdentity reports whether the caller is a known user. It drives whether
	// Odin defaults or asks.
	HasIdentity bool
	UserID      string
}

// odinScopeFromContext derives the caller's scope.
//
...
func odinScopeFromContext(ctx context.Context) odinScope {
	userID, _ := ctx.Value(schemas.BifrostContextKeyUserID).(string)
	if userID == "" {
		return odinScope{}
	}
	return odinScope{HasIdentity: true, UserID: userID}
}

// applyOdinScope narrows filters to the caller's default when the question named
// no scope of its own.
//
...
func applyOdinScope(filters *logstore.SearchFilters, scope odinScope) {
	if filters == nil || !scope.HasIdentity {
		return
	}
	if odinFiltersNameAScope(filters) {
		return
	}
	filters.UserIDs = []string{scope.UserID}
}

// odinFiltersNameAScope reports whether the model asked about a particular
// slice of traffic.
//
...
func odinFiltersNameAScope(filters *logstore.SearchFilters) bool {
...
// odinScopeNote describes, in one line, what a result actually covers.
//
...
func odinScopeNote(filters *logstore.SearchFilters, scope odinScope) string {
	switch {
	...
	case odinFiltersNameAScope(filters):
...
// odinDescribeScopeTool lets Odin find out who is asking and what it could
// narrow to, so it can ask a specific question rather than a vague one.
func odinDescribeScopeTool() odinTool {
	return odinTool{
		name: "describe_scope",
		...
		execute: func(ctx context.Context, deps *odinToolDeps, _ map[string]any) (any, error) {
			const limit = 50
			out := map[string]any{
			...
			for key, dimension := range map[string]logstore.RankingDimension{
				...
				names, err := odinDimensionNames(ctx, deps, dimension)
				if err != nil {
					return nil, err
				}
				out[key] = names
...
// odinDimensionNames lists the entities seen on a dimension.
//
...
func odinDimensionNames(ctx context.Context, deps *odinToolDeps, dimension logstore.RankingDimension) ([]string, error) {
	limit := 25
