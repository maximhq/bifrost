...
func NewSessionHandler(configStore configstore.ConfigStore, logger schemas.Logger) *SessionHandler {
...
func (h *SessionHandler) isAuthEnabled(ctx *fasthttp.RequestCtx) {
	...
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to get auth config: %v", err), h.logger)
		return
	}
	if authConfig == nil {
...
func (h *SessionHandler) login(ctx *fasthttp.RequestCtx) {
	...
	if !authConfig.IsEnabled {
		SendError(ctx, fasthttp.StatusForbidden, "Authentication is not enabled", h.logger)
		return
	}

	// Verify credentials
	if payload.Username != authConfig.AdminUserName {
		SendError(ctx, fasthttp.StatusUnauthorized, "Invalid username or password", h.logger)
		return
	}
	compare, err := encrypt.CompareHash(authConfig.AdminPassword, payload.Password)
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to compare password: %v", err), h.logger)
		return
	}
	if !compare {
		SendError(ctx, fasthttp.StatusUnauthorized, "Invalid username or password", h.logger)
...
func (h *SessionHandler) logout(ctx *fasthttp.RequestCtx) {
	...
	if token != "" {
		...
		if err != nil {
			...
			h.logger.Warn(fmt.Sprintf("failed to delete session: %v", err))
		}
	}
