...
func newRealtimeTurnContext(
	...
	if baseCtx != nil {
		if externalSessionID, ok := baseCtx.Value(schemas.BifrostContextKeyRealtimeSessionID).(string); ok && strings.TrimSpace(externalSessionID) != "" {
	...
	if resolvedSessionID != "" {
		...
		ctx.SetValue(schemas.BifrostContextKeyRealtimeSessionID, resolvedSessionID)
	}
...
func buildRealtimeTurnPreRequest(provider schemas.ModelProvider, model string, inputText string, toolOutputs []bfws.RealtimeToolOutput) *schemas.BifrostRequest {
	input := make([]schemas.ResponsesMessage, 0, len(toolOutputs)+1)
	for _, toolOutput := range toolOutputs {
		summary := strings.TrimSpace(toolOutput.Summary)
		if summary == "" {
			continue
		}
	...
	if trimmed := strings.TrimSpace(inputText); trimmed != "" {
		...
		input = append(input, schemas.ResponsesMessage{
			Type:    &itemType,
			Role:    &role,
			Content: &schemas.ResponsesMessageContent{ContentStr: schemas.Ptr(trimmed)},
		})
	}
...
func shouldGracefullyDisconnectRealtime(bifrostErr *schemas.BifrostError) bool {
	...
	for _, value := range values {
		...
		if strings.Contains(lower, "budget_exceeded") || strings.Contains(lower, "budget exceeded") {
			return true
		}
		if bifrost.IsRateLimitErrorMessage(lower) {
			return true
		}
	}
...
func startRealtimeTurnHooks(
	...
	startedAt := time.Now()
	turnCtx := newRealtimeTurnContext(baseCtx, "", session.ID(), session.ProviderSessionID(), realtimeTurnSourceEI, rtProvider.RealtimeTurnStartEvent(), key)
	setRealtimeTurnStreamContext(turnCtx, startedAt, false)
	req := buildRealtimeTurnPreRequest(provider, model, session.PeekRealtimeInputText(), session.PeekRealtimeToolOutputs())
...
func finalizeRealtimeTurnHooks(
	...
	inputText := session.ConsumeRealtimeInputText()
	inputRaw := session.ConsumeRealtimeInputRaw()
	toolOutputs := session.ConsumeRealtimeToolOutputs()
	rawRequest := combineRealtimeInputRaw(inputRaw, toolOutputs)
	postResponse := buildRealtimeTurnPostResponse(rtProvider, provider, model, rawRequest, rawResponse, contentOverride)
	...
	startedAt := time.Now()
	preCtx := newRealtimeTurnContext(baseCtx, "", session.ID(), session.ProviderSessionID(), realtimeTurnSourceEI, rtProvider.RealtimeTurnStartEvent(), key)
	setRealtimeTurnStreamContext(preCtx, startedAt, false)
	preReq := buildRealtimeTurnPreRequest(provider, model, inputText, toolOutputs)
