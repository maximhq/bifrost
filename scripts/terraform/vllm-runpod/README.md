...
The pod enforces the timeout itself, so it fires even if your laptop is asleep or offline. `entrypoint.sh` becomes the container entrypoint. It records a deadline in `/workspace/.teardown_at`, then `exec`s `vllm serve` with the usual args. When the deadline passes it calls the RunPod REST API with the pod's own `RUNPOD_POD_ID` and pod-scoped `RUNPOD_API_KEY`, both injected by RunPod.

- **When the clock starts:** when the container first starts, after the image pull. Image pull time doesn't count, but the model weight download that follows does. Container restarts don't reset it.
