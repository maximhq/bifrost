...
- Added `mcp.clientConfigs[].oauthConfig` — inline OAuth bootstrap config (`clientId`, `clientSecret`, `authorizeUrl`, `tokenUrl`, `registrationUrl`, `scopes`) for `authType: oauth` / `per_user_oauth`, with all fields optional (missing URLs are discovered via RFC 8414, a missing `clientId` triggers RFC 7591 dynamic client registration). Renders into `oauth_config`, forbidden via schema unless `authType` is `oauth` or `per_user_oauth`. Replaces the previous `oauthConfigId` field, which no longer exists in `transports/config.schema.json` (it referenced an API-created OAuth config row, not something declarable in static config) and was silently invalid — any chart user relying on it would have failed schema validation.
- Added `authType: token_exchange` plus `mcp.clientConfigs[].tokenExchange` (`audience`, `clientId`, `clientSecret`, `scopes`) for delegated token-exchange auth, where each caller's identity-provider token is exchanged at runtime for a short-lived token scoped to the MCP server's audience. Renders into `token_exchange`, required when `authType` is `token_exchange` and forbidden otherwise.
