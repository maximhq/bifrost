...
- Added ClickHouse as a `storage.logsStore.type` option. Set `type: clickhouse` and a `storage.logsStore.clickhouse` block (`host` required; optional `port`, `database`, `username`, `password`, `protocol`, `secure`, `dialTimeout`, `cluster`, `managedReplication`, `tableEngine`). Managed replication emits no `ON CLUSTER` clause or explicit Keeper path; non-replicated MergeTree mode may use `ON CLUSTER` when `cluster` is set.
