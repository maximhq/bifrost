...
func TestPreLLMHook_TransformsForcedResponsesRequestToChatCompletion(t *testing.T) {
	plugin := &CompatPlugin{config: Config{}}
	...
	result, bifrostErr, err := plugin.PostLLMHook(ctx, &schemas.BifrostResponse{
		ChatResponse: &schemas.BifrostChatResponse{
			...
			ExtraFields: schemas.BifrostResponseExtraFields{
				...
				OriginalModelRequested: "test-model",
			},
		},
	}, nil)
	if err != nil {
	...
	if !result.ResponsesResponse.ExtraFields.ResponsesToChatCompletionFallback {
		t.Fatal("expected responses-to-chat fallback marker to be set")
	}
