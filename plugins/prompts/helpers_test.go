...
func (r *staticResolver) Resolve(_ *schemas.BifrostContext, _ *schemas.BifrostRequest) (string, uint, bool, error) {
	return r.promptID, r.versionID, r.versionSpecified, r.err
}
...
func newPluginWithStore(s promptStore) *Plugin {
	return &Plugin{
		...
		versionsByID: make(map[uint]*tables.TablePromptVersion),
	}
}
...
// into the store.
func newTestPlugin(resolver PromptResolver, promptMap map[string]*tables.TablePrompt, versionMap map[uint]*tables.TablePromptVersion) *Plugin {
...
func newTestPluginWithLogger(resolver PromptResolver, promptMap map[string]*tables.TablePrompt, versionMap map[uint]*tables.TablePromptVersion, log schemas.Logger) *Plugin {
	if resolver == nil {
		resolver = &staticResolver{}
	}
	if promptMap == nil {
	...
	if versionMap == nil {
		versionMap = make(map[uint]*tables.TablePromptVersion)
	}
	return &Plugin{
		store:        nil,
		logger:       log,
