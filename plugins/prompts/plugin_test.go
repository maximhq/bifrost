...
func TestLoadCache_PopulatesMaps(t *testing.T) {
	...
	assert.Len(t, p.versionsByID, 2)
	assert.NotNil(t, p.promptsByID["p1"])
	assert.NotNil(t, p.versionsByID[1])
}
...
func TestPreLLMHook_UseLatestVersion(t *testing.T) {
	...
	p := newTestPlugin(
		...
		map[uint]*tables.TablePromptVersion{1: &v},
	)
...
func TestPreLLMHook_UseSpecificVersion(t *testing.T) {
	...
	p := newTestPlugin(
		&staticResolver{promptID: "p1", versionID: 2, versionSpecified: true},
		map[string]*tables.TablePrompt{"p1": &prompt},
		map[uint]*tables.TablePromptVersion{1: &vLatest, 2: &vOld},
	)
...
func TestPreLLMHook_VersionNotFound(t *testing.T) {
	...
	p := newTestPluginWithLogger(
		&staticResolver{promptID: "p1", versionID: 99, versionSpecified: true},
		map[string]*tables.TablePrompt{"p1": &prompt},
		map[uint]*tables.TablePromptVersion{1: &v},
		log,
	)
...
func TestPreLLMHook_VersionBelongsToDifferentPrompt(t *testing.T) {
	...
	p := newTestPluginWithLogger(
		&staticResolver{promptID: "p1", versionID: 1, versionSpecified: true},
		map[string]*tables.TablePrompt{"p1": &prompt},
		map[uint]*tables.TablePromptVersion{1: &v},
		log,
	)
...
func TestHTTPTransportPreHook_SetsVersionID(t *testing.T) {
	...
	ctx := bfCtx()
	req := &schemas.HTTPRequest{
		Headers: map[string]string{PromptVersionIDHeader: "42"},
	}

	_, _ = p.HTTPTransportPreHook(ctx, req)

	got, _ := ctx.Value(PromptVersionIDKey).(string)
...
func TestHTTPTransportPreHook_CaseInsensitiveHeaders(t *testing.T) {
	p := newTestPlugin(nil, nil, nil)
	ctx := bfCtx()
	// "Bifrost-Prompt-Id" is a title-case variant of the canonical "bifrost-prompt-id".
	req := &schemas.HTTPRequest{
		Headers: map[string]string{"Bifrost-Prompt-Id": "upper-case"},
	}
...
func TestParsePromptVersionID(t *testing.T) {
	type want struct {
		id        uint
		specified bool
		wantErr   bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctx := bfCtx()
			if tt.value != nil {
				ctx.SetValue(PromptVersionIDKey, tt.value)
			}

			id, specified, err := parsePromptVersionID(ctx)
...
func TestHTTPTransportPreHook_StreamTrue_SetsStreamContext(t *testing.T) {
	...
	streamVal, _ := ctx.Value(schemas.BifrostContextKeyStreamRequest).(bool)
	assert.True(t, streamVal, "expected BifrostContextKeyStreamRequest=true when version has stream:true")
}
...
func TestHTTPTransportPreHook_StreamFalse_NoStreamContext(t *testing.T) {
	...
	assert.Nil(t, ctx.Value(schemas.BifrostContextKeyStreamRequest),
		"expected BifrostContextKeyStreamRequest not set when version has stream:false")
}
...
func TestHTTPTransportPreHook_NoStreamParam_NoStreamContext(t *testing.T) {
	...
	assert.Nil(t, ctx.Value(schemas.BifrostContextKeyStreamRequest),
		"expected BifrostContextKeyStreamRequest not set when no stream key in params")
}
...
func TestLoadCacheAndPreLLMHook_SpecificVersion(t *testing.T) {
	...
	p.resolver = &staticResolver{promptID: "p1", versionID: 2, versionSpecified: true}
