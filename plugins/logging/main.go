...
func (p *LoggerPlugin) PreHook(ctx *context.Context, req *schemas.BifrostRequest) (*schemas.BifrostRequest, *schemas.PluginShortCircuit, error) {
	...
	if streaming.IsStreamingResponse(ctx) {
...
func (p *LoggerPlugin) PostHook(ctx *context.Context, result *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) (*schemas.BifrostResponse, *schemas.BifrostError, error) {
	...
	if streaming.IsStreamingResponse(ctx) {
		...
		p.logger.Debug("[logging] streaming response processed %v", streamResponse)
		if streamResponse != nil {
			...
			go func() {
				...
				processingErr := retryOnNotFound(*ctx, func() error {
					return p.updateStreamingLogEntry(*ctx, logMsg.RequestID, logMsg.Timestamp, logMsg.SemanticCacheDebug, logMsg.StreamResponse, streamResponse.Type == streaming.StreamResponseTypeFinal)
				})
				if processingErr != nil {
					...
					p.mu.Lock()
					if p.logCallback != nil {
						if updatedEntry, getErr := p.getLogEntry(*ctx, logMsg.RequestID); getErr == nil {
		...
		go func() {
			...
			if logMsg.UpdateData != nil && p.pricingManager != nil {
				cost := p.pricingManager.CalculateCostWithCacheDebug(result, provider, model, schemas.ChatCompletionRequest)
