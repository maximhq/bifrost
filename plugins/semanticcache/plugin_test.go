...
func clearTestKeysWithStore(t *testing.T, vs store.VectorStore, prefix string) {
	t.Helper()
	...
	for {
		batch, c, err := vs.GetAll(ctx, pattern, cursor, 1000)
		if err != nil {
	...
	if len(keys) > 0 {
		if err := vs.Delete(ctx, keys); err != nil {
