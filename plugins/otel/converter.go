...
func createResourceSpan(traceID, spanID string, timestamp time.Time, req *schemas.BifrostRequest) *ResourceSpan {
	...
	if req != nil {
		...
		if req.Input.TranscriptionInput != nil {
			...
			params = append(params, kvStr("gen_ai.input.transcription", fileData))
...
func completeResourceSpan(span *ResourceSpan, timestamp time.Time, resp *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) *ResourceSpan {
