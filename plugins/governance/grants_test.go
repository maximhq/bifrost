...
func TestResolveEffectiveAccessDoesNotRecordOnContext(t *testing.T) {
	vk := buildVKForMCPStamping([]string{"read_file"})
	store := &grantSetStore{}
	plugin := newAccessTestPlugin(t, vk, store)
	ctx := newPreRequestCtx(nil, nil)

	access := plugin.ResolveEffectiveAccess(ctx)

	require.NotNil(t, access)
	assert.Equal(t, 1, store.calls)
	assert.Nil(t, grants.EffectiveAccessFromContext(ctx), "inspection must not mutate request-scoped access")
	assert.Nil(t, plugin.ResolveEffectiveAccess(nil))
}
