...
// asked of the request's access rather than of the key directly.
func accessFor(vk *configstoreTables.TableVirtualKey, openClients map[string]string) *grants.EffectiveAccess {
	return grants.NewEffectiveAccess(vkGrant(vk, openClients), nil, "", nil, nil)
}
...
func TestIsMCPToolAllowedByVK_UsesInMemoryStore(t *testing.T) {
	...
	grant := (&LocalGovernanceStore{inMemoryStore: store}).grantForVirtualKey(emptyCtx(), buildVKNoMCPConfigs())
	access := grants.NewEffectiveAccess(grant, nil, "", nil, nil)

	assert.True(t, access.IsMCPToolAllowed("youtube-search"),
		"the store resolves AllowOnAllVirtualKeys clients when it builds the key's grant")
}
...
func TestToolChecks_StoreWithoutOpenClients_Blocked(t *testing.T) {
	grant := (&LocalGovernanceStore{}).grantForVirtualKey(emptyCtx(), buildVKNoMCPConfigs())
	access := grants.NewEffectiveAccess(grant, nil, "", nil, nil)

	assert.False(t, access.IsMCPToolAllowed("youtube-search"),
		"no open clients means no grant, so no tool is allowed")
}
