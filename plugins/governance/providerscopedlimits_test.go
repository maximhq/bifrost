...
func TestProviderScopedLimitsFundOnlyTheirOwnProvider(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			resolved := resolveLimits(ctx, store, tc.provider, tc.model)
			require.NotNil(t, resolved, "the key resolves, so the request has an access to settle limits onto")

			assert.ElementsMatch(t, tc.budgets, limitIDsOf(resolved.ResolvedBudgets()),
				"the holder's own budgets plus this provider's, and no other provider's")
			assert.ElementsMatch(t, tc.rateLimits, limitIDsOf(resolved.ResolvedRateLimits()),
