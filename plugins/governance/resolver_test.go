...
func TestBudgetResolver_EvaluateRequest_GracePeriodValue(t *testing.T) {
	...
	result := resolver.Evaluate(ctx, "sk-bf-rotated-out", schemas.OpenAI, "gpt-4", schemas.ChatCompletionRequest, false, false)
	assertDecision(t, DecisionAllow, result)
	...
	result = resolver.Evaluate(ctx, "sk-bf-current", schemas.OpenAI, "gpt-4", schemas.ChatCompletionRequest, false, false)
	assertDecision(t, DecisionAllow, result)
}
