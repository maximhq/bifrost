...
// promote every request made with it to the unrestricted access a request carrying no credential at all
// has, which is the widest access the system grants.
func TestPreLLMHookRefusesACredentialThatResolvesToNothing(t *testing.T) {
	plugin := refusalTestPlugin(t, nil)
	ctx := schemas.NewBifrostContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.BifrostContextKeyVirtualKey, refusalTestUnknownVKValue)
...
// it to, is what every deployment that has not turned on governance looks like — it is allowed, and it is
// allowed by carrying the unrestricted access rather than by carrying none, so nothing downstream has to
// read an absent access as permission to proceed.
//
...
func TestPreLLMHookAllowsARequestThatPresentsNothing(t *testing.T) {
	...
	require.NotNil(t, access, "the request carries an access, so no consumer has to special-case its absence")
...
func TestTheAccessChecksAllowACredentialThatResolvedToNothing(t *testing.T) {
	...
	require.Nil(t, grants.EffectiveAccessFromContext(ctx), "the store resolved no access for this credential")
