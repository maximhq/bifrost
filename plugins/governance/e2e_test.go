...
func TestBudgetStateAfterReset(t *testing.T) {
	...
	waitTime := time.Until(startTime.Add(16 * time.Second))
	if waitTime > 0 {
		t.Logf("Waiting %.1f seconds for budget to reset...", waitTime.Seconds())
		time.Sleep(waitTime)
	}
	...
	// After lazy reset, the usage should be much lower than before (only the cost of the new request)
	// The reset happens BEFORE adding the new cost, so usage = 0 + new_request_cost
	// This should be much smaller than usageAfterRequest (which was usage before reset)
	if usageAfterReset >= usageAfterRequest {
		t.Fatalf("Budget not reset: usage after reset ($%.6f) should be less than before ($%.6f)",
			usageAfterReset, usageAfterRequest)
	}

	// The usage after reset should be approximately the cost of one request (~$0.003)
	// This confirms the budget was reset to 0 and then the new cost was added
	if usageAfterReset > 0.01 { // Allow some margin for actual LLM response
		t.Fatalf("Budget usage after reset too high: $%.6f (expected ~$0.003 for one request)", usageAfterReset)
	}

	// Verify LastReset was updated
	if lastReset3 == lastReset1 {
		t.Logf("Warning: LastReset timestamp appears unchanged (may be same second)")
	} else {
		t.Logf("LastReset timestamp was updated ✓")
	}
...
func TestTeamDeletionCascade(t *testing.T) {
	...
	// Expected: VK may or may not work depending on implementation
	// But it should no longer have access to team budget
	resp2 := MakeRequest(t, APIRequest{
		Method: "POST",
		Path:   "/v1/chat/completions",
		Body: ChatCompletionRequest{
	...
	t.Logf("VK request after team deletion: status=%d", resp2.StatusCode)
	t.Logf("Team deletion cascade verified ✓")
}
...
func TestWeightedProviderLoadBalancing(t *testing.T) {
	...
	for i := 0; i < numRequests; i++ {
		...
		if model, ok := resp.Body["model"].(string); ok {
			if strings.Contains(model, "openai") || strings.Contains(model, "gpt-4o") {
				openaiCount++
				t.Logf("Request %d routed to OpenAI (model: %s)", i+1, model)
			} else if strings.Contains(model, "azure") {
				azureCount++
