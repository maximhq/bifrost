...
// ============================================================================
// Store Tests - Provider Rate grants.Limit
// ============================================================================
...
// ============================================================================
// Store Tests - Model Rate grants.Limit
// ============================================================================
...
// ============================================================================
// Store Tests - Update Provider Rate grants.Limit Usage
// ============================================================================
...
// ============================================================================
// Store Tests - Update Model Rate grants.Limit Usage
// ============================================================================
...
func TestPostHook_TracksVirtualKeyUsageWhenUserIDPresent(t *testing.T) {
	...
	// The funnel settles what this request answers to before anything is billed against it, so the test
	// runs it — an update carrying no limits is billed nothing, by design.
...
func TestPostMCPHook_TracksVirtualKeyUsageWhenUserIDPresent(t *testing.T) {
	...
	// Evaluating the tool call settles the limits it answers to, and billing reads them from there. Tool
	// execution names no provider and no model, so what it answers to is whatever funds the holder.
	require.NotNil(t, resolveLimits(ctx, store, "", ""))

	_, _, hookErr := plugin.PostMCPHook(ctx, resp, nil)
...
// ============================================================================
// Store Tests - Per-VK-Scoped Model Budget / Rate grants.Limit
// ============================================================================
...
func TestStore_CheckVirtualKeyScopedModelBudget_NilVK(t *testing.T) {
	...
	decision, err := checkScopedBudgets(store, context.Background(), grants.GrantType(""), "", schemas.OpenAI, "gpt-4", nil)
	assert.NoError(t, err)
	assert.Equal(t, DecisionAllow, decision)
}
...
func TestStore_CheckVirtualKeyScopedModelBudget_NoConfig(t *testing.T) {
	...
	decision, err := checkScopedBudgets(store, context.Background(), grants.GrantTypeVirtualKey, vk.ID, schemas.OpenAI, "gpt-4", nil)
	assert.NoError(t, err)
	assert.Equal(t, DecisionAllow, decision)
}
...
func TestStore_CheckVirtualKeyScopedModelBudget_WithinLimit(t *testing.T) {
	...
	_, err = checkScopedBudgets(store, context.Background(), grants.GrantTypeVirtualKey, vk.ID, schemas.OpenAI, "gpt-4", nil)
...
func TestStore_CheckVirtualKeyScopedModelBudget_OnlyAppliesToMatchingVK(t *testing.T) {
	...
	decision, err := checkScopedBudgets(store, context.Background(), grants.GrantTypeVirtualKey, otherVK.ID, schemas.OpenAI, "gpt-4", nil)
	assert.NoError(t, err)
	assert.Equal(t, DecisionAllow, decision)
}
...
func TestStore_ScopedAndGlobalModelBudgetsBothApply(t *testing.T) {
	...
	budgets, _ := store.ProviderAndModelLimits(context.Background(), &grants.Grant{
		Type: grants.GrantTypeVirtualKey, ID: vk.ID, IsActive: true,
	}, schemas.OpenAI, "gpt-4")
	require.Len(t, budgets, 1, "the deployment's model budget, and nothing of the holder's")
	assert.Equal(t, grants.LimitHolderModelConfig, budgets[0].HolderKind)
...
func TestStore_CheckVirtualKeyScopedModelRateLimit_TokenLimitExceeded(t *testing.T) {
	...
	decision, err := checkScopedRateLimits(store, context.Background(), grants.GrantTypeVirtualKey, vk.ID, schemas.OpenAI, "gpt-4", nil, nil)
...
func TestStore_VirtualKeyScopedModel_RecordThenCheck_TokenLimitTrips(t *testing.T) {
	...
	decision, err := checkScopedRateLimits(store, context.Background(), grants.GrantTypeVirtualKey, vk.ID, schemas.Anthropic, "claude-opus-4-7", nil, nil)
	require.NoError(t, err)
	require.Equal(t, DecisionAllow, decision)
...
func TestStore_VirtualKeyScopedModel_RecordThenCheck_BudgetTrips(t *testing.T) {
	...
	decision, err := checkScopedBudgets(store, context.Background(), grants.GrantTypeVirtualKey, vk.ID, schemas.Anthropic, "claude-opus-4-7", nil)
	require.NoError(t, err)
	require.Equal(t, DecisionAllow, decision)
	...
	_, err = checkScopedBudgets(store, context.Background(), grants.GrantTypeVirtualKey, vk.ID, schemas.Anthropic, "claude-opus-4-7", nil)
