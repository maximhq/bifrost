...
// ResolveEffectiveAccess resolves the access for a request without recording it on the
// request. Callers that only need to inspect the current grants should not mutate the
// request-scoped answer consumed by the inference pipeline.
func (p *GovernancePlugin) ResolveEffectiveAccess(ctx *schemas.BifrostContext) *grants.EffectiveAccess {
	if ctx == nil {
		return nil
	}
	...
	return grants.NewEffectiveAccess(base, scoping, mode, p.modelCatalog, p.inMemoryStore)
}
