...
func Init(ctx context.Context, config *Config, logger schemas.Logger, store configstore.ConfigStore) (*GovernancePlugin, error) {
	if store == nil {
		return nil, fmt.Errorf("governance plugin requires config store to be enabled")
	}
	governanceStore, err := NewGovernanceStore(logger, store)
	if err != nil {
