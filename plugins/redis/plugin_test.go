...
func TestRedisPlugin(t *testing.T) {
	...
	config := RedisPluginConfig{
		TTL:    5 * time.Minute,
		Prefix: "test:bifrost:cache:",
	}

	logger := bifrost.NewDefaultLogger(schemas.LogLevelDebug)

	// Initialize the Redis plugin with the client
	plugin, err := NewRedisPlugin(redisClient, config, logger)
	if err != nil {
	...
	testRequest := &schemas.BifrostRequest{
		...
		Input: schemas.RequestInput{
			ChatCompletionInput: &[]schemas.BifrostMessage{
				{
					Role:    "user",
					Content: bifrost.Ptr("What is Bifrost? Answer in one short sentence."),
				},
			},
		},
	...
	if len(response1.Choices) > 0 && response1.Choices[0].Message.Content != nil {
		log.Printf("Response: %s", *response1.Choices[0].Message.Content)
	}
	...
	if len(response2.Choices) > 0 && response2.Choices[0].Message.Content != nil {
		log.Printf("Response: %s", *response2.Choices[0].Message.Content)
	}
	...
	if len(response1.Choices) > 0 && len(response2.Choices) > 0 {
		if response1.Choices[0].Message.Content != nil && response2.Choices[0].Message.Content != nil {
