...
<Steps>
  ...
  <Step title="Enable Allow Direct API Keys">
    Go to **Config > Security** and enable **Allow Direct API Keys**. This is required so Bifrost forwards the OAuth Bearer token from Codex's request headers to the upstream provider.
  </Step>
  <Step title="Configure Codex">
    ...
    model = "openai/gpt-5.4"
    ```
    No API key or `env_key` is needed — Codex sends its OAuth token automatically.
  </Step>
...
<Note>
The ChatGPT OAuth passthrough requires no API keys configured on the OpenAI provider in Bifrost. The OAuth token from Codex's `Authorization` header is forwarded directly to the ChatGPT backend.
</Note>
