...
<Note>
  ...
  `base_url` also accepts an `env.VARIABLE_NAME` reference (e.g.
  `"base_url": "env.VLLM_BASE_URL"`), resolved from the environment at
  configuration load time — useful for keeping `config.json` committed while
  the endpoint varies per environment.
</Note>
