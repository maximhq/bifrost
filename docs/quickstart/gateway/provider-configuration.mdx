...
Provider APIs commonly return a request identifier in a response header. Bifrost can capture this identifier and store it with request logs, making it easier to correlate a failed or slow request with the provider's support and debugging tools.

Capture is **disabled by default**. Enable it per provider with `provider_request_id`:
...
Bifrost uses built-in response-header defaults for OpenAI (`x-request-id`), Anthropic (`request-id`), and Azure (`apim-request-id`). A custom provider inherits the default for its `base_provider_type`:
...
For other providers or gateways, set an explicit response header:
...
Header names are normalized to lowercase and must be valid HTTP header tokens. Authentication and API-key headers, such as `authorization` and `x-api-key`, are rejected. Captured values are limited to 512 characters; oversized values are discarded without logging their contents.

The logs UI shows the ID from the final provider attempt and the request ID trail across retries and fallbacks. You can also filter logs by an exact provider request ID.
