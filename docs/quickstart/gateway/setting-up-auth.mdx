...
<Note>
  **Before the first admin account exists:** Bifrost's management API (`/api/*`) accepts a request only when it
  carries the operator's **setup token** in the `X-Bifrost-Setup-Token` header. The operator configures this value
  ahead of time via `setup_token` in `config.json` (a literal value or an `env.VAR_NAME` / `vault.path` reference, same
  as `admin_username`/`admin_password`) or the `BIFROST_SETUP_TOKEN` environment variable. A request without it gets
  `401`; if no setup token is configured at all, the management API answers `403` to everything until you set one.
  Health, version and login routes stay open, inference is unaffected, and once an admin account exists the header is
  no longer needed. The setup token is never persisted or logged, and every node in a multi-node deployment reads the
  same configured value.

  The dashboard does not send the header, so create the first admin account before you use it. The simplest way is to
  provision it in `config.json`:
  ...
  Automation that creates it through `PUT /api/config` sends the setup token in the `X-Bifrost-Setup-Token` header
  (or as `auth_config.setup_token` in the body) on that request and on any other management call it makes first.
</Note>
