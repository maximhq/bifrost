...
| migrate-only | false  | `-migrate-only`   | `-e BIFROST_MIGRATE_ONLY=1`     | Run database migrations and exit (for one-shot migration jobs) |
| no-migrate | false    | `-no-migrate`     | `-e BIFROST_NO_MIGRATE=1`       | Skip database migrations on startup; fails fast if any are pending |
