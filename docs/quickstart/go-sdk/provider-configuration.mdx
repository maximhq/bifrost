...
Provider APIs commonly return a request identifier in a response header. Bifrost can capture this identifier and attach it to request logs for provider-side debugging. Capture is **disabled by default** and must be enabled per provider.
...
Bifrost uses built-in response-header defaults for OpenAI (`x-request-id`), Anthropic (`request-id`), and Azure (`apim-request-id`). Custom providers inherit the default for their `CustomProviderConfig.BaseProviderType`. For another provider or gateway, configure `HeaderName` explicitly:
...
When the logging plugin is enabled, the final provider request ID and the complete retry/fallback trail are available on the log entry. Gateway users can also filter logs by an exact provider request ID.
