...
When the logging plugin is enabled, the final provider request ID and the bounded retry/fallback trail are available on the log entry. Gateway users can also filter logs by an exact provider request ID.
