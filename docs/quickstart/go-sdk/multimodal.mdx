...
Bifrost supports multimodal embeddings — generating vector representations that capture text, images, audio, video, and documents together. The `Input` field on `BifrostEmbeddingRequest` is a `[]EmbeddingInputItem`; each item's `Content` field is an `EmbeddingContent`, a slice of typed parts.
...
Cohere `embed-v4.0` accepts a text part and an image part in the same content entry. The image must be a base64 data URI — Cohere rejects direct URLs.
...
A `URL` on a non-image part is sent as Gemini's `fileData.fileUri`, which only accepts a Files API URI — upload the media first, or pass the bytes inline via `Data` instead. Image parts additionally accept a base64 data URI in `URL`.

```go
audioURL := "https://generativelanguage.googleapis.com/v1beta/files/<file-id>" // from the Files API
...
```go
pdfURL   := "https://generativelanguage.googleapis.com/v1beta/files/<file-id>" // from the Files API
...
`Input` is a `[]EmbeddingInputItem`. Leave `Params` nil on an item and it inherits the
request-level defaults; set it and that item alone uses the override — a query embedded with
`RETRIEVAL_QUERY` alongside documents embedded with `RETRIEVAL_DOCUMENT`, say.

```go
query := "how do I reset my password?"
doc   := "Password reset guide"

resp, err := client.EmbeddingRequest(schemas.NewBifrostContext(ctx, schemas.NoDeadline), &schemas.BifrostEmbeddingRequest{
    Provider: schemas.Gemini,
    Model:    "gemini-embedding-001",
    // Default for every item that does not override it.
    Params: &schemas.EmbeddingParameters{Dimensions: schemas.Ptr(768)},
    Input: []schemas.EmbeddingInputItem{
        {
            Content: schemas.EmbeddingContent{
                {Type: schemas.EmbeddingContentPartTypeText, Text: &query},
            },
            Params: &schemas.EmbeddingParameters{
                TaskType:   schemas.Ptr("RETRIEVAL_QUERY"),
                Dimensions: schemas.Ptr(768),
            },
        },
        {
            Content: schemas.EmbeddingContent{
                {Type: schemas.EmbeddingContentPartTypeText, Text: &doc},
            },
            Params: &schemas.EmbeddingParameters{
...
// resp.Data has one entry per input item, in order.
```

An item that sets `Params` **replaces** the request-level params wholesale — the two are not
merged field by field, so repeat any request-level value you still want (as `Dimensions` is
repeated above).
