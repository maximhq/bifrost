...
<Warning>
A direct key bypasses governance, weighted selection, and rotation/fallback — the key is used exactly as supplied. Prefer [API Key Selection](#api-key-selection) when you want to reference a Bifrost-managed key instead of passing the secret.
</Warning>
