...
vLLM is a self-hosted OpenAI-compatible server. Bifrost forwards Chat Completions `reasoning_effort` and Responses `reasoning.effort` without applying OpenAI's model-name taxonomy, so values such as `xhigh` reach your checkpoint as-is. Whether a given value is accepted depends on the model loaded in vLLM (for example Qwen3.8 accepts `xhigh`, `medium`, and `low`, but not `high`).
...
<Accordion title="Reasoning effort is not remapped by model name">
**Severity**: Low
**Behavior**: Unlike hosted OpenAI, Bifrost does not rewrite `xhigh` to `high` for vLLM based on the checkpoint name.
**Impact**: Send the effort values your loaded model actually supports. A value OpenAI would coerce (for example `xhigh` on a non-GPT-5.2 name) is forwarded unchanged and may 400 if that checkpoint rejects it.
</Accordion>
