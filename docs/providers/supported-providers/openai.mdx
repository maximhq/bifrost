...
| `params.n` | int | ❌ | Number of images to generate (1-10) |
| `params.size` | string | ❌ | Image size: `"256x256"`, `"512x512"`, `"1024x1024"`, `"1792x1024"`, `"1024x1792"`, `"1536x1024"`, `"1024x1536"`, `"auto"` |
| `params.quality` | string | ❌ | Image quality: `"auto"`, `"high"`, `"medium"`, `"low"`, `"hd"`, `"standard"` |
| `params.style` | string | ❌ | Image style: `"natural"`, `"vivid"` |
| `params.response_format` | string | ❌ | Response format: `"url"` or `"b64_json"` |
| `params.background` | string | ❌ | Background: `"transparent"`, `"opaque"`, `"auto"` |
| `params.output_format` | string | ❌ | Output format: `"png"`, `"webp"`, `"jpeg"` |
| `params.output_compression` | int | ❌ | Compression level (0-100%) |
| `params.partial_images` | int | ❌ | Number of partial images (0-3) |
| `params.moderation` | string | ❌ | Moderation level: `"low"`, `"auto"` |
| `params.user` | string | ❌ | User identifier |

---

**Request Conversion (Bifrost → OpenAI)**
...
- **Parameters**: All fields from `bifrostReq.Params` (`ImageGenerationParameters`) are embedded directly into the OpenAI request struct via struct embedding. No field mapping or transformation is performed.
- **Streaming**: When streaming is requested, `stream: true` is set in the request body.

**Response Conversion (OpenAI → Bifrost)**
