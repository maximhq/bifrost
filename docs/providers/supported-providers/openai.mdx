...
When the upstream provider returns richer model objects, Bifrost preserves those top-level fields in the native `/v1/models` response, including fields such as `name`, `description`, context limits, `architecture`, `supported_parameters`, and provider-specific `pricing`. Clients can use this metadata for model pickers, context limits, modality or tool support, and pricing displays without a separate `metadata` wrapper.
