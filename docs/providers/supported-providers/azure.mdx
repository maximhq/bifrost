...
| `input.prompt` | Direct pass-through | Prompt text for image generation |
...
- **Model & Prompt**: `bifrostReq.Model` → `req.Model` (mapped to deployment), `bifrostReq.Input.Prompt` → `req.Prompt`
...
Image generation streaming is supported and uses OpenAI's streaming format with Server-Sent Events (SSE). The endpoint format includes the deployment ID:

`/openai/deployments/{deployment}/images/generations?api-version={version}`
