...
The Hugging Face provider supports routing to 20+ inference backends. Below is the current list of supported providers and their capabilities (as of August 2026):
...
<Warning>
Usage metadata depends on the selected inference provider and model. In particular, Novita models do not consistently return token usage, and other Hugging Face inference backends may behave similarly. When an upstream backend omits usage, Bifrost cannot determine how many tokens the request consumed or calculate and correlate its cost. Applications that rely on token or cost tracking should handle missing usage and validate the specific provider and model they use.
</Warning>
