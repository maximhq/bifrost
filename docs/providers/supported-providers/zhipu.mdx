...
- **Reasoning support** - GLM models think by default; `reasoning_effort` is honored on GLM-5.2 and stripped elsewhere. Preserved-thinking is controlled via the `thinking` extra param (`{"type": "enabled", "clear_thinking": false}`)
...
- **Default (OpenAI-compatible):** `reasoning_effort` is forwarded only for GLM-5.2 (including the Coding Plan 1M-context alias `glm-5.2[1m]`) and stripped for every other model — the vendor rejects the field there. GLM-4.x thinking is always on; its behavior is tuned via the `thinking` extra param.
...
<Accordion title="Reasoning Effort Stripped Except GLM-5.2">
**Severity**: Medium
**Behavior**: On the OpenAI-compatible path, `reasoning_effort` is only forwarded for GLM-5.2-series models; it is stripped for all others (the vendor rejects the field there)
**Impact**: A `reasoning_effort` set on a GLM-4.x model is silently dropped; GLM-4.x models always think
