...
DeepSeek's current V4 models use a peak/off-peak schedule in **Asia/Shanghai**:

- Monday–Friday, 09:00–12:00 and 14:00–18:00: peak pricing (`2×`)
- All other times: off-peak/base pricing (`1×`)
- Saturday and Sunday are off-peak all day; only weekdays have peak windows

Bifrost stores this as a provider-generic `pricing_schedule`, so another provider can use the same timezone/calendar/rule format without a provider-specific billing path.
