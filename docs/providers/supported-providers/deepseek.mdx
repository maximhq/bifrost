...
DeepSeek has announced a forthcoming peak/off-peak pricing schedule for its V4 models in **Asia/Shanghai**. Its published pricing page currently describes:

- Daily 09:00–12:00 and 14:00–18:00 windows at peak pricing (`2×`)
- All other times at off-peak/base pricing (`1×`)

Before enabling DeepSeek peak pricing, check DeepSeek's current pricing page and billing status. When it becomes effective, Bifrost stores it as a provider-generic `pricing_schedule`, so another provider can use the same timezone/calendar/rule format without a provider-specific billing path.

Pricing is classified using the **provider attempt start time**. A request that starts during a peak window remains peak-priced even if it completes after the window ends. The schedule multiplier is applied after service-tier and context-tier prices, so it composes with existing tiered pricing rather than replacing it.

The UI exposes this schedule under **Models** → **Model Catalog** → a DeepSeek V4 model → **Edit**. The API also returns it as `pricing_schedule` from `GET /api/models/details` and accepts it through `PUT /api/models/catalog`:
...
Use `"clear_pricing_schedule": true` to remove a schedule. Workday/holiday and makeup-workday calendars are not supported yet; `iso_weekday` uses the ISO weekday only.
