...
| Responses API | ✅ | ✅ | Bedrock-backed models (Anthropic, Amazon) |
...
<Note>
**Backend-specific features**: Tool calling and vision capabilities are supported across all backends — GPT, Claude, Gemini, and Amazon Nova models all support tool/function calling and vision (multi-modal image inputs). The Responses API supports native tool calling for Bedrock-backed models (Anthropic and Amazon) via the Converse API.
</Note>
...
- `auth_url` - OAuth2 token endpoint URL from service key (required)
...
```json
{
  ...
  "url": "→ auth_url (append /oauth/token)",
  "serviceurls": {
    "AI_API_URL": "→ base_url"
  }
}
...
Set the following in the Chat Completions request form:

- **Model**: `sapaicore/<model-name>` (e.g., `sapaicore/gpt-4o`, `sapaicore/anthropic--claude-3.5-sonnet`, `sapaicore/gemini-1.5-pro`)
- **Messages**: Add a user message with your prompt
- **Max Completion Tokens**: Set as needed per model
...
The Responses API is supported for Bedrock-backed models (Anthropic/Amazon) and uses the Bedrock Converse API under the hood. This enables native tool/function calling and streaming.

<Tabs>
<Tab title="Web UI">

Set the following in the Responses API request form:

- **Model**: `sapaicore/anthropic--claude-3.5-sonnet` (or any Bedrock-backed model)
- **Input**: Add a user message with your prompt
- **Tools** (optional): Add function definitions for tool calling
- **Stream** (optional): Enable for streaming responses
...
curl -N -X POST http://localhost:8080/v1/responses \
  ...
  -d '{
    ...
    "input": [
      {"role": "user", "content": [
        {"type": "input_text", "text": "Write a short poem about the sea"}
      ]}
    ],
    "stream": true
  }'
...
curl -X POST http://localhost:8080/v1/responses \
  ...
  -d '{
    ...
    "input": [
      {"role": "user", "content": [
        {"type": "input_text", "text": "What'\''s the weather in Paris and Berlin?"}
      ]}
    ],
    "tools": [
      {
...
The Responses API uses the same provider configuration as Chat Completions. See the [Chat Completions config.json tab](#example-requests) above.

</Tab>
</Tabs>

<Note>
Currently, the SAP AI Core provider supports the Responses API only for Bedrock-backed models (Anthropic/Amazon). OpenAI/Vertex-backed models should use Chat Completions.
</Note>
...
Select `sapaicore` as the provider and set the model to an embedding model (e.g., `text-embedding-ada-002`).
...
Uses the same provider configuration as Chat Completions. See the [Chat Completions config.json tab](#example-requests) above.
...
Navigate to the Models page and filter by the `sapaicore` provider.
