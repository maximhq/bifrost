...
Runware additionally exposes an **OpenAI-compatible LLM API** at the same base URL (`/v1/chat/completions`, `/v1/models`). Bifrost routes chat completions (unary and streaming) and model listing through its shared OpenAI-compatible handlers, so any Runware-hosted text model (e.g. DeepSeek, Qwen, GLM, Kimi, MiniMax) works with the standard `chat/completions` schema, including tool calling and structured outputs.
