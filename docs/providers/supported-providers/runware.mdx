...
Bifrost routes chat completions (unary and streaming) and the Responses API (unary and streaming, translated to chat completions) through its shared OpenAI-compatible handlers, and lists models through Runware's paged `modelSearch` catalog task.
...
|-----------|-----------|----------|-----------|
| Chat Completion | ✅ | `/v1/chat/completions` | `chat/completions` (OpenAI-compatible) |
| Chat Completion (stream) | ✅ | `/v1/chat/completions` | `chat/completions` (SSE) |
| Responses API | ✅ | `/v1/responses` | muxed through chat completions |
| Responses API (stream) | ✅ | `/v1/responses` | muxed through chat completions (SSE) |
