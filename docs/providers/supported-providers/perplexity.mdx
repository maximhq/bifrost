...
<Note>
An explicit `model` always wins over a `preset`'s own default model (live-verified against api.perplexity.ai) — the example above pins `openai/gpt-5.6-sol` regardless of `preset`. To let the preset choose the model instead, set `"model": "perplexity/preset"` — a Bifrost-only sentinel, not a real Perplexity model — together with `preset`: Bifrost omits `model` from the wire request in that case so Perplexity's preset default applies. Bare `sonar` and every other real model name are unaffected and always win over `preset`, same as any other explicit model.
</Note>
