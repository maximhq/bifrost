...
<Note>
An explicit `model` always wins over a `preset`'s own default model (live-verified against api.perplexity.ai) — the example above pins `openai/gpt-5.6-sol` regardless of `preset`. To let the preset choose the model instead, set `"model": "sonar"` (Bifrost's generic Agent API entry point): Bifrost omits `model` from the wire request in that case so Perplexity's preset default applies.
</Note>
