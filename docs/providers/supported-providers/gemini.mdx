...
<Note>
Supports text and multimodal embeddings (text, image, audio, video, and PDF via `file` parts). Multiple input items are sent as a batch — each item maps to a separate entry in `requests[]`.
</Note>

**Request Parameters**:
- `input` → `ToGeminiEmbeddingRequest` maps each `EmbeddingInputItem.Content` to a separate `requests[]` entry (batch). Each content's parts map to `content.parts[]`.
