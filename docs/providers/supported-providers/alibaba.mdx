...
- `output_config.effort` is forwarded for **qwen3.8-max**, **hosted glm-5.2+**, and **deepseek-v4-pro/flash**. The mount does not map out-of-enum values server-side — it validates the value against its internal `reasoning_effort` enum (`none`/`minimal`/`low`/`medium`/`high`/`xhigh`) and rejects anything else — so the gateway clamps `max`→`xhigh` itself and forwards every other value verbatim. The mount also rejects `output_config.effort` and `thinking.budget_tokens` set together (`'reasoning_effort' and 'thinking_budget' cannot be set simultaneously`) and engages thinking on its own from the effort value, so when an effort is set Bifrost sends the effort alone and omits the `thinking` field. (Verified live 2026-08-23.) For all other models the field is stripped.
