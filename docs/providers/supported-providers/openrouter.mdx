...
OpenRouter serves TypeSafe's jev family of System One judgment models on `/api/alpha/decisions`. Bifrost routes decision requests for these models there with your OpenRouter key, using the same request and answer conversion as the [TypeSafe provider](/providers/supported-providers/typesafe#decisions-api).

| Parameter | Mapping |
|-----------|---------|
| `model` | System One model ID (e.g., `typesafe/jev-1.13`, `~typesafe/jev-latest`) |
| `state` | Direct pass-through (string, object, or array) |
| `questions` | `kind` maps to native `type`; criteria pass through unchanged |

| Bifrost field | Built from native | Notes |
|---------------|-------------------|-------|
| `id` | `id` | OpenRouter generation ID |
| `model` | `model` | Resolved upstream model (e.g., `typesafe/jev-1.13-20260917`) |
| `usage.prompt_tokens` / `usage.completion_tokens` | `usage.input_tokens` / `usage.output_tokens` | |
| `usage.cost.total_cost` | `usage.cost` | Billed amount in USD, used as-is for logging and budgets |

Any other model on `/v1/decisions` returns `unsupported_operation` from the provider, so Bifrost emulates the decision through the model's tool calling. OpenRouter's `/v1/models` does not list the jev models, so Bifrost adds them to the model list.
