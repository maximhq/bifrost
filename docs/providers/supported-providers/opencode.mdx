...
OpenCode's subscription gateway (Go) expects an `x-opencode-session` request header carrying one stable ID per conversation. OpenCode uses it for session-level attribution on shared subscription backends, which is how resold subscriptions get caught (unrelated sessions piling up on one key). Header-less Go requests may be rejected outright. Zen, the pay-as-you-go API, does not enforce it. Bifrost sends the header to both gateways anyway: Zen ignores it, and uniform emission keeps zen-go fallbacks identical.

Bifrost resolves the value once per request and forwards it upstream on every inference call (chat, responses, streaming — but not `ListModels`):

1. A client-sent `x-opencode-session` header is forwarded verbatim (preferred — e.g. Hermes past 0.21.0 sends this automatically).
2. Otherwise the request's existing session identity is reused (`x-bf-session-id`, then harness headers such as `x-session-affinity` / `x-session-id`).
3. Otherwise Bifrost synthesizes a one-off UUID so the request stays compliant (no cache benefit — multi-turn warmth requires the client to send a stable session).

```bash
# One stable ID per conversation
curl http://localhost:8080/v1/chat/completions \
  -H "Authorization: Bearer $BIFROST_VK" \
  -H "x-opencode-session: my-conversation-123" \
  -H "Content-Type: application/json" \
  -d '{"model": "opencode-go/glm-5", "messages": [{"role": "user", "content": "hi"}]}'
```

Two details worth knowing:

- **Per-tenant namespacing** — the upstream value is scoped to your virtual key (`sha256(vk, session)` prefix), so identical session strings from different tenants never share cache warmth or hotspot a backend. Without a virtual key the raw value is used.
- **Opencode-only** — the header is never forwarded to other providers, even if you reuse a session ID across them. Likewise, do not set a static `x-opencode-session` in `network_config.extra_headers`: a single fixed value would pin every conversation to one session and defeat affinity.
