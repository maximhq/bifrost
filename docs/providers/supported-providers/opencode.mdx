...
OpenCode Go asks each client for a stable `x-opencode-session` per conversation, which OpenCode uses to optimize routing and prompt caching; Go traffic is additionally monitored for abuse (see [Where can I use it?](https://opencode.ai/docs/go/#where-can-i-use-it)). Header-less Go requests may be rejected outright (enforced from 09/06). Zen, the pay-as-you-go API, does not enforce it. Bifrost sends the header to both gateways anyway: Zen ignores it, and uniform emission keeps zen-go fallbacks identical.
