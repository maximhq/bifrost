...
5. Set **Region**: e.g., `us-central1`
6. Set **Auth Credentials**: Paste your service account JSON or reference an env var (e.g., `env.VERTEX_CREDENTIALS`)
7. Configure **Aliases**: Map model names to fine-tuned model IDs (if using fine-tuned models)
8. Save
...
- Use `deployments` inside `vertex_key_config` instead of the top-level `aliases` field for fine-tuned model mappings.
</Note>
...
7. Set **Project Number** (Optional): Required only when using fine-tuned models
