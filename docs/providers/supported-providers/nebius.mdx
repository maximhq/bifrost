...
| `params.size` | string | ❌ | Image size in WxH format (e.g., `"1024x1024"`). Converted to separate `width` and `height` integers |
| `params.output_format` | string | ❌ | Output format: `"png"`, `"jpeg"`, `"webp"`. Note: `"jpeg"` is converted to `"jpg"` |
| `params.response_format` | string | ❌ | Response format: `"url"` or `"b64_json"` |
| `params.seed` | int | ❌ | Seed for reproducible generation  |
| `params.negative_prompt` | string | ❌ | Negative prompt |
| `params.num_inference_steps` | int | ❌ | Number of inference stepsd |
| `params.extra_params` | object | ❌ | Nebius-specific parameters (see below) |
