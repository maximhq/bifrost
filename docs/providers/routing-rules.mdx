...
<Info>
This only affects a downgrade across the release that added key-pinned fallbacks, and only when a rule has a `key_id` on a fallback.
...
**Symptom**: After the rollback, the log shows `failed to load plugin routing` at startup, the routing plugin reports an error status, and `GET /api/routing/rules` returns 500.

**Cause**: Since 2.2.3, a fallback with a `key_id` is stored as a JSON object in the `fallbacks` column of the `routing_rules` table. Earlier versions only decode string entries. The decode fails for the whole row, and because rules load in one query, that single row takes every routing rule down with it, including rules that never had a pinned fallback.

**Solution**: Rewrite each object entry back to its `"provider/model"` string, then restart Bifrost. The pinned key is dropped, so that fallback goes back to load-balanced key selection. Every other entry is left as it is. Run the statement for your config store backend once against the database; rows without an object entry are not touched.
