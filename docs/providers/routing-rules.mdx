...
Routing Rules provide dynamic, expression-based control over request routing. They execute **after governance routing** and **before adaptive load balancing**, allowing you to make sophisticated routing decisions based on request context, headers, parameters, capacity metrics, and organizational hierarchy.
...
```mermaid
flowchart TD
    Start["Request arrives<br/>with model + provider"]

    subgraph Gov["Governance Layer"]
        GovValidation["1. Virtual Key Validation<br/>(active, permissions, hierarchy)"]
        GovRouting["2. Provider Governance Routing<br/>(weighted selection from provider_configs)"]
    end

    subgraph Rules["Routing Rules Layer"]
        RuleCheck{"3. Match CEL<br/>Expression?"}
        RuleDecision["Rule Matched:<br/>New provider/model/fallbacks"]
        RulePass["No Match:<br/>Keep original"]
    end

    subgraph LB["Load Balancing Layer"]
        LB1["4. Level 1: Provider Selection<br/>(performance-based, if no prefix)"]
        LB2["5. Level 2: Key Selection<br/>(performance-based)"]
    end

    Execute["Execute request<br/>with selected provider + key"]

    Start --> GovValidation --> GovRouting
    GovRouting --> RuleCheck
    RuleCheck -->|Yes| RuleDecision --> LB1
    RuleCheck -->|No| RulePass --> LB1
    LB1 --> LB2 --> Execute
```
...
Governance routing (Virtual Key provider_configs) runs FIRST:

```
1. Virtual Key governance routing → provider/model selection (weighted random)
2. Routing Rules → check expressions (first-match-wins)
3. If rule matches → override provider/model
4. If no rule matches → use governance-selected provider/model
```

**Example:** If governance selects "azure/gpt-4o" but a rule matches with expression `budget_used > 85`, the rule overrides it to "groq/gpt-3.5-turbo".

### Interaction with Load Balancing

Routing rules run BEFORE adaptive load balancing:

```
1. Governance + Routing Rules → provider/model decision (deterministic)
2. Load Balancing Level 1 → skipped (provider already determined)
3. Load Balancing Level 2 → key selection (performance-based within selected provider)
```

**Key Insight:** Even when routing rules select a provider, load balancing Level 2 still optimizes which API key to use within that provider.
...
<CardGroup cols={2}>
  <Card title="Provider Routing" icon="route" href="/docs/providers/provider-routing">
  ...
  <Card title="Governance" icon="shield-check" href="/features/governance">
    Learn about governance layer (runs before routing rules)
  </Card>
