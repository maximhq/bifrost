...
- **The last attempt is never cut off.** In the example, OpenAI and Azure each get 1.5 s. Bedrock, the last fallback, is never cut off by the TTFT deadline and runs under the normal timeouts, so a slow last provider still gets its full request timeout. A rule without fallbacks (and a request that brings none) is never cut off.
...
### Routing Stops Working After a Downgrade

<Info>
This only affects a downgrade across the release that added key-pinned fallbacks, and only when a rule has a `key_id` (or, in `config.json`, a `provider_key_name`) on a fallback.

- OSS: rolling back from 2.2.3 or later to 2.2.2 or earlier
- Enterprise: rolling back from v2.2.3 or later to v2.2.2 or earlier

Staying on either side of that line, or downgrading with no pinned fallback in any rule, is unaffected.
</Info>

**Symptom**: After the rollback, the log shows `failed to load plugin routing` at startup, the routing plugin reports an error status, and `GET /api/routing/rules` returns 500. If the pinned fallback is in `config.json` rather than the database, Bifrost exits at startup with `failed to unmarshal config` instead.

**Cause**: Since 2.2.3, a fallback with a pinned key is a JSON object, both in `config.json` and in the `fallbacks` column of the `routing_rules` table. Earlier versions only decode string entries. In the database, the decode fails for the whole row, and because rules load in one query, that single row takes every routing rule down with it, including rules that never had a pinned fallback. In `config.json`, the decode fails for the whole file.

**Solution**: Rewrite each object entry back to its `"provider/model"` string, then restart Bifrost. The pinned key is dropped, so that fallback goes back to load-balanced key selection. Every other entry is left as it is.

If `config.json` defines routing rules, edit the file first: replace each fallback object under `governance.routing_rules[].fallbacks` with its `"provider/model"` string. Then run the statement for your config store backend once against the database; rows without an object entry are not touched.
...
SET fallbacks = (
  SELECT jsonb_agg(
    CASE jsonb_typeof(e.value)
      WHEN 'object' THEN to_jsonb((e.value ->> 'provider') || '/' || COALESCE(e.value ->> 'model', ''))
      ELSE e.value
    END
    ORDER BY e.ord
  )::text
  FROM jsonb_array_elements(routing_rules.fallbacks::jsonb) WITH ORDINALITY AS e(value, ord)
)
WHERE fallbacks IS NOT NULL
  AND EXISTS (
    SELECT 1 FROM jsonb_array_elements(routing_rules.fallbacks::jsonb) AS e
...
SET fallbacks = (
  ...
  FROM (
    SELECT CASE e.type
      WHEN 'object' THEN json_extract(e.value, '$.provider') || '/' || COALESCE(json_extract(e.value, '$.model'), '')
      ELSE e.value
    END AS entry
    FROM json_each(routing_rules.fallbacks) AS e
    ORDER BY e.key
  )
)
WHERE fallbacks IS NOT NULL
  AND EXISTS (
    SELECT 1 FROM json_each(routing_rules.fallbacks) AS e WHERE e.type = 'object'
  );
```
</Tab>
</Tabs>

A fallback object without a `model` becomes `"provider/"`, which keeps the incoming model, the same as the string form. Back up the database before running the update. If you upgrade again later, pin the key on those fallbacks a second time through the API or UI.
