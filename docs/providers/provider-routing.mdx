...
<Info>
**Position in routing pipeline**: Routing Rules execute **after governance routing** and **before adaptive load balancing**, enabling dynamic provider/model overrides based on runtime conditions like headers, parameters, capacity metrics, and organizational hierarchy.
</Info>
...
flowchart TD
    ...
    Start --> VKValidation --> GovRouting
    GovRouting --> RuleMatch
    RuleMatch -->|Yes| RuleDecision --> LB1
    RuleMatch -->|No| NoMatch --> LB1
    LB1 --> LB2 --> Execute["Execute with<br/>selected provider + key"]
```

### How It Works

1. **Governance selects** a provider using weighted random (or load balancing selects based on performance)
2. **Routing rules evaluate** in scope precedence order (VirtualKey → Team → Customer → Global)
3. **First matching rule** overrides the provider/model/fallbacks
4. **No match** means original provider/model continues to load balancing
5. **Load balancing Level 2** selects the best key within the determined provider
...
Routing Rules work **after** governance routing:
...
If no rule matches, governance decision continues unchanged.
...
For complete documentation, see [Routing Rules Documentation](/docs/providers/routing-rules).
...
<CardGroup cols={2}>
  ...
  <Card title="Routing Rules" icon="sliders" href="/docs/providers/routing-rules">
