...
<Note>
  **Session affinity** sits on top of this order. After PreRequestHook it moves the
  provider that last served the session to the front of the fallback chain, and inside
  Level 2 it reuses the key that last served the session while that key stays in the
  pool. Routing decides which providers and keys are offered; the session only decides
  their order. See [Session Affinity](/providers/session-affinity).
</Note>
