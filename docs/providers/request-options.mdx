...
<Warning>
Per-request overrides are **disabled by default**. You must first enable `allow_per_request_content_storage_override` in your logging configuration (or in the UI under **Logs Settings**) before this header or context key has any effect. When the toggle is off, this value is ignored and the client `disable_content_logging` setting, overridden by the virtual key's own `disable_content_logging` when it has one, is authoritative.
</Warning>
...
**Precedence (when override is enabled):** The per-request value takes precedence over both the global `disable_content_logging` setting and the virtual key's own `disable_content_logging` (see [Content Logging](/features/observability/content-logging)). A value of `true` suppresses content; `false` forces content on.
