...
- **Pattern matching** - target an exact model name or use boundary wildcards for prefix, suffix, or contains matching (e.g. `gpt-4*`, `*-free`, or `*sonnet*`)
...
| `wildcard` | Boundary wildcard match | `gpt-4*` matches by prefix, `*-free` by suffix, and `*sonnet*` by containment |

<Info>
Wildcard patterns support `*` at the beginning, end, or both ends. A standalone `*` matches every model. Interior wildcards such as `gpt-*-mini` and general regular expressions are not supported.
</Info>

Within the same scope, exact matches take precedence over wildcard matches. When multiple wildcard patterns match, the pattern with the longest literal portion wins; if the literal lengths are equal, an edge-anchored prefix or suffix wins over a contains pattern. Remaining ties preserve creation or configuration order.
...
3. Fill in the form:
   ...
   - **Pattern** - enter the model name or a boundary wildcard pattern
...
| `pattern` | string | Yes | Model name or wildcard pattern with `*` at the beginning, end, or both ends |
