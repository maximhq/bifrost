...
The `params_config` field enables opt-in serialization behavior for custom providers that extend a standard provider protocol.

For OpenAI-compatible custom providers, you can preserve `cache_control` on chat message content blocks and tools:
...
By default, the OpenAI serializer strips `cache_control` for official OpenAI compatibility. Enable `preserve_cache_control` only for OpenAI-compatible custom providers whose upstream API accepts this field, such as providers that support prompt caching through content block breakpoints.
