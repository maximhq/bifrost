...
Every management endpoint in the [API reference](/api-reference) states the permission it needs in a **Required permission** line, written as `Resource:Operation` (for example `Dashboard:View`). You do not need to work it out from the URL: open the endpoint page and grant exactly what it lists.
...
Because these rules have exceptions, always rely on the **Required permission** line on the endpoint page rather than the URL.
