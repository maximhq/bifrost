...
<Note>
  `connection_type`, `stdio_config`, `auth_type`, and `connection_string` are immutable after
  creation and are ignored by this endpoint. To change a client's connection type or command,
