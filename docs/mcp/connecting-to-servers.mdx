...
Use per-user OAuth when each end-user should access the upstream service under their own account (e.g., each user's personal Notion workspace or GitHub repos). The same caller identity (VK, signed-in user, or asserted session ID) is honored on both the MCP Gateway and the LLM Gateway, and auth happens lazily — the first tool call that needs an upstream token gets back an inline `authorize_url`.
