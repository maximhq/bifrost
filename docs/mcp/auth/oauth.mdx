...
<Warning>
Replacing the `client_id` marks every credential Bifrost stored under the old one `needs_reauth` and stops using it, since none of them can be refreshed against a client the provider no longer associates them with. It does not revoke the access tokens the provider already issued: Bifrost never calls the provider's revocation endpoint (see [Revoke](#revoke)), so those can stay valid at the provider until they expire. On a shared `oauth` server that is the one connection credential. On a `per_user_oauth` server it is **every end user's credential as well**, and each of them will have to sign in again. Plain **Reauthorize** leaves end users alone; only this action touches them.
</Warning>
...
Error cases: 400 if the client's `auth_type` is not OAuth-based, if it never completed initial authorization (use `initiate-verification` instead), if the OAuth config has no `registration_url` (the provider offers nowhere to register, so only a manually issued `client_id` can be used), or if the provider refuses the registration request (answers it with a 4xx); 404 for an unknown ID; 503 when no OAuth provider is configured. A provider that cannot be reached, or that answers with a 5xx, is a 500: nothing in the request was wrong, and nothing was registered or changed, so it is safe to retry. So is a 500 whose message starts `failed to persist the newly registered oauth client`: Bifrost is unchanged (the stored `client_id` and every token are as they were), but the provider did register a replacement before the local store failed, so it holds a registration Bifrost will never use, and each retry adds one more. Bifrost cannot delete those; remove them at the provider if it lists registered clients.
