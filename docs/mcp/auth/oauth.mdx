...
Error cases: everything `reauthorize` returns, plus 400 when the OAuth config has no `registration_url` (the provider offers nowhere to register, so only a manually issued `client_id` can be used) or when the provider rejects the registration request.
