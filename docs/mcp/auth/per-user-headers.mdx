...
**Lifecycle across restarts and config edits:** the verified state (discovered tools) is server-side and survives restarts and config.json re-syncs, including edits to `per_user_header_keys` — new keys apply to end-user submissions without re-verification. The key list cannot be *emptied*, though: an edit that removes all keys is ignored and the stored keys kept (with a warning at boot). Config-file edits to `auth_type`, `connection_type`, `connection_string`, `stdio_config` are ignored on reconciliation with a boot warning; delete the client and re-declare it to change them for a config.json-declared client. (The update API `PUT /api/mcp/client/{id}` does accept these for an in-place reconfigure, except switching *into* `oauth`/`per_user_oauth`.)
