...
- **Editing** headers on an existing MCP client triggers a connection reset so the new headers take effect immediately. The MCP client's `connection_type`, `connection_string`, `stdio_config`, and `auth_type` can also be edited via the update API (`PUT /api/mcp/client/{id}`): changing any of them tears down and re-dials the transport under the same client id (an in-place reconfigure). Switching `auth_type` *into* `oauth`/`per_user_oauth` after creation is rejected — recreate the client instead; switching *into* `per_user_headers`/`token_exchange` parks the client in `pending_verification` for re-verification. (Edits to these fields in `config.json` are still ignored on reconciliation, with a boot warning.)
