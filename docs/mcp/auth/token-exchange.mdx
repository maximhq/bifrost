...
Token exchange needs an application registered at your identity provider with the token-exchange (or on-behalf-of) grant enabled and permission to mint tokens for your MCP servers' audiences. **Recommended:** register a dedicated application for this, separate from your SSO login application — it keeps each MCP server's OBO grant scoped independently and limits what a leaked secret can reach. Bifrost places no constraint on this beyond what's configured per MCP client: any application your identity provider has authorized for the on-behalf-of grant against the target audience works, including your SSO login application itself if you've explicitly granted it that permission — the exchange app's client ID / secret are read solely from the MCP client's own `token_exchange` config, never compared against or combined with your SCIM provider's credentials. Consult your IdP's documentation for how to register this application; the exact grant name and setup steps vary by vendor:
...
1. **Token issuer mismatch — `oidc: id token issued by a different provider`.** Whether an app's tokens use the v1 issuer (`https://sts.windows.net/<tenant>/`) or the v2 issuer (`https://login.microsoftonline.com/<tenant>/v2.0`) is controlled by the **resource app's manifest**, not by which endpoint you called. If your MCP server validates against the v2 issuer (the standard OIDC discovery shape) but gets v1 tokens, open the resource app's registration → **Manifest** → set `"requestedAccessTokenVersion": 2` under the `api` block → Save.
2. **Audience mismatch — `oidc: expected audience "api://..." got ["..."]`.** Once an app issues v2 tokens, its access tokens carry the app's **client ID (bare GUID)** in `aud`, not the `api://...` Application ID URI you registered — even though you requested the scope using the URI form. Set `token_exchange.audience` (and the MCP server's own audience-validation config) to the bare GUID, not `api://<guid>`.
3. **`AADSTS240002: Input id_token cannot be used as 'urn:ietf:params:oauth:grant-type:jwt-bearer' grant`.** The assertion Entra's OBO endpoint accepts must be an **access token**, never an `id_token` — this applies to the caller's inbound credential that Bifrost re-exchanges, and to any token you hand-craft for manual testing. If you're minting a test token by hand and get this error, you grabbed the `id_token` instead of the `access_token`.
4. **`AADSTS501461: AcceptMappedClaims is only supported for a token audience matching the application GUID...`.** If your SSO app has custom claims mapping enabled (`acceptMappedClaims: true` in its manifest — typically on if you're mapping `roles`/`groups` claims for SCIM attribute mappings), requesting an access token audienced to that app's own `api://<guid>` Application ID URI is rejected outright. Request the scope against the **bare GUID** as the resource identifier instead (e.g. `<client-id>/access_as_user`, not `api://<client-id>/access_as_user`) — Entra treats an app's own client ID as an implicit alternate resource identifier alongside its App ID URI, and that form isn't subject to the same restriction.
5. **Testing OBO manually requires your exchange app to expose an API to itself.** The middle-tier app performing the OBO exchange needs an access token audienced to *itself* to use as the assertion (this is inherent to how Azure OBO validates the incoming token, not a Bifrost requirement). If you want to acquire that token by hand for testing, the exchange app needs its own **Expose an API** scope (e.g. `access_as_user`) with a self-referencing delegated permission under **API permissions** → **My APIs** → (same app) → that scope → admin consent. Bifrost's own exchange call at runtime doesn't need this from you — it already has the caller's validated identity — this is purely for manually minting a test assertion.
