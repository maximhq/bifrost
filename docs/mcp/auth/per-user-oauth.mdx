...
```text
Authentication required for Notion. Open this URL to connect your account: https://your-bifrost-domain.com/workspace/mcp-sessions/auth?flow=<flow-id>#t=<temp-token>
```
...
The temp token in the URL fragment (`#t=…`) authorizes anonymous browser visitors to complete the flow. It never reaches the server logs (fragments are not sent in the request line).
...
- `mcp_external_server_url` — used for the consent URL and any discovery surface
- `mcp_external_client_url` — registered as the `redirect_uri` with upstream providers
