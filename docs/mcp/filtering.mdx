...
Bifrost Enterprise adds **MCP Tool Groups** on top of the per-virtual-key controls described above. Tool groups let you build reusable sets of MCP tools and attach them to multiple governance resources, including virtual keys, teams, customers, users, providers, API keys, and MCP clients.

The enterprise list endpoint supports search and pagination so large catalogs stay manageable:

```bash
curl "http://localhost:8080/api/mcp/tool-groups?search=support&limit=25&offset=0"
```
...
Use tool groups when you want a centrally managed allow-list that can be reused across many enterprise identities or workloads instead of hand-configuring MCP tool access on each virtual key individually.
