...
can actually reach. Each server's contribution is capped at 4096 bytes and the whole aggregate at
16384 bytes, with an explicit marker when anything is trimmed. Both bounds are configurable via
`mcp.tool_manager_config.max_instructions_per_client` and `max_instructions_total` (or the
**MCP** settings page); `0` keeps the default. A per-server cap above the total is rejected.
