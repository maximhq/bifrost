...
<Note>
The `content` field contains a JSON summary of executed tool results. When a tool runs more than once, its value is an object keyed by `tool_call_id`. The `tool_calls` array contains only the non-auto-executable tools that require your approval. The `finish_reason` is set to `"stop"` to exit the agent loop.
</Note>
