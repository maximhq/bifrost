...
<Warning>
`issuer_url` is required whenever `mcp_server_auth_mode` is `both` or `oauth`, for single-host deployments too: saving a config that turns on discovery without one is rejected. Left to a per-request fallback, the issuer identity would be derived from the (unauthenticated, spoofable) request `Host` header, which lets an attacker who can reach the always-public `/.well-known/` endpoints, even on a locked dashboard, poison the `issuer`, `token_endpoint`, and `jwks_uri` that MCP clients trust.
</Warning>
