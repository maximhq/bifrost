...
2. **At each tool call, while the code is running (authoritative).** Every time the running code
   ...
   against `tools_to_auto_execute` again before making the call. A tool not on the list is refused
   at that moment, regardless of how it was called or whether the pre-flight scan let the code run
   without asking. This is what makes the guarantee below hold even for code the pre-flight step
