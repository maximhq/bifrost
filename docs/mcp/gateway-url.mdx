...
When at least one MCP server is configured with `per_user_oauth`, the `/mcp` endpoint serves per-user tokens lazily. Bifrost is **not** an OAuth Authorization Server — there is no `.well-known/oauth-protected-resource` discovery and no inbound consent screen. Inbound MCP clients identify themselves via headers:
...
When a tool call hits an MCP server the caller has not authenticated against, Bifrost returns an `mcp_auth_required` tool result containing an inline `authorize_url`. The user opens it, completes the upstream OAuth flow, and the next tool call executes normally.

<Note>
Claude Code's MCP client may proactively POST `/register` (RFC 7591 DCR) on `claude mcp add` and log `SDK auth failed: …`. Bifrost intentionally does not implement an OAuth stub for that probe; the `/mcp` connection itself works. See the [Claude Code bug report](https://github.com/anthropics/claude-code/issues/46640) for context.
</Note>

See [Per-User OAuth →](./per-user-oauth) for the full flow and identity options, and [MCP Sessions →](./sessions) for managing the resulting tokens.
