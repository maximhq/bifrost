...
| `description` | string | No | Free text. Operator-facing; never sent to a model |
| `instructions` | string | No | Model-facing guidance served on `initialize`. On update, `""` clears the stored text; omitting the field leaves it unchanged |
| `instructions_mode` | string | No | `append` (default) keeps the source servers' own instructions and adds these after; `replace` sends only these |
...
| `description` | string | No | Free text. Operator-facing; never sent to a model |
| `instructions` | string | No | Model-facing guidance served on `initialize`. Empty inherits only what the source MCP servers advertise |
| `instructions_mode` | string | No | `append` (default) or `replace`. `replace` drops the inherited text from every source server |
...
Source servers are quoted and attributed; your own text is the endpoint speaking, so it is
unwrapped and comes last. A Virtual MCP is a Bifrost concept, so nothing names it to the model
— to the client this is simply the server it connected to. With `replace`, your text is all the
client receives.
