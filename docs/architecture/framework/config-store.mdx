...
The ConfigStore includes a migration system to handle database schema changes between Bifrost versions. Migrations are automatically applied at startup, ensuring the database schema is always up-to-date. This process is managed by the `migrator` package and is transparent to the user. On PostgreSQL, an advisory lock serializes migrations across nodes so only one instance migrates at a time.

For multi-replica deployments, migrations can instead be applied out of band: run Bifrost with `--migrate-only` (applies all migrations and exits) as a one-shot job, and start the server replicas with `--no-migrate` (skips migrations, failing fast if any are pending). See [Database migrations in multi-replica deployments](../../deployment-guides/k8s#database-migrations-in-multi-replica-deployments).
