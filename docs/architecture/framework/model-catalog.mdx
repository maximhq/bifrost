...
- **Image Processing**: Per-image and token-based pricing with text/image token breakdown.
- **Video Processing**: Duration-based output pricing with optional token-based input.
...
// Returns 0.0 if pricing data is not found (logs a warning)
```
