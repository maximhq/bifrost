...
- **Time-Based Pricing**: Recurring `pricing_schedule` windows can apply a multiplier in an IANA timezone. Schedules use the provider attempt start time and are applied after service-tier and context-tier rates resolve. The schedule format is provider-generic; DeepSeek's weekday peak windows are one consumer, not a provider-specific billing path.
