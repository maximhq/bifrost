...
- **Text Operations**: Token-based pricing for chat completions, text completions, responses, and embeddings with provider-level cache token support.
- **Audio Processing**: Token-based and duration-based pricing for speech synthesis and transcription, with audio token detail breakdown.
...
- **Video Processing**: Duration-based output pricing (`output_cost_per_video_per_second`) with optional token-based input; when prompt-token usage is absent, input can also be billed by duration via `input_cost_per_video_per_second`.
