...
- **Model catalog:** `ctx.GetModelInfo(provider, model)` returns pricing, context window and capability metadata; `ctx.CalculateCost(resp)` prices a completed response with governance overrides applied; `ctx.CalculateCostBreakdown(resp)` returns that same price split into input, output and additional sides with per-category details (text, cache read, cache write, reasoning, guardrail, and so on)
