...
## Enterprise API Key Catalog

Bifrost Enterprise also exposes a dedicated API key catalog for managing gateway-issued API keys at scale. This is separate from provider keys: provider keys are used for upstream routing, while the enterprise API key catalog is used for issuing, searching, and governing access to Bifrost itself.

The list endpoint supports server-side search and pagination:

```bash
curl "http://localhost:8080/api/api-keys?search=prod&limit=25&offset=0"
```
...
The enterprise dashboard uses the same wrapped response shape for searchable CRUD workflows, so large API key inventories remain manageable without loading the full catalog into memory.
