...
Note: Both mechanisms reference a stored key (not the raw secret). The gateway resolves the key against configured provider keys and applies model filtering, deployment mapping, and weighted selection as usual.
