...
| `bifrost_error_requests_total`     | Counter   | Total failed requests, by effective status and normalized reason   | Base Labels, `status_code`, `error_type`, custom labels |
...
| `bifrost_provider_key_up`          | Gauge     | Per-key health: `1` after a successful attempt, `0` after a failed attempt, except a failure that says nothing about the key's health (`model_access`, `model_gone`, `region_blocked`), which leaves it unchanged | `provider`, `key_id`, `key_name`                    |
| `bifrost_key_rotation_events_total`| Counter   | Key rotations triggered by per-key failures (rate limit, rejected credential, exhausted quota, model this key cannot reach, retired model, region block), one increment per actual swap | `provider`, `requested_model`, `key_id`, `key_name`, `fail_reason` |
