...
| `bifrost_key_rotation_events_total`| Counter   | Key rotations triggered by per-key failures (rate limit, rejected credential, exhausted quota, model this key cannot reach, retired model), one increment per actual swap | `provider`, `requested_model`, `key_id`, `key_name`, `fail_reason` |
