...
1. **A cache key is mandatory for stored-key requests.** Caching only engages when a request carries a cache key (the `x-bf-cache-key` header, or the `CacheKey` context value in the Go SDK). Without one — and without a configured `default_cache_key` — the request bypasses the cache entirely. [Direct API Key](/providers/request-options#direct-api-key) requests are different: Bifrost derives their partition from the raw provider key when `direct_key_cache_secret_env` is configured.
...
| `direct_key_cache_secret_env` | string | `""` (empty) | Environment variable that contains the server-side HMAC secret used to isolate [Direct API Key](/providers/request-options#direct-api-key) cache partitions. The secret must be at least 32 bytes. |
...
### Direct Key cache isolation

Direct Key callers must not choose their own cache partition. In either cache mode, set `direct_key_cache_secret_env` to the name of a server-side environment variable containing at least 32 bytes of secret material. Bifrost derives a stable `direct-key-hmac-v1:<sha256>` namespace from the raw provider key and the secret; the raw key and secret are never stored in cache metadata. This derived namespace takes precedence over `x-bf-cache-key` and `default_cache_key`.
...
Set `BIFROST_CACHE_NAMESPACE_SECRET` in the Bifrost process environment. If the setting is absent, the environment variable is missing, the secret is shorter than 32 bytes, or the Direct Key is empty, Bifrost fails closed by skipping cache lookup and write for that request. Rotating the secret creates new namespaces, so existing Direct Key entries stop matching and expire normally.
...
<Warning>
**A cache key is mandatory for stored-key requests.** Caching only activates when a request carries a cache key. Without one (and without a configured `default_cache_key`), the request bypasses caching entirely. Direct Key requests use the server-derived namespace described above instead.
</Warning>
...
| `x-bf-cache-bypass` | `CacheBypassKey` | `true` or `1` | Skip both cache lookup and cache write. |
...
# Force a live provider request and don't store its response
curl -H "x-bf-cache-key: session-123" \
     -H "x-bf-cache-bypass: true" ...
```
...
// Force a live provider request and don't write its response
...
Bifrost also applies full bypass automatically when the request can depend on external or stateful execution, including tool, MCP, web-search, stored/background, continuation/conversation, audio, and non-text input modes. This prevents replaying responses whose result is not determined by the cache key alone.
