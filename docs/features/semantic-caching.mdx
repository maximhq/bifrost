...
<Note>
When a request carries a resolved virtual key, the effective cache partition is automatically
scoped to that virtual key on top of whatever `x-bf-cache-key`/`default_cache_key` you set - two
different virtual keys can never share a cache entry, even if they're configured with the exact
same cache key. This applies with zero configuration; there's nothing to opt into. Requests
without a virtual key (no governance/VK enforcement in use) are partitioned by cache key alone,
same as before.
</Note>
...
| `x-bf-cache-threshold` | `CacheThresholdKey` | float (0–1) | Raise the semantic similarity threshold for this request (stricter matching, fewer hits). Clamped to `[0,1]`; a value below the plugin's configured threshold is floored at the configured value rather than loosening it - callers can only make matching stricter, never looser. |
