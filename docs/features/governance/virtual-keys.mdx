...
**Content logging** (optional): **Inherit** (default), **Off for this key**, or **On for this key**. A setting on the key's team outranks it. See [Content logging per key](#content-logging-per-key).
...
- omitted or `null`: inherit (the key's team decides if it sets a value, otherwise `client.disable_content_logging`)
...
The key is part of the credential tier, below its business unit, team and user: a team that turns content off cannot be reopened by the key. Within the tier, a provider key or access profile that says `true` wins over a key that says `false`. The `x-bf-disable-content-logging` header, when per-request overrides are allowed, still has the final word for the log store. See [Content Logging](/features/observability/content-logging) for how each layer treats the log store and observability connectors.
