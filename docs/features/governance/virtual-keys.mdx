...
- **Delete after expire** (optional) — a cleanup job that runs once a day removes expired keys. Turn it on for the whole workspace under **Settings → Security → Delete Expired Virtual Keys** (config.json: `client.delete_expired_virtual_keys`, default `false`), or per key with the **Delete after expire** switch (API: `delete_after_expire`). A key with no value of its own follows the workspace setting; `true` or `false` on the key wins over it. Each run that deletes at least one key posts a notification listing the removed keys; runs that find nothing stay silent. The per-key value needs an expiry: setting it without one returns `400`, and clearing the expiry resets the key to inherit.
...
3. Optionally flip **Delete after expire**. The switch starts at the workspace setting from **Settings → Security**; leave it there to follow that setting, or flip it to decide for this key

![Virtual Key Expiry Picker](../../media/ui-virtual-key-expiry.png)

Expired keys show an **Expired** badge in the virtual keys table, with **auto-delete** appended when the key will be cleaned up, whether by its own setting or the workspace default.
...
Timestamps must be RFC3339 and in the future; otherwise the API returns `400`. On update, omitting `expires_at` leaves the current expiry unchanged. `delete_after_expire` can be sent on create or update: omit it to follow `client.delete_expired_virtual_keys`, send `true` or `false` to decide for this key, or send `null` on update to go back to inheriting. Any value is rejected with `400` when the key has no expiry.
