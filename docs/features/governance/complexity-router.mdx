...
<Note>
The default keyword lists are tuned for common request patterns and are a good starting point for most deployments. For domain-specific traffic, add or remove keywords based on the prompts your users actually send so the tiers match your routing strategy. Bifrost stores and displays the exact keywords you configure; internally, normal word-based keywords and phrases also match common word forms, such as `debug`, `debugging`, and `debugged`. Punctuation-heavy terms such as `ci/cd` still use literal matching. Keywords written in scripts that do not separate words with spaces — such as Chinese, Japanese and Thai — match anywhere inside a longer run of text, since there is no space to anchor them to. Korean is treated the same way, because particles and suffixes attach directly to the word. A short keyword can therefore match inside an unrelated longer word: the Japanese keyword `コード` ("code") also matches within `レコード` ("record"). An ordinary English keyword also matches when it is embedded in such a run, so `api` matches inside `このapiを直して`. If a keyword produces unwanted matches, replace it with a longer and more specific term.
</Note>
