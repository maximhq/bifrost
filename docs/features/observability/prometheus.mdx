...
| `bifrost_error_requests_total` | Counter | Failed requests, by effective `status_code` and normalized `error_type` — see [Error Types](#error-types) |
...
| `bifrost_provider_key_up` | Gauge | Per-key health. `1` after a successful attempt, `0` after a failed attempt, except a failure that says nothing about the key's health (`model_access`, `model_gone`, `region_blocked`), which leaves it unchanged. Labels: `provider`, `key_id`, `key_name`. |
| `bifrost_key_rotation_events_total` | Counter | Key rotations triggered by per-key failures (rate limit, rejected credential, exhausted quota, model this key cannot reach, retired model, region block), see below <sup>v1.5.0-prerelease4+</sup> |
...
`bifrost_error_requests_total` carries two error dimensions. `status_code` is the status the failure resolves to — the caller's status over HTTP, including the 500s Bifrost raises internally, but not for an error delivered inside an already-committed stream, where the caller saw a 200 and the error arrived in the event body. `error_type` is the normalized interpretation: a closed, low-cardinality vocabulary that answers **whose fault was it**, which is what alarms actually need.
...
`bifrost_key_rotation_events_total` is incremented once per **actual key rotation**, i.e. when a per-key failure causes the next attempt to switch to a different key. Rotation-triggering failures are bound to the specific key/account rather than the request (see [how failures are classified](/features/retries-and-fallbacks#how-failures-are-classified)):

- `rate_limit`: this key is rate-limited; another may have capacity.
- `credential`: bad / revoked key, or key lacks permission.
- `quota`: the account behind the key is out of credit or over a spend cap.
- `model_access` / `model_gone`: this key cannot reach the model, or the model was retired; another key may still serve it.
- `region_blocked`: the provider refuses the request's location. Gemini refuses a free-tier project where a billed project is served from the same address, and Bedrock checks the account's billing address as well as the caller's, so another key may serve. When the block is the gateway's own address (OpenAI checks only that), the walk ends with the provider's error after one refused call per key.

Permanent per-key failures rotate even at `max_retries = 0`, so the counter moves for them regardless of the retry budget. It is **not** incremented for:

- terminal failures (no key left to move to),
- same-key retries on transient 5xx / network errors,
- non-retryable request-bound 4xx.
...
| `fail_reason` | error type string | Reason the rotation fired: `rate_limit_error`, `authentication_error`, `billing_error`, `model_access_error` or `model_retired_error` |
