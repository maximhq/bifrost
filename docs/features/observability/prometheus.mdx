...
| `bifrost_provider_key_up` | Gauge | Per-key health. `1` after a successful attempt, `0` after a failed attempt, except a failure about the model rather than the key (`model_access`, `model_gone`), which leaves it unchanged. Labels: `provider`, `key_id`, `key_name`. |
| `bifrost_key_rotation_events_total` | Counter | Key rotations triggered by per-key failures (rate limit, rejected credential, exhausted quota, model this key cannot reach, retired model), see below <sup>v1.5.0-prerelease4+</sup> |
...
A `region_blocked` failure never rotates: the provider refuses the gateway's location, which every key shares.
