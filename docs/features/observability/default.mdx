...
| `attempt_trail` | Ordered array of every attempt. Each entry contains `attempt`, `key_id`, `key_name`, `fail_reason` (set on every failed attempt, including the terminal one, and omitted from the JSON on a successful attempt), `failure_class` and `status_code` (set on every failed attempt, including transport failures where the provider never answered: those are classed `transient` and carry Bifrost's synthetic `502`; see [how failures are classified](/features/retries-and-fallbacks#how-failures-are-classified)), and `triggered_rotation` (always present; `true` only when this attempt's per-key failure (a rate limit, a rejected credential, an exhausted quota, a model this key cannot reach, a retired model, or a region block on this key's project or account) caused the next retry to switch to a different key, `false` otherwise). |
