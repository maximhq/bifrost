...
| `attempt_trail` | Ordered array of every attempt. Each entry contains `attempt`, `key_id`, `key_name`, `fail_reason` (set on every failed attempt, including the terminal one, and omitted from the JSON on a successful attempt), `failure_class` and `status_code` (set on every failed attempt the provider answered; see [how failures are classified](/features/retries-and-fallbacks#how-failures-are-classified)), and `triggered_rotation` (always present; `true` only when this attempt's per-key failure (a rate limit, a rejected credential, an exhausted quota, a model this key cannot reach, or a retired model) caused the next retry to switch to a different key, `false` otherwise). |
