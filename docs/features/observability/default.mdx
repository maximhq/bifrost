...
[Required headers](../governance/required-headers) and logging headers serve different purposes and can be used together. Required headers enforce both presence **and** optional value constraints — a request is rejected with a 400 if a header is missing or its value doesn't match the configured pattern.

| Feature | Purpose | Effect on Request |
|---------|---------|-------------------|
| **Required Headers** | Enforce header presence and value constraints | Rejects request if missing or value doesn't match the pattern (400) |
...
```json
{
  "client": {
    "required_headers": {
      "X-Tenant-ID": "*",
      "X-Region": "us-east-1|eu-west-1"
    },
    "logging_headers": ["X-Tenant-ID", "X-Region"]
  }
}
```
