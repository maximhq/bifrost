...
| `apply_trace_dimensions_to_child_spans` | `boolean` | ❌ No | Merge `x-bf-dim-*` request dimensions onto child spans as well as the root (default: `false`). Reserved suffixes `path` and `method` are skipped; span-level attributes win on conflict |
