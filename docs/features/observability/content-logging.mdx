...
|-------------|-------------|
| **Log store** (Postgres / SQLite / ClickHouse row) | `client.disable_content_logging`, overridden per virtual key by the key's `disable_content_logging` |
...
| **Observability connectors** (BigQuery, Kafka, Pub/Sub, OpenTelemetry, Datadog) | Each connector's own `disable_content_logging`. A virtual key with content logging **off** also strips content from OpenTelemetry export |
...
### 2. Per virtual key: `disable_content_logging` on the key

A virtual key can carry its own decision, set in the key's editor under **Content logging**, through `POST`/`PUT /api/governance/virtual-keys`, or in `config.json`:
...
|-------|-----------|--------------------------|
| omitted / `null` | Inherits `client.disable_content_logging` | Each connector's own flag |
| `true` | Content is not stored for this key's traffic, whatever the client setting says, unless an allowed `x-bf-disable-content-logging: false` header re-enables it for one request | Content is stripped from OpenTelemetry export as well. Other connectors keep their own flag for now |
| `false` | Content is stored for this key's traffic even when the client setting is `true` | Unchanged: a key can never loosen a connector's own `disable_content_logging` |

The key's decision is admin configuration and needs no per-request override gate. `client.retain_content_in_object_storage` applies to a key-disabled request exactly as it does to a client-disabled one. On update, send `null` to clear the decision back to inherit; omitting the field leaves it unchanged.

### 3. Per-request: `x-bf-disable-content-logging`

Overrides the client and virtual key settings for a single request, in either direction: `true` suppresses content for a request that would otherwise be logged, `false` captures content while the client or key setting has turned it off.

Per-request overrides are **off by default**. Enable `client.allow_per_request_content_storage_override` first. While it is off, the header is ignored and the client setting, overridden by the virtual key's own setting when it has one, is authoritative.
...
Every observability connector has an independent `disable_content_logging`, defaulting to `false`. It is **not** inherited from the client setting. A virtual key with `disable_content_logging: true` tightens it for that key's traffic (OpenTelemetry today); nothing on a key can loosen it.
...
<Warning>
For a workload where content must not leave the deployment, set `disable_content_logging: true` on the client **and on every configured connector**. Setting only the client flag still exports full content everywhere. Two layers can still put content back into the log store: a virtual key with `disable_content_logging: false`, and the `x-bf-disable-content-logging: false` header while `client.allow_per_request_content_storage_override` is on. Audit your virtual keys for `false` and keep per-request overrides off. `client.retain_content_in_object_storage` keeps the full payload of every content-disabled request in the object storage bucket. If content must not be persisted at all, set it to `false`. If content may be retained but must not leave the deployment, it can stay on only with that bucket inside the approved deployment boundary.
</Warning>
