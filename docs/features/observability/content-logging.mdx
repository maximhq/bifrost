...
|-------------|-------------|
| **Log store** (Postgres / SQLite / ClickHouse row) | `client.disable_content_logging`, overridden by the admin layers (team, provider key, virtual key, and the enterprise levels) |
...
| **Observability connectors** (BigQuery, Kafka, Pub/Sub, OpenTelemetry, Datadog) | Each connector's own `disable_content_logging`. Admin layers that resolve to **off** also strip content from OpenTelemetry export |
...
### 2. Admin layers: team, provider key, and virtual key

Teams, provider keys, and virtual keys each carry their own tri-state `disable_content_logging`. The enterprise build adds the same field on business units, users, and access profiles. Set it in the entity's editor under **Content logging**, through its API, or in `config.json`:

```json
{
  "governance": {
    "teams": [
      { "id": "team-legal", "name": "legal", "disable_content_logging": true }
    ],
    "virtual_keys": [
      {
        "id": "vk-support-bot",
        "name": "support-bot",
        "value": "env.SUPPORT_BOT_VK",
        "team_id": "team-legal",
...
In this example the team outranks the key, so `support-bot` traffic keeps content out of the log store even though the key says `false`.

Each field is tri-state:

| Value | Log store | Observability connectors |
|-------|-----------|--------------------------|
| omitted / `null` | Inherits: the next layer down decides | Each connector's own flag |
| `true` | Content is not stored for this traffic, unless an allowed `x-bf-disable-content-logging: false` header re-enables it for one request | Content is stripped from OpenTelemetry export as well. Other connectors keep their own flag for now |
| `false` | Content is stored even when the client setting is `true` | Unchanged: an admin layer can never loosen a connector's own `disable_content_logging` |

The layers resolve top down. The first one that sets a value decides, and a lower one applies only when every layer above it inherits:

| Order | Layer | Set on |
|-------|-------|--------|
| 1 | Business unit | The business units the caller belongs to (Enterprise) |
| 2 | Team | The virtual key's team, or the caller's teams |
| 3 | User | The calling user (Enterprise) |
| 4 | Credential | The virtual key, the provider key that serves the request, and the caller's access profiles (Enterprise). These are peers: if any says `true`, content is off |
| 5 | Client | `client.disable_content_logging` |

The org hierarchy outranks the credential. A team that turns content off cannot be reopened by a virtual key or provider key that turns it on, and a team that turns it on wins over a key that turns it off. Where one layer holds several values (a user in two business units, a user holding two access profiles), any `true` wins.

The provider key is known only once Bifrost picks it, after the request has passed through governance. Once a request has been sent on a provider key that says `true`, a retry or fallback onto another key keeps content off for that request.

These decisions are admin configuration and need no per-request override gate. `client.retain_content_in_object_storage` applies to a request they disable exactly as it does to a client-disabled one. On update, send `null` to clear a team's or virtual key's decision back to inherit; omitting the field leaves it unchanged. A provider key is saved whole, so omitting the field on a key update clears it.

### 3. Per-request: `x-bf-disable-content-logging`

Overrides the client setting and every admin layer for a single request, in either direction: `true` suppresses content for a request that would otherwise be logged, `false` captures content while the client or an admin layer has turned it off.

Per-request overrides are **off by default**. Enable `client.allow_per_request_content_storage_override` first. While it is off, the header is ignored and the client setting, overridden by the admin layers when one of them decides, is authoritative.
...
### 4. Per-connector: each connector's own flag

Every observability connector has an independent `disable_content_logging`, defaulting to `false`. It is **not** inherited from the client setting. Admin layers that resolve to `true` tighten it for that traffic (OpenTelemetry today); no admin layer can loosen it.
...
| [Datadog](/features/observability/datadog) | `disable_content_logging` | Covers APM spans and LLM Observability payloads |
...
<Warning>
For a workload where content must not leave the deployment, set `disable_content_logging: true` on the client **and on every configured connector**. Setting only the client flag still exports full content everywhere. Two things can still put content back into the log store: an admin layer (team, provider key, virtual key, or an enterprise level) with `disable_content_logging: false`, and the `x-bf-disable-content-logging: false` header while `client.allow_per_request_content_storage_override` is on. Audit those entities for `false` and keep per-request overrides off. `client.retain_content_in_object_storage` keeps the full payload of every content-disabled request in the object storage bucket. If content must not be persisted at all, set it to `false`. If content may be retained but must not leave the deployment, it can stay on only with that bucket inside the approved deployment boundary.
</Warning>
