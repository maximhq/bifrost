...
| `export_raw_payloads` | `boolean` | No | `false` | When `true`, stores the raw provider request/response bodies in the `raw_request` and `raw_response` columns. Requires the provider's `store_raw_request_response`; suppressed by `disable_content_logging`. Bodies over 256 KB are dropped. Guardrail redaction still applies; raw bodies carry provider-specific fields the normalized view does not. |
