...
- **Retries** - When a provider returns a transient server error (network issue, 5xx) or a per-key failure (`429` rate-limit, `401`/`403` auth, `402` billing), Bifrost automatically retries the same request against the same provider. Transient-server retries reuse the same key with exponential backoff; per-key failures rotate to a different API key from your pool. Backoff is skipped only when rotating away from a _permanent_ per-key failure (`401`/`402`/`403`) where waiting offers nothing — for `429` rotations a backoff is still applied to let account-level quota windows slide.
...
<Frame>
  <img
    src="/media/ui-retries-network-config.png"
    alt="Retries configuration in the Bifrost Web UI showing Max Retries, Retry Backoff Initial, and Retry Backoff Max fields under Network Config"
  />
</Frame>
...
### Fallback on abnormally slow streams

An upstream can keep a stream alive while producing text too slowly for an interactive or agent workload. `stream_idle_timeout_in_seconds` does not catch this condition because data is still arriving.

Configure `stream_throughput_guard` on the primary provider to hold its initial text chunks behind a pre-commit probe:
...
```json
{
  "providers": {
    "openai": {
      "network_config": {
        "stream_throughput_guard": {
          "minimum_output_characters_per_second": 20,
          "probe_window_in_seconds": 5
        }
      }
    }
  }
...
`minimum_output_characters_per_second` is required and must be an integer from 1 to 100,000. `probe_window_in_seconds` is optional, defaults to 5 seconds, and must be an integer from 1 to 60. Unknown fields are rejected.

The guard is disabled when omitted. It applies to streamed text completions, chat completions, and Responses API requests. Bifrost buffers lifecycle and text chunks until the provider emits enough output to pass the configured rate. Healthy streams commit early; if the probe expires first, Bifrost cancels only that provider attempt and uses the existing retry and fallback chain.

The probe target is capped at 256 output characters, and buffered data is capped at 512 chunks or 1 MiB. Probe expiry produces the retryable `stream_throughput_below_minimum` error. Reaching a buffer cap before the rate can be verified produces the retryable `stream_throughput_probe_buffer_exceeded` error. Both paths use the configured retry and fallback chain without exposing the rejected attempt's buffered output.

<Note>
  The threshold is measured in Unicode output characters per second, not
  provider-reported tokens per second. Live token counts are generally
  unavailable until a stream's final usage event. Calibrate the threshold from
  observed output for each provider and model.
</Note>

Because no primary chunk is sent before the probe commits, a fallback produces one coherent consumer-visible stream. After the probe commits, Bifrost never switches providers mid-stream: replaying from another provider would duplicate or contradict output the consumer already received. Short responses that finish before filling the sample are returned normally.

The guard can add time to first token, and a rejected primary attempt may still be billed by the upstream before the fallback begins. Keep the probe window and retry budget aligned with your latency and cost limits.

### Key rotation on per-key failures

<Note>
  Key rotation on retries requires **v1.5.0-prerelease4 or later**. Rotation on
  auth (401/403) and billing (402) errors (in addition to rate limits) requires
  the retry-logic-enhancements release.
</Note>
...
<Info>
  The retry budget is set per-provider in `network_config`. If your fallback
  providers have different retry configurations, each will use their own
  settings.
</Info>
...
The failure context attached to each entry is intentionally categorical — only the error type (e.g. `rate_limit_error`) and HTTP status code. The upstream provider message is _never_ included, since providers can echo back API keys, tokens, or user input. The key identifier surfaced in retry rotation notes is the user-set key **name**, not the secret value.
