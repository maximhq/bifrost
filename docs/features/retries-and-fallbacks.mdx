...
| `region_blocked` | The provider refuses the gateway's location: OpenAI `403 unsupported_country_region_territory`, Gemini "User location is not supported" / "not available in your country", Anthropic on Bedrock "not allowed from unsupported countries". Every key shares that location, so no other key would fare better | No | No |
...
| Region block (`403 unsupported_country_region_territory`, Gemini "User location is not supported") | No | - | - |
...
<Note>
Key rotation on retries requires **v1.5.0-prerelease4 or later**. Rotation on auth (401/403) and billing (402) errors (in addition to rate limits) requires the retry-logic-enhancements release. Rotation on model access and retired model errors, and rotation past permanent failures at `max_retries: 0`, require the failure-class release.
</Note>
...
**Permanent per-key failures** (credential, quota, model access, retired model) are different: the failing key is marked **permanently dead** for the remainder of the request and is never reset, and the attempt is granted back rather than charged against `max_retries`, so the walk to the next key happens even at `max_retries: 0`. If any key was refused for the model, that error is returned. If every configured key ends up permanently dead for an account-level reason (credential, quota) within the configured budget, Bifrost returns `502 upstream_credentials_exhausted` and skips any remaining retries; if the pool ran out on a granted attempt, the last upstream error is returned as is.

The walk has a cost worth knowing: a model no key can reach, including a typo in the model name, now costs one upstream call per configured key before the error (or the fallback provider) is reached, instead of one.
