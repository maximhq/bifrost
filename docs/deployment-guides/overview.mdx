...
|---|---|
| One Bifrost instance using SQLite | SQLite in a persistent `/app/data` directory |
| One instance using PostgreSQL for both stores | A reachable PostgreSQL database; no persistent Bifrost `/app/data` volume |
...
| Several instances with configuration managed through the dashboard or API | Bifrost Enterprise clustering with PostgreSQL |
...
<Warning>
A SQLite database belongs to one Bifrost process. Do not mount the same SQLite database into several containers. OSS instances using a shared PostgreSQL configuration store also do not automatically update each other's in-memory configuration; use the [OSS multinode setup](/deployment-guides/how-to/multinode) or Enterprise clustering.
</Warning>
...
- An application directory at `/app/data`; persist it only when a store uses SQLite.
...
3. For SQLite, prepare persistent `/app/data` storage. For PostgreSQL-only storage, prepare the database connection instead.
