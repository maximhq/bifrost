...
<Warning>
**This is a separate control from the dashboard password.** Locking the dashboard with an admin password does not, by itself, protect `/v1/*` - that surface is intentionally public-by-default because it's the API your own clients call, and enforcement is delegated to this toggle instead of session auth. Check both.
</Warning>
...
<Note>
**Defaults on when you create your first admin account**, unless your `PUT /api/config` request explicitly sets it to `false`. This only applies the moment the account is created - it does not retroactively change the setting for a deployment that already has one, so if you set up your admin account before this behavior shipped, check the toggle yourself. `GET /api/session/is-auth-enabled` also reports the current state as `inference_auth_enforced`, and the Security Settings page shows a warning when the dashboard is locked but this is still off.

If you don't already have a virtual key, inference calls will start returning `401 virtual_key_required` as soon as this is on - [create one](/features/governance/virtual-keys) before or immediately after locking the dashboard so existing integrations don't break.
</Note>
