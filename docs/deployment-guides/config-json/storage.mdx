...
### ClickHouse Replicated Database

For a native ClickHouse `ENGINE = Replicated` database, let ClickHouse manage
DDL replication. Bifrost creates `ReplicatedReplacingMergeTree(ver)` tables
without `ON CLUSTER` or an explicit Keeper path.
...
```json
{
  "logs_store": {
    ...
    "config": {
      ...
      "table_engine": "REPLICATED_MERGETREE"
    }
  }
}
```

`managed_replication` and `table_engine` can also be supplied as
`CLICKHOUSE_MANAGED_REPLICATION` and `CLICKHOUSE_TABLE_ENGINE`. A
`table_engine` omitted from the configuration defaults to `MergeTree`.
Select `REPLICATED_MERGETREE` explicitly for managed mode. Do not set `cluster`
in managed mode. For unmanaged `REPLICATED_MERGETREE`, `cluster` is required as
a deployment-safety check, but Bifrost does not emit `ON CLUSTER`; create the
table on each replica through your migration process. With an explicit
non-replicated `"table_engine": "MergeTree"`, a configured `cluster` does emit
`ON CLUSTER` so every member receives the table definition.
