...
| `password` | - | Database password (supports `env.` prefix). Use `password_command` when the password must be generated dynamically. |
| `password_command` | - | Command executed without a shell to produce the database password on stdout for each new physical connection. |
