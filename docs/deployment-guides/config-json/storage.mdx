...
```json
{
  "logs_store": {
    ...
    "config": {
      ...
      "engine": "REPLICATED_MERGETREE"
    }
  }
}
```

`managed_replication` and `engine` can also be supplied as
`CLICKHOUSE_MANAGED_REPLICATION` and `CLICKHOUSE_ENGINE`. Do not set `cluster`
...
non-replicated `"engine": "MergeTree"`, a configured `cluster` does emit
