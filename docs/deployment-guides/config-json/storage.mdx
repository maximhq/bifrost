...
| `password` | - | Database password (supports `env.` prefix). Mutually exclusive with `password_command`; configure only one password source alongside `db_name`. |
| `password_command` | - | Command executed without a shell to produce the database password on stdout for each new physical connection. Mutually exclusive with `password`; runtime validation rejects configs that set both fields. |
