...
| `disable_content_logging` | No | Tri-state. Omit to inherit: the key's team decides if it sets a value, otherwise `client.disable_content_logging`. The team outranks the key. `true` keeps this key's request and response content out of the log store (unless an allowed `x-bf-disable-content-logging: false` header re-enables it for one request) and out of OpenTelemetry export. `false` stores content for this key even when the client setting is `true`. See [Content Logging](/features/observability/content-logging) |
...
A team can also carry `disable_content_logging` (tri-state: omit to inherit, `true` forces content off, `false` forces it on). It outranks every virtual key and provider key its traffic comes in with. See [Content Logging](/features/observability/content-logging#2-admin-layers-team-provider-key-and-virtual-key) for the full order.
