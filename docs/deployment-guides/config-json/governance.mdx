...
| `key_ids` | No | Provider keys allowed for this VK. `["*"]` = all keys; `[]` = deny all. Entries may be key `name` values (recommended for GitOps, stable across environments) or key `key_id` UUIDs |
