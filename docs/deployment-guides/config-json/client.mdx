...
Set `disable_content_logging: true` for HIPAA / PCI compliance workloads where message content must not be persisted, and keep `retain_content_in_object_storage: false`: with it on, the full payload of every content-disabled request is still persisted in the object storage bucket. A virtual key with its own `disable_content_logging: false` overrides this for its traffic, so audit your keys as well.
...
For a compliance workload where content must not leave the deployment, set `disable_content_logging: true` **on every configured connector as well**. These flags do not cover every exported value: BigQuery writes `params` and `tools` regardless of its flag, and captured request headers are exported regardless of content flags. If those fields or headers can contain sensitive data, do not export them or apply separate controls. Keep `allow_per_request_content_storage_override` off, and make sure no virtual key sets `disable_content_logging: false`. If `retain_content_in_object_storage` is on, its bucket holds the full payload of every content-disabled request: turn it off when content must not be persisted, or keep the bucket inside the approved deployment boundary when retention is allowed but content must not leave the deployment. See [Content Logging](/features/observability/content-logging) for how the layers compose, and for `retain_content_in_object_storage`, which keeps content in your storage bucket while hiding it from the UI and API.
</Warning>
...
| `delete_expired_virtual_keys` | boolean | `false` | Delete expired virtual keys with a daily cleanup job. A key can override this with its own `delete_after_expire`. See [Virtual key expiry](../../features/governance/virtual-keys#key-expiry) |
