...
1. Set `matview_refresh_interval: "off"` in the PostgreSQL `logs_store` config. Server pods still create/repair the views and run one refresh at boot (so dashboards work immediately), but never refresh again.
