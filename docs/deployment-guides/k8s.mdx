...
<Note>
This example uses the standard Amazon EBS CSI driver (`ebs.csi.aws.com`). Install the EKS add-on before applying it. EKS Auto Mode uses a different provisioner, so use an Auto Mode storage class instead.
</Note>
