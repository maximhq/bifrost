...
1. **Run migrations as a one-shot Job** using the `--migrate-only` flag (or the `BIFROST_MIGRATE_ONLY=1` environment variable). The pod applies all pending migrations and exits.
2. **Start server pods with `--no-migrate`** (or `BIFROST_NO_MIGRATE=1`). They skip migrations entirely and fail fast with a clear error if the schema is not current.
