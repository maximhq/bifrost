...
Use ClickHouse as the log store by setting `storage.logsStore.type` to
`clickhouse`. For a native `ENGINE = Replicated` database, use managed
replication. Bifrost emits `ReplicatedReplacingMergeTree(ver)` without
`ON CLUSTER` and without an explicit Keeper path.
...
storage:
  logsStore:
    ...
    clickhouse:
      ...
      database: bifrost
      managedReplication: true
      tableEngine: REPLICATED_MERGETREE
```

Do not set `cluster` in managed mode. For unmanaged replicated tables,
`cluster` is required as a deployment-safety check, but table DDL must be
applied to each replica by an external migration mechanism because Bifrost does
not send `ON CLUSTER` for replicated engines. `tableEngine` defaults to
`MergeTree`; select `REPLICATED_MERGETREE` explicitly for managed mode. With
`tableEngine: MergeTree`, a
configured `cluster` does send `ON CLUSTER` to create missing tables and
columns on every member.
