...
| `bifrost.plugins.semanticCache.config.direct_key_cache_secret_env` | Environment variable containing the Direct Key namespace HMAC secret | `""` |
...
**Direct / hash mode with Direct Key isolation** (no embedding provider needed):

```bash
kubectl create secret generic bifrost-cache-namespace \
  --from-literal=BIFROST_CACHE_NAMESPACE_SECRET='replace-with-at-least-32-random-bytes'
```
...
envFrom:
  - secretRef:
      name: bifrost-cache-namespace
```

The Secret key must match the environment variable named by `direct_key_cache_secret_env`, or be mapped to it through your preferred pod environment configuration.
