...
This isn't limited to secrets - `network_config.base_url` accepts the same `env.` (and `vault.`) reference form, so you can keep `config.json` committed and diffable while a provider's base_url still varies by environment (dev vs. prod endpoint) instead of templating the file before Bifrost starts:
