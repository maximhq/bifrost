...
| One replica with Railway volume and SQLite | **Preview with permission check** | Railway mounts volumes as root; the Bifrost image runs as UID `1000`. Verify write access before relying on it. |
| One replica with external PostgreSQL | **Preview** | Fits ephemeral replacement when configuration is supplied at startup. |
...
Set the health-check path to `/health`, generate a public domain if external access is needed, and deploy one replica.
...
For a single-instance SQLite deployment, create `config.json` from the [SQLite configuration](/deployment-guides/runtime-contract#sqlite), encode and inject it using Step 3, remove the PostgreSQL variables, and attach a Railway volume at `/app/data`.
