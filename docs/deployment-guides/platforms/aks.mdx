...
| SQLite | One replica and a `ReadWriteOnce` managed disk or another compatible CSI volume |
| PostgreSQL | Any reachable supported PostgreSQL, inside or outside Azure |
...
Use SQLite only for a single Bifrost replica. On AKS, select a compatible ReadWriteOnce StorageClass such as an approved managed-disk CSI class.
...
- Pending SQLite pod: inspect the PVC, CSI driver, StorageClass, and zone/node-pool constraints.
