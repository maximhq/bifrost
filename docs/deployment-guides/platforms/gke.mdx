...
| SQLite | One replica and a `ReadWriteOnce` Persistent Disk or another compatible CSI volume |
| PostgreSQL | Any PostgreSQL reachable from the pod network, regardless of provider |
...
Use SQLite only for a single Bifrost replica. On GKE, select a compatible ReadWriteOnce StorageClass; the default Persistent Disk class is suitable for many single-replica deployments.
...
- Unschedulable SQLite pod: inspect StorageClass, PVC events, zone constraints, and Autopilot policy messages.
