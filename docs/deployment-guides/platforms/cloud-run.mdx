...
| Durable one-instance PostgreSQL | **Preview** | External stores survive replacement; live qualification is still required. |
| SQLite production | **Not supported on the default filesystem** | Cloud Run filesystem writes consume memory and do not survive instance termination. |
...
This path stores both Bifrost stores in PostgreSQL. It mounts `config.json` from Secret Manager outside `/app/data`, copies it into the writable application directory, and starts one Cloud Run instance.

Cloud Run's normal writable filesystem is ephemeral, so it cannot provide durable SQLite storage. Use PostgreSQL for configuration and logs that must survive revision or instance replacement.
...
The service account must already exist and be usable by the deployer. The database can be Cloud SQL or any reachable PostgreSQL service.
