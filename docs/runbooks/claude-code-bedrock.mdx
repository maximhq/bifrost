...
Start Claude Code normally. If you are using Edge, choose the desired model as you normally would. If you configured Claude Code directly, run:

```text
/model
```

With discovery enabled, confirm that the configured `claude-*` deployment names appear under **From gateway**. Select one, then send the following prompt. With Edge, send the same prompt after selecting a model normally:
