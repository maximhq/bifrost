...
<Warning>
These GPT models use Bedrock model IDs. The AWS model cards mark global inference as unsupported, so do not prepend `global.` or `us.` and do not add a `-v1:0` suffix. Configure them under Bifrost's **AWS Bedrock** provider.
</Warning>
