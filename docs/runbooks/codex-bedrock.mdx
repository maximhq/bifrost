...
<Warning>
These GPT models use Bedrock model IDs. The AWS model cards mark global inference as unsupported, so do not prepend `global.` or `us.` and do not add a `-v1:0` suffix in the deployment name. Configure them under Bifrost's **AWS Bedrock** provider, not the classic **AWS Bedrock** provider.
</Warning>

The recommended deployment names are Bifrost aliases, not AWS-defined identifiers. You may choose another name, but the value selected in Codex must match the Bifrost deployment name exactly and should retain the `bedrock/` provider prefix.
...
You only need to add the models that you intend to expose to Codex.
...
See [Aliasing Models](/providers/aliasing-models#static-aliasing) for the complete alias configuration reference and [AWS Bedrock](/providers/supported-providers/bedrock) for provider behavior.
