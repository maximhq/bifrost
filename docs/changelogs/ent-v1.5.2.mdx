...
Release on `transports/v1.6.2`. A focused patch release with two governance and SSO fixes: ghost usage cleanup now runs on targeted governance resets, and Generic OIDC providers prefer the ID token for auth cookie validation.

## 🐞 Fixed

- **Ghost Cleanup on Targeted Governance Resets** - Targeted governance usage resets now trigger ghost cleanup, so stale usage entries left behind by removed entities are purged instead of lingering until the next full reset cycle.
- **Generic OIDC ID Token Preference** - `SCIMProviderGeneric` was added to the `ChooseAuthCookieToken` ID token preference list alongside Google, Okta, Entra, and Auth0. AD FS and similar IdPs stamp access tokens with a different issuer than the OIDC discovery issuer, so the ID token is the only one whose `iss` is guaranteed to match. Keycloak and unrecognised providers continue to fall back to the access token.
