---
title: "v1.4.3"
description: "Enterprise v1.4.3 changelog - 2026-05-21"
---
...
A large release on a clean OSS base of `transports/v1.5.3` (v1.4.2 had pinned an untagged commit). Headline items: **Data Access Control (DAC) goes fleet-wide** so `own-data` / `team-data` / `all-data` scoping now governs API keys, access profiles, roles, guardrails, MCP clients and tool groups, customers, teams, business units, and OAuth tokens, with targeted per-entity cache hydration replacing broad reloads. **Broker-mode clustering** routes all inter-node traffic through a central gRPC relay, enabling deployment on platforms without inbound peer connectivity such as Cloud Run. **Temporary access tokens** and **MCP per-user OAuth** add scoped, time-limited credentials and per-user MCP authorization. The release also brings two new guardrail providers (CrowdStrike AIDR and Patronus AI), team and access-profile calendar alignment, cluster-gossiped feature flags, and a dedicated Dashboard RBAC resource.
...
- **Virtual Keys tab on the Access Profile sheet** : view, assign, detach, and inline-create virtual keys directly from an access profile; AP-template budgets and rate limits are enforced and usage-tracked for attached keys.
- **Direct access-profile assignment** : virtual keys carry an `access_profile_id` for direct access profile assignment.
...
- **Caller context on access profile copy** : creating a virtual key during access profile copy no longer leaks the caller's context.
...
- **UI fixes** : OAuth popup message validation, constrained table column widths, provider API form padding, and image parameter passthrough.
...
github.com/maximhq/bifrost/transports            v1.5.3
