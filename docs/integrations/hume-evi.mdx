...
If the `hume` object is present, `default_model` is required and must contain a non-whitespace value. Restart Bifrost after changing these settings.
...
- OpenAI-compatible request formats use `parallel_tool_calls: false` only when the selected model advertises support for that parameter.
- If Bifrost cannot express or verify a single-call constraint for the provider/model combination, it returns HTTP 400 instead of risking a parallel tool response that Hume cannot process.
...
- Bifrost MCP tools are not injected into Hume streaming requests.
...
| HTTP 400 says exactly one completion is required | The request contains `n` greater than one. | Set `n: 1` or omit it. |
| HTTP 400 says the provider/model cannot guarantee Hume's single-tool-call requirement | Tools are present, but the selected provider/model cannot express or verify serial tool calling. | Choose a supported tool-calling model/provider or remove tools for that configuration. |
