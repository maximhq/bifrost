...
```bash
curl -X POST http://localhost:8080/governance/customers \
...
```bash
curl -X POST http://localhost:8080/governance/teams \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST http://localhost:8080/governance/virtual_keys \
  -H "Content-Type: application/json" \
  -d '{
...
### Cost Management
- **Hierarchical Budgets**: Budgets cascade from Customer → Team → Virtual Key
...
> **💡 New to Bifrost?** Start with the [main documentation](/) to understand Bifrost's core concepts before configuring governance.
