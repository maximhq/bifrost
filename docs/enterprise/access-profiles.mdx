...
- If `vk_rotation_cooldown` is configured (Config → Security), the previous value keeps authenticating until the cooldown expires, so callers have time to pick up the new key.
- Without a cooldown the previous value stops working immediately.
...
- **Automatic key rotation** - An interval (`1h` to `365d`, typically `30d`) and optional first-run time. When due, a background job rotates every managed key, honouring `vk_rotation_cooldown`, and posts start/finish notifications. Off by default. See [Automatic virtual key rotation](#automatic-virtual-key-rotation).
