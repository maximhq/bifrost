...
Bifrost does not currently support Okta SCIM management APIs or inbound Okta SCIM provisioning. Do not create or configure an Okta SCIM app for Bifrost yet.

For Okta, Bifrost uses the API token from Step 7 to sync users in the background every **24 hours**. During that sync, Bifrost reconciles imported users, role mappings, team mappings, and business-unit mappings from Okta.
...
Support for Okta SCIM management APIs is coming soon.
