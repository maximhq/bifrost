...
Guardrail rule management is enterprise-only and now supports search and pagination in both the UI and the management API. Use the `search`, `limit`, and `offset` query parameters on `GET /api/guardrails/rules` to browse large rule catalogs without fetching every rule at once.

```bash
curl "http://localhost:8080/api/guardrails/rules?search=pii&limit=25&offset=0"
```
...
The wrapped response shape is returned consistently, including when you omit pagination parameters, so enterprise UI clients and automation can rely on `count`, `total_count`, `limit`, and `offset`.
