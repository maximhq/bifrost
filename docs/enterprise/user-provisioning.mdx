...
`trusted_networks` is the private IP/CIDR allowlist the SSRF guard consults before outbound discovery calls, so a self-hosted IdP on an internal network is reachable - see [Generic OIDC](/enterprise/setting-up-generic-oidc/oidc) for when you need it. Entries are `{ cidr, description }`; a bare IP is a single host and hostnames are rejected. Declaring the key makes `config.json` own the whole list (an explicit `[]` clears ranges added from the dashboard); omit it to leave them untouched.
