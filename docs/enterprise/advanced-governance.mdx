...
- **User-Level Governance** - Individual user authentication plus per-user budgets and rate limits
...
- **Personal Rate Limits** - Token and request ceilings per individual user
...
User-level governance is an enterprise-only management surface. In the enterprise dashboard, administrators can open the **Users** view, search for a user, and assign or edit limits directly from the **Set Limits** action.

The same workflow is available over the management API:

```bash
curl -X POST http://localhost:8080/api/governance/users \
...
```bash
curl -X PUT http://localhost:8080/api/governance/users/user-alice-001 \
  -H "Content-Type: application/json" \
  -d '{
    "budget": {
      "max_limit": 400,
...
Use `DELETE /api/governance/users/{user_id}` to remove user-specific limits and fall back to the surrounding team, customer, and virtual key policies.

### How User Governance Composes with Other Controls

User-level governance does not replace the rest of Bifrost governance. It adds another enforcement layer:

- Identity provider authentication establishes the enterprise user context.
- User budgets and rate limits are evaluated for that principal.
- Virtual key policies still apply, including provider/model restrictions and allowed API key routing.
- Team and customer budgets remain useful for higher-level rollups and shared spending controls.
...
In enterprise deployments, this means a single request can be governed by both the authenticated user and the virtual key it carries. The user-level policy decides whether the caller still has budget or rate-limit headroom, while the virtual key continues to constrain the providers, models, API keys, and MCP tools available to that request.
