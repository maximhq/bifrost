...
The cooldown prevents alert storms by suppressing repeat notifications after a rule fires. The cooldown window is measured from the last `sent` history record for the same rule, scope, and target.
