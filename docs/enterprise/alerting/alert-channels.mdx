...
- **HTTPS required.** Slack, Microsoft Teams, and PagerDuty always require HTTPS. Webhook channels may use HTTP only when `webhook_network.allow_http` is `true`.
- **SSRF protection.** Destination hosts that resolve to private, loopback, or link-local addresses are blocked by default. This includes `localhost`, the AWS instance metadata endpoint (`169.254.169.254`), and `metadata.google.internal`. Set `webhook_network.allow_private_network` to `true` to disable this check.
...
<Warning>
Enabling `allow_http` or `allow_private_network` weakens security protections. Only enable these for trusted internal or air-gapped networks.
</Warning>
