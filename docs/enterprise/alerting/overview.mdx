...
<Steps>
  ...
  <Step title="Cooldowns are applied">
    When a rule matches, its cooldown suppresses repeat notifications for the same rule, scope, and target. The cooldown window is measured from the last `sent` history record. Channels can define their own optional cooldown on top of the rule cooldown.
  </Step>
...
- **Rule cooldown**: Suppresses repeat notifications for the same rule, scope, and target until the cooldown window elapses. Default is 60 seconds. The cooldown window is measured from the last `sent` history record.
...
Cooldowns are enforced by checking [alert history](/enterprise/alerting/alert-history) for recent `sent` status rows within the cooldown window.

---

## Clustering behavior

When Bifrost runs as a [cluster](/enterprise/clustering), only the leader node evaluates rules and dispatches notifications. The leader check happens before each evaluation sweep. When a node loses leadership, its alerting engine stops until leadership is regained.
...
| --- | --- | --- | --- |
| `webhook_network.allow_http` | boolean | `false` | When `true`, permits `http://` webhook URLs for channel types that support it. |
| `webhook_network.allow_private_network` | boolean | `false` | When `true`, permits webhook destinations on private, loopback, or link-local addresses. |
