...
import MaximView from "./maximView";
import NewrelicView from "./newRelicView";
import OtelView from "./otelView";
...
export default function ObservabilityView() {
	const [activeTab, setActiveTab] = useState<string | undefined>(supportedPlatforms[0].id);
	return (
		<div className="flex h-full flex-row gap-4">
			...
			<div className="w-full pt-4">
				{activeTab === "otel" && <OtelView />}
				{activeTab === "maxim" && <MaximView />}
				{activeTab === "datadog" && <DatadogView />}
				{activeTab === "newrelic" && <NewrelicView />}
			</div>
		</div>
	);
}
