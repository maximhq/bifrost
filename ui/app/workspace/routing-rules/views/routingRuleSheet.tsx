...
export function RoutingRuleSheet({
	...
	// Get available providers and models from existing rules
	...
	useEffect(() => {
		if (editingRule) {
			...
			// Restore the query from stored object or derive from CEL expression so the builder is pre-populated
			const initialQuery =
				editingRule.query ??
				(editingRule.cel_expression?.trim()
					? convertCELToRuleGroup(editingRule.cel_expression)
					: undefined) ??
				defaultQuery;
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="dark:bg-card flex w-full flex-col min-w-1/2 gap-4 overflow-x-hidden bg-white p-8">
				...
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
					...
					<div className="space-y-3">
						...
						<div className="grid grid-cols-2 gap-4">
							...
							<div className="space-y-2">
								...
								<Input
									id="model"
									placeholder="e.g., gpt-4-turbo (optional)"
									{...register("model")}
								/>
					...
					{/* Fallbacks */}
					<div className="space-y-3">
						<Label htmlFor="fallbacks">Fallbacks</Label>
						<Textarea
							id="fallbacks"
						...
						/>
						<p className="text-muted-foreground text-xs">One fallback per line (format: provider/model)</p>
					</div>
