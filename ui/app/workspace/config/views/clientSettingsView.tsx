...
export default function ClientSettingsView() {
	...
	const handleAllowlistChange = useCallback((index: number, value: string) => {
		setLocalConfig((prev) => ({
			...
			header_filter_config: {
				...prev.header_filter_config,
				allowlist: (prev.header_filter_config?.allowlist || []).map((h, i) => (i === index ? value.toLowerCase() : h)),
			},
		}));
	}, []);
	...
	const handleDenylistChange = useCallback((index: number, value: string) => {
		setLocalConfig((prev) => ({
			...
			header_filter_config: {
				...prev.header_filter_config,
				denylist: (prev.header_filter_config?.denylist || []).map((h, i) => (i === index ? value.toLowerCase() : h)),
			},
		}));
	}, []);
			...
			<div className="space-y-4">
				...
				<div className="flex items-center justify-between space-x-2">
					<div className="space-y-0.5">
						...
						<p className="text-muted-foreground text-sm">Enable litellm-specific fallbacks for text completion for Groq.</p>
					</div>
					<Switch
			...
			<div className="space-y-4">
				...
				<Accordion type="multiple" className="w-full rounded-sm border px-4">
					...
					<AccordionItem value="security-note">
						...
						<AccordionContent>
							<p className="text-sm">Some headers are always blocked for security reasons regardless of configuration:</p>
