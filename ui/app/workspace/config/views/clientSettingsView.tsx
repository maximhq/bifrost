...
export default function ClientSettingsView() {
	...
	const hasCoreConfigChanges = useMemo(() => {
		...
		return (
			...
			localConfig.compat?.convert_text_to_chat !== config.compat?.convert_text_to_chat ||
			localConfig.compat?.convert_chat_to_responses !== config.compat?.convert_chat_to_responses ||
			localConfig.compat?.should_drop_params !== config.compat?.should_drop_params ||
