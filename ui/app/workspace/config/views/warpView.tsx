...
export default function WarpView() {
	...
	// still says otherwise. Comparing timestamps rather than the raw strings
	// tolerates the picker's minute precision vs. the server's stored
	// precision.
	const canResumeBackfill =
		...
		!!backfillStart &&
		!!backfillEnd &&
		new Date(backfillStart).getTime() === new Date(backfillStatus.start_time).getTime() &&
		new Date(backfillEnd).getTime() === new Date(backfillStatus.end_time).getTime();
	...
	// rows. Comparing timestamps rather than the raw strings tolerates the
	// picker's minute precision vs. the server's stored precision.
									...
									<>
										{canResumeBackfill && (
											<Button
												...
												disabled={isStartingBackfill || !hasSettingsUpdateAccess || hasChanges}
