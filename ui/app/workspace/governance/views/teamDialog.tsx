...
export default function TeamDialog({ team, customers, onSave, onCancel }: TeamDialogProps) {
	...
	const validator = useMemo(() => {
		...
		const budgetValidators = formData.budgets.flatMap((row, idx) => {
			...
			return [
				Validator.minValue(row.maxLimit, 0.01, t("workspace.governance.teams.dialog.budgetMaxLimit", { index: idx + 1 })),
				Validator.required(row.resetDuration, t("workspace.governance.teams.dialog.budgetResetRequired", { index: idx + 1 })),
			];
		});
		...
		return new Validator([
			Validator.required(formData.name.trim(), t("workspace.governance.teams.dialog.teamNameRequired")),
			Validator.custom(formData.isDirty, t("workspace.governance.teams.dialog.noChanges")),
			...budgetValidators,
			Validator.custom(uniqueDurations === populatedDurations.length, t("workspace.governance.teams.dialog.distinctBudgetDurations")),

			// Rate limit validation - token limits
			...(formData.tokenMaxLimit !== undefined && formData.tokenMaxLimit !== null
				? [
						Validator.minValue(tokenMaxLimitNum || 0, 1, t("workspace.governance.teams.dialog.tokenMaxLimit")),
						Validator.required(formData.tokenResetDuration, t("workspace.governance.teams.dialog.tokenResetRequired")),
					]
				: []),
			...
			...(formData.requestMaxLimit !== undefined && formData.requestMaxLimit !== null
				? [
						Validator.minValue(requestMaxLimitNum || 0, 1, t("workspace.governance.teams.dialog.requestMaxLimit")),
						Validator.required(formData.requestResetDuration, t("workspace.governance.teams.dialog.requestResetRequired")),
					]
				: []),
		]);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			if (isEditing && team) {
				...
				toast.success(t("workspace.config.updatedSuccess", { feature: t("workspace.governance.teams.title") }));
			} else {
				// Create new team
			...
			<DialogContent className="sm:max-w-2xl" data-testid="team-dialog-content">
				<DialogHeader>
					<DialogTitle className="flex items-center gap-2">
						{isEditing ? t("workspace.governance.teams.dialog.editTitle") : t("workspace.governance.teams.dialog.createTitle")}
					</DialogTitle>
					<DialogDescription>
						{isEditing ? t("workspace.governance.teams.dialog.editDescription") : t("workspace.governance.teams.dialog.createDescription")}
					</DialogDescription>
				</DialogHeader>
				...
				<form onSubmit={handleSubmit} className="">
					<div className="space-y-6">
						...
						<div className="">
							<div className="space-y-2">
								<Label htmlFor="name">{t("workspace.governance.teams.dialog.teamName")}</Label>
								<Input
									id="name"
									placeholder={t("workspace.governance.teams.dialog.teamNamePlaceholder")}
									value={formData.name}
									maxLength={50}
							...
							{customers?.length > 0 && (
								<div className="space-y-2">
									<Label htmlFor="customer">{t("workspace.governance.teams.dialog.customerOptional")}</Label>
									<Select
									...
									>
										<SelectTrigger id="customer" className="w-full" data-testid="team-customer-select-trigger">
											<SelectValue placeholder={t("workspace.governance.teams.dialog.selectCustomer")} />
										</SelectTrigger>
										<SelectContent>
											<SelectItem value="__none__" data-testid="team-customer-option-none">
												{t("workspace.governance.teams.dialog.none")}
											</SelectItem>
									...
									<p className="text-muted-foreground text-sm">{t("workspace.governance.teams.dialog.assignCustomerHint")}</p>
								</div>
							)}
						</div>
						...
						<div className="space-y-3">
							<div className="flex items-center justify-between">
								<Label>{t("workspace.governance.teams.dialog.budgets")}</Label>
								<button
									type="button"
							...
							{formData.budgets.length === 0 && (
								<p className="text-muted-foreground text-xs">{t("workspace.governance.teams.dialog.noBudgets")}</p>
							)}
									...
									<div className="flex items-start gap-2">
										<div className="flex-1">
											<NumberAndSelect
												...
												label={t("workspace.governance.teams.dialog.budgetMaxSpend", { index: idx + 1 })}
												value={row.maxLimit}
										...
										>
											{t("workspace.governance.teams.dialog.remove")}
										</button>
									</div>
										...
										<div className="flex items-center justify-between gap-4 rounded-md border px-3 py-2">
											<div className="space-y-0.5">
												<Label htmlFor={`team-budget-calendar-aligned-toggle-${idx}`} className="text-sm font-normal">
													{t("workspace.governance.teams.dialog.alignCalendar")}
												</Label>
												<p className="text-muted-foreground text-xs">{t("workspace.governance.teams.dialog.alignCalendarDescription")}</p>
											</div>
											<Switch
						...
						>
							<AlertDialogContent>
								<AlertDialogHeader>
									<AlertDialogTitle>{t("workspace.governance.teams.dialog.resetBudgetUsageTitle")}</AlertDialogTitle>
									<AlertDialogDescription>
										{t("workspace.governance.teams.dialog.resetBudgetUsageDescription", {
											period:
												pendingCalendarAlignIdx !== null && formData.budgets[pendingCalendarAlignIdx]?.resetDuration === "1d"
													? t("workspace.governance.teams.dialog.periodDay")
													: pendingCalendarAlignIdx !== null && formData.budgets[pendingCalendarAlignIdx]?.resetDuration === "1w"
														? t("workspace.governance.teams.dialog.periodWeek")
														: pendingCalendarAlignIdx !== null && formData.budgets[pendingCalendarAlignIdx]?.resetDuration === "1M"
															? t("workspace.governance.teams.dialog.periodMonth")
															: pendingCalendarAlignIdx !== null && formData.budgets[pendingCalendarAlignIdx]?.resetDuration === "1Y"
																? t("workspace.governance.teams.dialog.periodYear")
																: t("workspace.governance.teams.dialog.periodDefault"),
										})}
									</AlertDialogDescription>
								</AlertDialogHeader>
								<AlertDialogFooter>
									...
									>
										{t("workspace.governance.teams.dialog.enableCalendarAlignment")}
									</AlertDialogAction>
								</AlertDialogFooter>
							</AlertDialogContent>
						</AlertDialog>
						...
						{isEditing && ((team?.budgets && team.budgets.length > 0) || team?.rate_limit) && (
							<div className="bg-muted/50 space-y-4 rounded-lg border p-4">
								<p className="text-sm font-medium">{t("workspace.governance.teams.dialog.currentUsage")}</p>
								<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
									{team?.budgets?.map((b) => (
										<div key={b.id} className="space-y-1">
											<p className="text-muted-foreground text-xs">
												{t("workspace.governance.teams.dialog.budgetUsage", { duration: b.reset_duration })}
											</p>
											...
											<p className="text-muted-foreground text-xs">
												{t("workspace.governance.teams.dialog.lastReset")}{" "}
									...
									{team?.rate_limit?.token_max_limit && (
										<div className="space-y-1">
											<p className="text-muted-foreground text-xs">{t("workspace.governance.teams.dialog.tokens")}</p>
											<div className="flex items-center gap-2">
												<span className="font-mono text-sm">
									...
									{team?.rate_limit?.request_max_limit && (
										<div className="space-y-1">
											<p className="text-muted-foreground text-xs">{t("workspace.governance.teams.dialog.requests")}</p>
											<div className="flex items-center gap-2">
												<span className="font-mono text-sm">
					...
					<FormFooter
						validator={validator}
						label={t("workspace.governance.teams.dialog.teamLabel")}
						onCancel={onCancel}
						isLoading={loading}
						isEditing={isEditing}
