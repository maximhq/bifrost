...
const FILE_SOURCE_OPTIONS = [
  ...
  { value: "filepath", label: "Via file path", shortLabel: "File path" },
...
function FileAddForm({
  ...
  const [filepath, setFilepath] = useState(initialEntry?.filepath ?? "");
  ...
  const handleSubmit = async () => {
    ...
    if (sourceType === "filepath") entry.filepath = filepath;
  ...
  return (
    ...
    >
      ...
      {(sourceType === "url" || sourceType === "filepath") && (
        <div className="flex items-start gap-2 rounded-sm border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900 dark:border-blue-900/50 dark:bg-blue-950/30 dark:text-blue-200">
          ...
          <span>
            ...
            this {sourceType === "url" ? "URL" : "absolute path"} when the skill
            file is retrieved.
          </span>
        </div>
      )}
