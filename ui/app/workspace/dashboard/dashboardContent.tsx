...
export default function DashboardPage() {
	...
	// Compute fresh default timestamps on each mount (avoids stale module-level values in long SPA sessions)
	const [queryParsers] = useState(() => {
		const now = Math.floor(Date.now() / 1000);
		return {
			start_time: parseAsInteger.withDefault(now - 24 * 3600),
			end_time: parseAsInteger.withDefault(now),
	...
	const [urlState, setUrlState] = useQueryStates(queryParsers, {
		history: "push",
		shallow: false,
	});
	...
	const fetchAllData = useCallback(async () => {
		...
		} else if (histogramResult.error) {
			toast.error("Failed to load request volume data");
		}
