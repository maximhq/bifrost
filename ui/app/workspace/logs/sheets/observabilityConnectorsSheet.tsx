...
export function ObservabilityConnectorsSheet({ open, onOpenChange }: ObservabilityConnectorsSheetProps) {
  ...
  return (
		<Sheet open={open} onOpenChange={handleOpenChange}>
			<SheetContent side="right" className="dark:bg-card flex w-full flex-col overflow-x-hidden bg-white px-8 sm:max-w-3xl">
				<SheetTitle className="sr-only">{view === "connectors" ? "Connectors" : "Observability settings"}</SheetTitle>
				<div className="custom-scrollbar min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-1 pt-8">
					{view === "connectors" ? (
						<>
						...
						<>
							<div className="mb-0 flex items-center gap-2">
								<Button variant="ghost" size="icon" className="-ml-2 h-8 w-8" onClick={showConnectors} aria-label="Back to connectors" data-testid="observability-connectors-back-btn">
