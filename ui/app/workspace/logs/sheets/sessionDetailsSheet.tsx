	...
	useEffect(() => {
		...
		const unsubscribe = subscribe("log", (data) => {
			...
			if (operation === "create") {
				...
				setHasMore((prev) => prev || fetchedCount < totalCount + 1);
			}
		});
		return unsubscribe;
	}, [open, sessionId, liveEnabled, subscribe, fetchedCount, totalCount, sortSessionLogs]);
			...
			<SheetContent className="flex w-full flex-col gap-4 overflow-x-hidden p-8 sm:max-w-[60%]">
				...
				{hasMore ? (
					<div className="flex justify-center">
						<Button variant="outline" onClick={() => loadSessionPage(fetchedCount)} disabled={loadingSession}>
