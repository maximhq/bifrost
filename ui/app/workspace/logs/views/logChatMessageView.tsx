		...
		<div className={cn("bg-muted/30 rounded border p-3 text-xs", className)}>
			<div className="flex items-start justify-between gap-3">
				...
				{canDownload && (
					...
					>
						<Download className="h-3.5 w-3.5" />下载</Button>
				)}
			</div>
	...
	return (
		<div className="flex w-full flex-col gap-2">
			...
			{message.reasoning && (
				<>
					{isJson(message.reasoning) ? (
						<CollapsibleBox title="推理" onCopy={() => JSON.stringify(cleanJson(message.reasoning), null, 2)} collapsedHeight={100}>
							<CodeEditor
								className="z-0 w-full"
								shouldAdjustInitialHeight={true}
								maxHeight={200}
						...
						<CollapsibleBox title="推理" onCopy={() => message.reasoning || ""} collapsedHeight={100}>
			...
			{message.refusal && (
				<>
					{isJson(message.refusal) ? (
						<CollapsibleBox title="拒绝" onCopy={() => JSON.stringify(cleanJson(message.refusal), null, 2)} collapsedHeight={100}>
							<CodeEditor
								className="z-0 w-full"
								shouldAdjustInitialHeight={true}
								maxHeight={150}
						...
						<CollapsibleBox title="拒绝" onCopy={() => message.refusal || ""} collapsedHeight={100}>
					...
					{typeof message.content === "string" ? (
						<>
							{isJson(message.content) ? (
								...
								<CollapsibleBox title="内容" onCopy={() => (message.content as string) || ""} collapsedHeight={100}>
									<div className="custom-scrollbar max-h-[400px] overflow-y-auto px-6 py-2 font-mono text-xs break-words whitespace-pre-wrap">
										{message.content}
									</div>
								</CollapsibleBox>
			...
			{message.annotations && message.annotations.length > 0 && (
				<CollapsibleBox title="注释" onCopy={() => JSON.stringify(message.annotations, null, 2)} collapsedHeight={100}>
					<CodeEditor
						className="z-0 w-full"
						shouldAdjustInitialHeight={true}
						maxHeight={400}
			...
			{/* Handle audio output */}
			{message.audio && (
				<CollapsibleBox title="音频输出" collapsedHeight={150}>
					<div className="space-y-4 px-6 py-4">
						{message.audio.transcript && (
							<div className="space-y-2">
								<div className="text-muted-foreground text-xs font-medium">转录文本：</div>
								<div className="font-mono text-xs break-words whitespace-pre-wrap">{message.audio.transcript}</div>
							</div>
						)}
						{message.audio.data && (
							<div className="space-y-2">
								<div className="text-muted-foreground text-xs font-medium">音频：</div>
