			...
			{!credential ? (
				...
				<DefinitionList>
					<Row label="Status">
						...
						{credential.status === "needs_reauth" && providerRejectedTheClient(credential.status_reason) && (
							<Hint>
								The provider rejected Bifrost&apos;s client itself, not just this token, so redoing consent with it will fail the
								same way. Use Reauthorize with a new client from the server&apos;s actions menu to register a replacement first.
							</Hint>
						)}
