			...
			<SheetContent className="flex w-full flex-col overflow-x-hidden p-8 sm:max-w-2xl">
				<div className="flex w-full flex-col gap-6">
					<div className="flex items-center gap-2 font-medium">
						<p className="text-md max-w-full truncate">Request ID: {log.id}</p>
				...
				{log.input_history && log.input_history.map((message) => <LogMessageView message={message} />)}

				<div className="mt-4 w-full text-center text-sm font-medium">Response</div>
				{log.status === "success" ? (
					log.output_message && <LogMessageView message={log.output_message} />
				) : (
					<div className="w-full rounded-sm border">
