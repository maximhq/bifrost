...
export function NewMessageInputView() {
	...
	const handleRun = useCallback(async () => {
		if (isStreaming) return;
		// If there's pending input, add it first then run
		const input = userInput.trim();
		const currentAttachments = attachments.length > 0 ? [...attachments] : undefined;
		...
		await onSendMessage(input, currentAttachments);
		// Focus the input after a short delay to ensure the input is updated
		setTimeout(() => {
			userInputRef.current?.focus();
		}, 100);
	}, [userInput, attachments, isStreaming, onSendMessage]);
	...
	return (
		<div
			className="group relative max-h-[500px] overflow-y-auto border-t px-4 py-2 shrink-0"
			{...(supportsVision
				? {
		...
		>
			...
			<div className="mb-1 flex items-center">
				<MessageRoleSwitcher role={inputRole} disabled={isStreaming} onRoleChange={setInputRole} />
				{supportsVision && (
					<div className="ml-auto">
						...
						/>
						<Paperclip
							className="text-muted-foreground hover:text-foreground h-3.5 w-3.5 shrink-0 cursor-pointer"
			...
			<div className="relative">
				<Textarea
					...
					onPaste={handlePaste}
					className="text-muted-foreground min-h-[60px] dark:bg-transparent resize-none rounded-none border-0 bg-transparent p-0 pr-16 text-sm shadow-none focus-visible:ring-0 focus-visible:ring-offset-0"
