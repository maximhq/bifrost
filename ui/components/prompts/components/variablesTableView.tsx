...
export function VariablesTableView({
	...
	return (
		<div className="flex flex-col gap-3">
			<Label className="text-muted-foreground text-xs font-medium uppercase">{t("promptRepo.variables")}</Label>
			<p className="text-muted-foreground text-xs">
				<Trans
					t={t}
					i18nKey="promptRepo.variablesHelp"
					values={{ syntax: "{{ }}" }}
					components={{ code0: <code className="bg-muted rounded px-1" /> }}
				/>
			</p>
				...
				<table className="w-full table-fixed text-sm">
					<thead>
						<tr className="bg-muted/50 border-border border-b">
							<th className="text-muted-foreground w-[40%] max-w-[40%] px-3 py-1.5 text-left text-xs font-medium">{t("promptRepo.variable")}</th>
							<th className="text-muted-foreground px-3 py-1.5 text-left text-xs font-medium">{t("promptRepo.value")}</th>
						</tr>
					</thead>
					<tbody>
