...
export function UserMessageView({
	...
	const addAttachments = useCallback(
		(newAttachments: MessageContent[]) => {
			const clone = message.clone();
			clone.attachments = [...message.attachments, ...newAttachments];
			onChange(clone.serialized);
		},
		[message, onChange],
	);
	...
	return (
		<div
			className="group relative hover:border-border rounded-lg border border-transparent px-3 py-2 transition-colors"
			ref={containerRef}
			{...(canAttach
				? {
			...
			<div>
				{editMode ? (
					<Textarea
						autoFocus
						value={content}
						className="text-muted-foreground min-h-[20px] resize-none rounded-none border-0 bg-transparent p-0 text-sm shadow-none focus-visible:ring-0 focus-visible:ring-offset-0"
						disabled={disabled}
						onChange={(e) => {
							const clone = message.clone();
