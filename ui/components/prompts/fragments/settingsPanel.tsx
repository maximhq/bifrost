...
import { useGetAllKeysQuery, useGetModelDatasheetQuery, useGetProvidersQuery, useLazyGetModelsQuery } from '@/lib/store/apis/providersApi'
...
export function SettingsPanel() {
  ...
  // Apply default values from model datasheet when model changes
  const { data: datasheetData } = useGetModelDatasheetQuery(model, { skip: !model })
  const appliedDefaultsForModel = useRef<string>('')

  useEffect(() => {
    if (!datasheetData?.model_parameters || !model || appliedDefaultsForModel.current === model) return
    appliedDefaultsForModel.current = model

    const defaults: Record<string, any> = {}
    for (const param of datasheetData.model_parameters) {
      if (param.default !== undefined && !(param.id in modelParams)) {
        defaults[param.id] = param.default
      }
    }
    if (Object.keys(defaults).length > 0) {
      onModelParamsChange({ ...modelParams, ...defaults })
    }
  }, [datasheetData, model, modelParams, onModelParamsChange])
