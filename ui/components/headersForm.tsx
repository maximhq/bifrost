...
export default function HeadersForm({
	...
	const allFilled = useMemo(() => requiredKeys.every((k) => (values[k] ?? "").trim() !== ""), [requiredKeys, values]);
	...
	const handleTest = async () => {
		if (!onTest || !allFilled || busy) return;
		await onTest(values);
	};
		...
		<form onSubmit={handleSubmit} className="space-y-4">
			{adminHeaderKeys && adminHeaderKeys.length > 0 && (
				<div className="rounded-md border border-muted-foreground/20 bg-muted/40 p-3">
					<div className="flex items-center gap-1.5">
						<p className="text-xs font-medium text-muted-foreground">Static admin headers</p>
						<TooltipProvider>
							<Tooltip>
								...
								<TooltipContent className="max-w-xs">
									<p>These headers are set by the admin on the MCP client and accompany every request alongside your submitted values. You can&apos;t edit them here.</p>
								</TooltipContent>
							</Tooltip>
						</TooltipProvider>
					</div>
					<div className="mt-2 flex flex-wrap gap-1.5">
						{adminHeaderKeys.map((name) => (
							<code key={name} className="rounded bg-background px-1.5 py-0.5 text-xs font-mono">
								{name}
							</code>
						))}
					</div>
			...
			<div className="space-y-3">
				{requiredKeys.length === 0 ? (
					<p className="text-sm text-muted-foreground">No header keys have been declared on this MCP client.</p>
				) : (
						...
						return (
							<div key={key} className="space-y-1.5">
								<div className="flex items-center justify-between gap-2">
									...
									{wasSubmitted && (
										<span className="text-xs text-muted-foreground">Previously submitted</span>
									)}
								</div>
			...
			<div className="flex items-center justify-end gap-2 pt-2">
				...
				{!hideSubmit && (
					<Button
						type="submit"
						disabled={!allFilled || busy || requiredKeys.length === 0}
