...
const NumberInput = React.forwardRef<HTMLInputElement, NumberInputProps>(
	(
		...
		const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => {
			props.onBlur?.(e);
			if (!internalValue) {
				if (props.preventOnBlurFallback) {
					onChange?.(undefined);
				} else {
					if (!errorMessage) setErrorMessage(`Value cannot be empty, replaced with ${fallbackValue}`);
		...
		return (
			<>
				<TooltipProvider delayDuration={100} disableHoverableContent>
					<Tooltip open={!!errorMessage && !props.hideWarnings}>
