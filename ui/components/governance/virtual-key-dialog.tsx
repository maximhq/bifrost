...
const createInitialState = (virtualKey?: VirtualKey | null): Omit<VirtualKeyFormData, 'isDirty'> => {
  return {
    ...
    // Budget
    budgetMaxLimit: virtualKey?.budget ? virtualKey.budget.max_limit / 100 : undefined, // Convert cents to dollars
    ...
    try {
      if (isEditing && virtualKey) {
        ...
        if (formData.budgetMaxLimit) {
          updateData.budget = {
            max_limit: Math.round(formData.budgetMaxLimit * 100), // Convert dollars to cents
