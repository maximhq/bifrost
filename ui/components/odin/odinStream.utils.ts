...
export type OdinEventType = "start" | "delta" | "tool_call_start" | "tool_call_end" | "error" | "done";
 ...
 */
export function odinToolLabel(name: string): string {
...
export function odinErrorMessage(code: string | undefined, message: string | undefined): string {
	switch (code) {
		case "not_configured":
			return "Odin is not configured yet.";
		case "max_iterations":
			return "Odin could not settle on an answer. Try a narrower question.";
		case "timeout":
