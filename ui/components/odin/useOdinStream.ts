import { parseOdinFrame, splitOdinFrames, type OdinEvent, type OdinQuestion, type OdinUsage } from "@/components/odin/odinStream.utils";
import type { OdinTurn, OdinTurnToolCall } from "@/lib/contexts/odinContext";
import { getApiBaseUrl } from "@/lib/utils/port";
import { useCallback, useRef, useState } from "react";

interface UseOdinStreamOptions {
	/** Called once the turn is complete, so the finished turn can join the conversation. */
	onTurnComplete: (turn: OdinTurn) => void;
}

interface UseOdinStreamResult {
	/** Text streamed so far for the in-flight answer. */
	streamingText: string;
	/** Tool calls made during the in-flight answer, in order. */
	streamingToolCalls: OdinTurnToolCall[];
	isStreaming: boolean;
	/** Terminal error for the in-flight turn, if it failed. */
	error: string | null;
	/** Set when Odin ended its turn by asking something. */
	question: OdinQuestion | null;
	clearQuestion: () => void;
	send: (history: OdinTurn[], question: string) => Promise<void>;
	stop: () => void;
...
/**
 * Drives one Odin request and exposes the in-flight answer.
 *
 * The streaming text lives here rather than in OdinProvider on purpose: a
 ...
 */
export function useOdinStream({ onTurnComplete }: UseOdinStreamOptions): UseOdinStreamResult {
	const [streamingText, setStreamingText] = useState("");
	const [streamingToolCalls, setStreamingToolCalls] = useState<OdinTurnToolCall[]>([]);
	const [isStreaming, setIsStreaming] = useState(false);
	const [error, setError] = useState<string | null>(null);
	const [question, setQuestion] = useState<OdinQuestion | null>(null);
	...
	const send = useCallback(
		async (history: OdinTurn[], question: string) => {
			stop();
			...
			let text = "";
			let toolCalls: OdinTurnToolCall[] = [];
			let terminalError: string | null = null;
			let posed: OdinQuestion | null = null;
			let usage: OdinUsage | undefined;

			const applyEvent = (event: OdinEvent) => {
				switch (event.type) {
					case "delta":
						text += event.delta ?? "";
			...
			try {
				const response = await fetch(`${getApiBaseUrl()}/odin/chat`, {
					method: "POST",
					credentials: "include",
				...
				if (!response.ok) {
					...
					throw new Error(reason ? `${reason}:` : `Odin request failed (${response.status})`);
				}

				const reader = response.body?.getReader();
				if (!reader) throw new Error("Odin returned no response body");
				...
				for (;;) {
					...
					const { frames, rest } = splitOdinFrames(buffer);
					buffer = rest;
					for (const frame of frames) {
						const event = parseOdinFrame(frame);
				...
				if (!(caught instanceof DOMException && caught.name === "AbortError")) {
					terminalError = caught instanceof Error ? caught.message : "Odin request failed";
				}
			} finally {
