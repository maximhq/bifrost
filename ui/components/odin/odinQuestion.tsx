...
import type { OdinQuestion } from "@/components/odin/odinStream.utils";
import { cn } from "@/lib/utils";
import { MessageCircleQuestion } from "lucide-react";
import { useEffect, useState } from "react";

interface OdinQuestionCardProps {
	question: OdinQuestion;
	/** Sends the chosen answer. `label` is what was read, `answer` is what Odin receives. */
	onAnswer: (answer: string, label?: string) => void;
	/** Declines to answer, letting Odin proceed with its own judgement. */
	onSkip: () => void;
}
...
/**
 * A structured question from Odin, rendered as a picker.
 *
 ...
 * both faster and gives Odin an answer it can use directly instead of one it has
 * to re-interpret.
 *
 * Skip is always available. A question you cannot decline is a wall, and Odin
 * can proceed on its own judgement as long as it says what it assumed.
 */
export default function OdinQuestionCard({ question, onAnswer, onSkip }: OdinQuestionCardProps) {
	...
	return (
		...
		<div className="bg-background rounded-md" data-testid="odin-question">
			<div className="border-primary/40 bg-primary/5 space-y-3 rounded-md border p-3">
				...
				<div className="space-y-1">
					{question.options.map((option, index) => (
						<button
							...
							data-testid={`odin-question-option-${index}`}
				...
				<div className="flex items-center justify-end gap-2">
					<Button type="button" variant="ghost" size="sm" onClick={onSkip} data-testid="odin-question-skip" className="font-normal">
