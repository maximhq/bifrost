...
import { matchOdinCommands, resolveOdinCommand, type OdinCommand } from "@/components/odin/odinCommands";
...
interface OdinComposerProps {
	...
	onCommand?: (command: OdinCommand) => void;
	isStreaming: boolean;
	disabled?: boolean;
	/**
 ...
 */
export default function OdinComposer({ isStreaming, disabled, attached, provider, model, onCommand, onSend, onStop }: OdinComposerProps) {
	...
	const commands = onCommand ? matchOdinCommands(value) : [];
	const menuOpen = commands.length > 0;

	const runCommand = (command: OdinCommand) => {
		onCommand?.(command);
		setValue("");
		setHighlighted(0);
	};
	...
	const submit = () => {
		...
		const command = resolveOdinCommand(text);
		if (command) {
			runCommand(command);
			return;
		}
	...
	return (
		<div className={cn("shrink-0 px-3 pb-3", attached ? "pt-0" : "pt-3")}>
			{menuOpen && (
				<div className="bg-popover mb-2 overflow-hidden rounded-md border shadow-sm" data-testid="odin-command-menu">
					{commands.map((command, index) => (
						<button
							...
							data-testid={`odin-command-${command.id}`}
							className={cn(
			...
			<div className="focus-within:border-ring bg-background flex flex-col gap-2 rounded-lg border p-2 transition-colors">
				<TextareaAutosize
					...
					minRows={1}
					maxRows={8}
					data-testid="odin-composer-input"
				...
				<div className="flex items-center justify-between gap-2">
					{/* Which model is answering. Odin runs on a model chosen separately
					...
					<Link
						...
						data-testid="odin-composer-model"
					>
