...
function convertRuleToCEL(rule: RuleType): string {
  ...
  if (isKeyValueField) {
    ...
    if (keyValuePair && keyValuePair.key && keyValuePair.value) {
      ...
      // For 'in' operator with keyValue, we need special handling
      if (operator === 'in' || operator === 'notIn') {
        return `${fieldPath} ${celOperator} ${actualValue}`;
      }
...
/**
 * Convert CEL expression to rule group (inverse operation)
 * This is a simplified parser - handles basic expressions
 */
export function convertCELToRuleGroup(celExpression: string): RuleGroupType | undefined {
  if (!celExpression || !celExpression.trim()) {
    return undefined;
  }

  // This is a simplified implementation
  // For complex nested expressions, this may not work perfectly
  // In a real implementation, you'd use a proper CEL parser
  ...
  // For now, return the default if expression exists
  // The full implementation would parse the CEL expression back into rules
