 ...
 *
 * Enterprise deployments can replace the Bifrost logo and icon. The read
 * endpoint is public (the login screen renders before any session exists); the
 * write endpoints only exist on the enterprise build, so on OSS the mutations
 * below resolve to 404 and every surface keeps the bundled defaults.
 *
