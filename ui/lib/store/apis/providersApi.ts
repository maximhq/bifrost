...
export const providersApi = baseApi.injectEndpoints({
	endpoints: (builder) => ({
		...
		getModelDetails: builder.query<ListModelDetailsResponse, { query?: string; provider?: string; limit?: number; unfiltered?: boolean }>({
			query: ({ query, provider, limit, unfiltered }) => {
				const params = new URLSearchParams();
				if (query) params.append("query", query);
				if (provider) params.append("provider", provider);
