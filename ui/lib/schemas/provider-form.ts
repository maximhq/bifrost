...
const KeySchema = z.object({
	...
	weight: z.number().min(0, "Key weights must be between 0 and 1").max(1, "Key weights must be between 0 and 1"),
...
export const ProviderFormSchema = z
	.object({
		...
		baseProviderType: z.enum([...KNOWN_PROVIDERS, ""]).optional(),
	...
	.superRefine((data, ctx) => {
		...
		const isCustomProvider =
			...
			!KNOWN_PROVIDERS.includes(data.selectedProvider as (typeof KNOWN_PROVIDERS)[number]);
		...
		if (isCustomProvider) {
			...
			if (KNOWN_PROVIDERS.includes(data.customProviderName?.trim() as (typeof KNOWN_PROVIDERS)[number])) {
				ctx.addIssue({
					code: z.ZodIssueCode.custom,
