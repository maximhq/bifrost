...
// Provider form data schema - matches the ProviderFormData interface exactly
export const providerFormDataSchema = z.object({
	provider: formModelProviderSchema,
	keys: z
		.array(
			z.object({
	...
	network_config: z
		.object({
			...
			defaultRequestTimeoutInSeconds: z
				.number()
				.min(1, "Timeout must be greater than 0 seconds")
				.max(300, "Timeout must be less than 300 seconds"),
			maxRetries: z.number().min(0, "Max retries must be greater than 0").max(10, "Max retries must be less than 10"),
		})
		.optional(),
	concurrency_and_buffer_size: z
		.object({
			concurrency: z.number().min(1, "Concurrency must be greater than 0").max(100, "Concurrency must be less than 100"),
			bufferSize: z.number().min(1, "Buffer size must be greater than 0").max(1000, "Buffer size must be less than 1000"),
		})
		.optional(),
	custom_provider_config: formCustomProviderConfigSchema.optional(),
});
...
export type ProviderFormDataSchema = z.infer<typeof providerFormDataSchema>;
