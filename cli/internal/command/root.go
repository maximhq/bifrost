...
func (r *Runner) Run(ctx context.Context, args []string) error {
	...
	case "context":
		...
		return ignoreFlagHelp(r.runContext(commandArgs, format))
	case "completion":
	...
	if name == "api" && (len(commandArgs) == 0 || commandArgs[0] != "call") {
		...
		return ignoreFlagHelp(r.runAPI(ctx, &environment{Output: format}, commandArgs))
	}
	if name == "resources" {
		format, formatErr := output.ParseFormat(globals.Output)
		if formatErr != nil {
		...
		return ignoreFlagHelp(r.runResources(format, commandArgs))
	}
