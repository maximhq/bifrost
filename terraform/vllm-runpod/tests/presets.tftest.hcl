...
run "gpt_oss_defaults" {
  ...
  assert {
    condition     = join(" ", local.vllm_args) == "openai/gpt-oss-20b --served-model-name gpt-oss-20b --host 0.0.0.0 --port 8000 --tensor-parallel-size 1 --gpu-memory-utilization 0.92 --max-model-len 131072 --enable-auto-tool-choice --tool-call-parser openai"
...
run "overrides_disable_parsers_and_append_args" {
  ...
  assert {
    condition     = join(" ", local.vllm_args) == "Qwen/Qwen3.6-35B-A3B --served-model-name qwen3.6-35b-a3b --host 0.0.0.0 --port 8000 --tensor-parallel-size 2 --gpu-memory-utilization 0.92 --max-model-len 32768 --enable-auto-tool-choice --tool-call-parser qwen3_coder --kv-cache-dtype fp8"
    error_message = "unexpected args: ${join(" ", local.vllm_args)}"
  }
  assert {
...
run "custom_preset" {
  ...
  assert {
    condition     = join(" ", local.vllm_args) == "ByteDance-Seed/Seed-OSS-36B-Instruct --served-model-name seed-oss-36b --host 0.0.0.0 --port 8000 --tensor-parallel-size 1 --gpu-memory-utilization 0.92 --max-model-len 32768 --enable-auto-tool-choice --tool-call-parser seed_oss"
    error_message = "unexpected args: ${join(" ", local.vllm_args)}"
  }
  assert {
