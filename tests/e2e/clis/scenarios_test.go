...
func validateFahrenheitReading(output string) error {
	if regexp.MustCompile(`(?i)-?\d+(?:\.\d+)?\s*(?:°\s*F\b|º\s*F\b|\bF\b|degrees?\s+F(?:ahrenheit)?\b|\bFahrenheit\b)`).MatchString(output) {
		return nil
	}
...
func validateTripSplit(output string) error {
	if !regexp.MustCompile(`\b2\b`).MatchString(output) {
