...
test.describe("LLM Logs", () => {
	...
	test.describe("Log Filtering", () => {
		test("should filter logs by provider", async ({ logsPage }) => {
			...
			if (!isVisible || SAMPLE_PROVIDERS.length < 1) {
				test.skip(!isVisible || SAMPLE_PROVIDERS.length < 1, "Provider filter not visible or no sample providers");
				return;
			}
		...
		test("should filter logs by model", async ({ logsPage }) => {
			...
			if (!isVisible || SAMPLE_MODELS.length < 1) {
				test.skip(!isVisible || SAMPLE_MODELS.length < 1, "Model filter not visible or no sample models");
				return;
			}
		...
		test("should filter logs by status", async ({ logsPage, page }) => {
			...
			await expect.poll(() => page.url(), { timeout: 5000, intervals: [200, 300, 500] }).toMatch(/status=success/);
		});
	...
	test.describe("Log Deletion", () => {
		test("should delete multiple logs at once", async ({ logsPage }) => {
			const initialCount = await logsPage.getLogCount();

			if (initialCount < 2) {
				test.skip(true, "Need at least two logs to test bulk delete");
				return;
			}

			const deletedCount = await logsPage.deleteFirstVisibleLogs(2);
			expect(deletedCount).toBe(2);

			await expect.poll(() => logsPage.getLogCount(), { timeout: 10000 }).toBe(initialCount - deletedCount);
		});

		test("should refill the table after deleting a full visible page", async ({ logsPage }) => {
			await logsPage.gotoWithSmallPageSize(5);

			const nextEnabled = await logsPage.nextPageBtn
				.first()
				.isEnabled()
				.catch(() => false);
			if (!nextEnabled) {
				test.skip(true, "Need more than one page of logs to verify page refill after delete");
				return;
			}

			const deletedCount = await logsPage.deleteFirstVisibleLogs(5);
			expect(deletedCount).toBe(5);

			await expect.poll(() => logsPage.getLogCount(), { timeout: 10000 }).toBeGreaterThan(0);

			await expect(logsPage.page.locator("text=/No results found/i")).not.toBeVisible();
		});
	});
	...
	test.describe("Pagination", () => {
		test("should navigate to next page", async ({ logsPage }) => {
			...
			await expect.poll(() => logsPage.getCurrentPageNumber(), { timeout: 5000 }).toBe(initialPage + 1);
		});
	...
	test.describe("Empty State", () => {
		test("should show empty state when no logs", async ({ logsPage }) => {
			...
			await expect(logsPage.page.locator("text=/No results found|No logs found/i")).toBeVisible({ timeout: 10000 });
		});
	});
	...
	test.describe("URL State Persistence", () => {
		test("should persist filters in URL", async ({ logsPage }) => {
			...
			await expect.poll(() => logsPage.page.url(), { timeout: 8000, intervals: [300, 500, 500] }).toContain("content_search=");
		...
		test("should restore state from URL", async ({ logsPage, page }) => {
			...
			await expect.poll(() => page.url(), { timeout: 5000, intervals: [200, 300, 500] }).toMatch(/start_time=\d+/);
			const url = page.url();
