...
export class LogsPage extends BasePage {
	...
	/**
	 * Disable live updates so row counts stay stable during destructive actions.
	 */
	async disableLiveUpdatesIfEnabled(): Promise<void> {
		const liveToggleVisible = await this.liveToggle
			.first()
			.isVisible()
			.catch(() => false);
		if (!liveToggleVisible) return;

		const liveDisabled = this.page.url().includes("live_enabled=false");
		if (liveDisabled) return;

		await this.toggleLiveUpdates();
		await this.page.waitForURL(/live_enabled=false/, { timeout: 5000 });
		await waitForNetworkIdle(this.page);
	}
	...
	/**
	 * Delete the first N visible logs using the bulk-delete action.
	 * Returns the number of logs it actually attempted to delete.
	 */
	async deleteFirstVisibleLogs(count: number): Promise<number> {
		if (count <= 0) return 0;

		await this.disableLiveUpdatesIfEnabled();
		await this.dismissToasts();

		const checkboxes = this.page.locator('[data-testid^="logs-checkbox-"]');
		const availableCount = await checkboxes.count();
		const deleteCount = Math.min(count, availableCount);
		...
		const bulkDeleteBtn = this.page.getByTestId("logs-bulk-delete-btn");
		await bulkDeleteBtn.waitFor({ state: "visible", timeout: 5000 });
		await bulkDeleteBtn.click();

		const confirmBtn = this.page.getByTestId("logs-confirm-bulk-delete-btn");
	...
	async selectAllLogs(): Promise<void> {
		await this.disableLiveUpdatesIfEnabled();
		const selectAllCheckbox = this.page.getByTestId("logs-select-all-checkbox");
		await selectAllCheckbox.waitFor({ state: "visible", timeout: 5000 });
		await selectAllCheckbox.click();
		await expect(this.page.getByTestId("logs-bulk-delete-btn")).toBeVisible({ timeout: 5000 });
	}

	async isAnyVisibleLogSelected(): Promise<boolean> {
		const checkboxes = this.page.locator('[data-testid^="logs-checkbox-"]');
		const count = await checkboxes.count();

		for (let index = 0; index < count; index += 1) {
			const checkbox = checkboxes.nth(index);
			const checked = await checkbox.getAttribute("data-state").catch(() => null);
			if (checked === "checked") {
				return true;
			}
		}
	...
	async getVisibleBulkSelectedCount(): Promise<number> {
		const checkboxes = this.page.locator('[data-testid^="logs-checkbox-"]');
		const count = await checkboxes.count();
		let selectedCount = 0;
