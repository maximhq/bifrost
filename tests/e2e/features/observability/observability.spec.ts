...
test.describe('Observability', () => {
  ...
  test.describe('Maxim Connector', () => {
    ...
    test('should display Maxim configuration form', async ({ observabilityPage }) => {
      ...
      // Should see some form elements for configuration
      const formVisible = await observabilityPage.page.locator('form').isVisible().catch(() => false)
      const inputsVisible = await observabilityPage.page.locator('input').first().isVisible().catch(() => false)
      const switchVisible = await observabilityPage.page.locator('button[role="switch"]').isVisible().catch(() => false)
      
      expect(formVisible || inputsVisible || switchVisible).toBe(true)
    })
  })
