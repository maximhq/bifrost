...
export class VirtualKeysPage extends BasePage {
  ...
  async deleteVirtualKey(
    ...
    const deleteResponse = await deleteResponsePromise.catch((err) => {
      console.warn(
        `[deleteVirtualKey] No DELETE response captured for "${name}": ${err}`,
      );
      return null;
    });
    if (deleteResponse && !deleteResponse.ok()) {
      console.warn(
        `[deleteVirtualKey] DELETE responded with ${deleteResponse.status()} for "${name}"`,
      );
    }
