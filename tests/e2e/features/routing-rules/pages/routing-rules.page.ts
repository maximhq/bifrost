...
import { waitForNetworkIdle, fillSelect } from '../../../core/utils/test-helpers'
...
export class RoutingRulesPage extends BasePage {
  ...
  async addFallbackProvider(provider: string, model?: string): Promise<void> {
    ...
    if (isVisible) {
      ...
      const fallbackInput = this.sheet.locator('input[placeholder*="fallback"]').or(
        this.sheet.locator('input').last()
      )
  ...
  async duplicateRule(name: string): Promise<string | null> {
    ...
    // Return the new rule name (typically original name + " (copy)")
    return `${name} (copy)`
  }
  ...
  async getRulePriority(name: string): Promise<number | null> {
    ...
    // Priority is typically shown in one of the table cells
    const cells = row.locator('td')
    const count = await cells.count()
    
    // Look for a cell that contains a number (priority column)
