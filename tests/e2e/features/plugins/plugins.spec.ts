...
test.describe('Plugins', () => {
  ...
  test.describe('Form Validation', () => {
    test('should require name for plugin', async ({ pluginsPage }) => {
      ...
      // Check if save button is disabled without name
      const isSaveDisabled = await pluginsPage.saveBtn.isDisabled().catch(() => false)

      if (isSaveDisabled) {
        expect(isSaveDisabled).toBe(true)
      } else {
        // If save is enabled, click and verify form doesn't submit
        await pluginsPage.saveBtn.click()
        const sheetStillVisible = await pluginsPage.sheet.isVisible()
        const hasError = await pluginsPage.page.locator('[role="alert"], .text-destructive').count() > 0
