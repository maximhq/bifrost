...
- [x] **Inbound `reasoning_details` replay on the NATIVE OpenAI-compatible surface** (folder 61: `/v1/chat/completions` assistant history turn replaying `reasoning_details[].text` — the shape Bifrost itself returns and a faithful client (pi, OpenRouter-style callers) sends back. Upstream `dd927388d` normalized all three spellings on the `/openai/*` ingress only; native ingress parses straight into `schemas.ChatMessage`, so `ChatAssistantMessage.UnmarshalJSON` left `Reasoning` nil and `ConvertBifrostMessagesToOpenAIMessages` — which carries reasoning outbound only as `reasoning_content`, `reasoning_details` being inbound-only there — dropped the chain of thought before the provider call. No error anywhere: multi-turn tool loops ran with no prior `think` block and the model confabulated (invented tool results, skipped cases). DeepSeek rows pin it over plain HTTP because its endpoint only forces `thinking:disabled` when an assistant tool_call turn carries no replayable reasoning, so a dropped replay is visible as "no reasoning came back" without `allow_per_request_raw_override`; the encrypted/empty-detail rows are the controls proving the fold reads plaintext only (a false-positive fold keeps thinking on and DeepSeek 400s). The OpenAI row asserts `extra_fields.raw_request.messages[].reasoning_content` directly and returns early where per-request capture is off. Distinct from folder 39 (which pins Anthropic-origin replay *acceptance* and the OpenAI→Anthropic reverse direction) and folder 60 (a *foreign signature* refusal).)
