...
- **Requests suffixed `(Coverage Probe)`** pass on any status. Use that suffix
  only when the outcome depends on deployment shape rather than on correctness
  — an unconfigured vault, a missing background job runner, a webhook receiver
  that is deliberately not listening.
