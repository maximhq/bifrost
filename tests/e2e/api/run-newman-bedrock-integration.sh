...
# Configuration
COLLECTION="bifrost-bedrock-integration.postman_collection.json"
...
run_newman() {
    ...
    if [ -n "${2:-}" ] && [ -f "${2}" ]; then
        ...
        if [[ "${1:-}" == "bedrock" ]]; then
            [ -n "${BIFROST_BEDROCK_API_KEY:-}" ] && cmd="$cmd --env-var \"bedrock_api_key=$BIFROST_BEDROCK_API_KEY\""
            [ -n "${BIFROST_BEDROCK_ACCESS_KEY:-}" ] && cmd="$cmd --env-var \"bedrock_access_key=$BIFROST_BEDROCK_ACCESS_KEY\""
            [ -n "${BIFROST_BEDROCK_SECRET_KEY:-}" ] && cmd="$cmd --env-var \"bedrock_secret_key=$BIFROST_BEDROCK_SECRET_KEY\""
            [ -n "${BIFROST_BEDROCK_REGION:-}" ] && cmd="$cmd --env-var \"bedrock_region=$BIFROST_BEDROCK_REGION\""
            [ -n "${BIFROST_BEDROCK_SESSION_TOKEN:-}" ] && cmd="$cmd --env-var \"bedrock_session_token=$BIFROST_BEDROCK_SESSION_TOKEN\""
        fi
    else
