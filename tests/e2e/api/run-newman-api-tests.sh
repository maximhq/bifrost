...
# Configuration
COLLECTION="bifrost-api-management.postman_collection.json"
REPORT_DIR="newman-reports/api-management"
...
if [[ "$REPORTERS" == *"html"* ]]; then
    CMD="$CMD --reporter-html-export $REPORT_DIR/report.html"
fi

if [[ "$REPORTERS" == *"json"* ]]; then
    CMD="$CMD --reporter-json-export $REPORT_DIR/report.json"
fi
