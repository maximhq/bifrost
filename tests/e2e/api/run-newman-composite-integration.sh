...
# Configuration
COLLECTION="bifrost-composite-integrations.postman_collection.json"
...
for jsonfile in "${PROVIDER_JSON_FILES[@]}"; do
    ...
    LOG_FILES+=("$logfile")
    NAMES+=("$name")
    ( run_newman "$name" "$jsonfile" > "$temp_logfile" 2>&1; post_process_log "$temp_logfile" "$logfile"; rm -f "$temp_logfile" ) &
    PIDS+=($!)
done
