...
// where nothing carries a [provider] prefix: both the denominator (which leaf
// belongs to which provider) and the CLI lines have to be attributed by keyword.
...
function matchProvider(haystack) {
...
// Which provider owns this leaf?
//
// The URL is tried first and on its own, because that is the only signal the
// CLI log also carries - the numerator is attributed from newman's "POST
// http://host/path" line. Matching the denominator on the full item JSON would
// disagree on every criss-cross row (an /openai/... endpoint carrying an
// anthropic model in the body lands in "anthropic" by body, "openai" by URL),
// and a provider whose numerator and denominator come from different rules can
// report more passes than it has requests.
//
// The request name is tried second, for the same reason: cross-cut rows route
// through a unified /v1/chat/completions that names no provider, and the name
// ("openai/gpt-4o-mini ...") is the only other thing the log prints.
//
// Item JSON + ancestor folder names, as in filter-collection.mjs, is the last
// resort - rows the log cannot attribute at all. Those inflate a denominator
// the numerator can never reach, which is the safe direction to be wrong in:
// the table under-reports progress rather than over-reporting passes.
function providerOfItem(item, ancestors) {
  ...
  return (
    ...
    matchProvider(
      stripBase64Blobs(JSON.stringify(item) + " " + ancestors.join(" ")).toLowerCase()
    )
  );
}
  ...
  for (const path of candidates) {
    ...
    try {
      ...
      walkLeaves(data.item || [], [], (node, ancestors) => {
        const p = providerOfItem(node, ancestors);
...
function inferProviderFromLine(line) {
  return matchProvider(line.toLowerCase());
}
