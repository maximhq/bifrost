...
// var -> name of the FIRST request that sets it (collection order). First rather than last
// because the earliest producer is the one guaranteed to have run before every consumer.
export function buildProducerIndex(entries) {
  const index = new Map();
  for (const { item } of entries) {
    for (const name of varsSetBy(item)) {
      if (!index.has(name)) index.set(name, item.name);
    }
  }
  return index;
}
...
// Returns [{ variable, producer }] sorted by variable name for stable output.
export function bodyDependencies(item, producerIndex) {
  ...
  for (const m of rawBodyOf(item).matchAll(TEMPLATE_RE)) {
    ...
    if (producer && producer !== item.name) deps.set(name, producer);
  }
  return [...deps.entries()]
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([variable, producer]) => ({ variable, producer }));
}
