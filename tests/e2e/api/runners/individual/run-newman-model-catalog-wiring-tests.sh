...
# Bifrost Model Catalog Wiring Test Runner
...
COLLECTION="collections/bifrost-model-catalog-wiring.postman_collection.json"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Bifrost Model Catalog Wiring Tests${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
# Forward the run-id prefix and the full per-provider credential set. The
# current scenarios only exercise openai, but the rest are forwarded so adding
# scenarios for other providers needs no runner change.
for v in \
    e2e_seed_prefix \
    openai_api_key anthropic_api_key gemini_api_key cohere_api_key \
    groq_api_key mistral_api_key openrouter_api_key replicate_api_key \
    parasail_api_key cerebras_api_key fireworks_api_key deepseek_api_key \
    huggingface_api_key perplexity_api_key sambanova_api_key voyage_api_key \
    xai_api_key elevenlabs_api_key \
    azure_endpoint azure_api_key azure_api_version \
    bedrock_aws_access_key_id bedrock_aws_secret_access_key bedrock_region \
    vertex_credentials_json vertex_project_id vertex_region \
    ; do
...
set +e
"${cmd[@]}" --reporter-htmlextra-export "$REPORT_DIR/report.html" --reporter-htmlextra-title "Bifrost Model Catalog Wiring"
EXIT_CODE=$?
set -e
