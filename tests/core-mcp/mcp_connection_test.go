...
func TestLocalToolRegistration(t *testing.T) {
	...
	// Find the bifrost-internal client
	var bifrostClient *schemas.MCPClient
	for i := range clients {
		if clients[i].Config.ID == "bifrost-internal" {
			bifrostClient = &clients[i]
			break
		}
	}

	require.NotNil(t, bifrostClient, "bifrost-internal client should exist")
...
func TestToolExecutionError(t *testing.T) {
	...
	// Error tool should return an error
