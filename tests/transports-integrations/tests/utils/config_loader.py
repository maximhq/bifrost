...
class ConfigLoader:
    ...
    def get_integration_url(self, integration: str, use_fallback: bool = False) -> str:
        """Get the complete URL for a integration

        Args:
            integration: Integration name (openai, anthropic, google, litellm)
            use_fallback: If True, use direct integration URL instead of Bifrost
        ...
        Examples:
            ...
            get_integration_url("anthropic", use_fallback=True) -> "https://api.anthropic.com"
        """
        if use_fallback:
            return self._config["fallback_urls"].get(integration, "")
    ...
    def print_config_summary(self):
        ...
        for integration, models in self._config["models"].items():
            ...
            print(f"    Streaming: {models['streaming']}")
...
def get_integration_url(integration: str, use_fallback: bool = False) -> str:
    """Convenience function to get integration URL"""
    return get_config().get_integration_url(integration, use_fallback)
