...
def assert_valid_cohere_embedding_response(response, expected_count: int, expected_dimensions: int | None = None):
    ...
    assert response.embeddings.float_ is not None, "Response embeddings should have float vectors"
...
class TestCohereIntegration:
    ...
    def test_01_single_text_embedding(self, test_config, provider, model):
        ...
        print(f"✓ Single text embedding: provider={provider} dims={len(response.embeddings.float_[0])}")
    ...
    def test_02_batch_text_embeddings(self, test_config, provider, model):
        ...
        print(f"✓ Batch text embeddings: provider={provider} count=3 dims={len(response.embeddings.float_[0])}")
    ...
    @pytest.mark.parametrize(
        "provider,model",
        get_cross_provider_params_for_scenario("embeddings", include_providers=["cohere", "bedrock"]),
    )
    def test_07_multiple_embedding_types(self, test_config, provider, model):
        ...
        int8_vectors = response.embeddings.int8
        assert len(int8_vectors) == 1, (
            f"Expected 1 int8 embedding, got {len(int8_vectors)}"
        )
        # int8 and float are distinct representations, so an empty or float-valued
        # int8 vector means the encoding was lost rather than returned alongside.
        assert len(int8_vectors[0]) > 0, "int8 embedding should not be empty"
        assert all(isinstance(v, int) for v in int8_vectors[0]), (
            "int8 embedding values should be integers"
        )
        assert all(-128 <= v <= 127 for v in int8_vectors[0]), (
            "int8 embedding values should fit the signed int8 range"
        )
    ...
    def test_09_image_embedding(self, test_config, provider, model):
        ...
        assert_valid_cohere_embedding_response(response, expected_count=1)
        print(f"✓ Image embedding: provider={provider} dims={len(response.embeddings.float_[0])}")
    ...
    def test_10_multimodal_mixed_inputs_embedding(self, test_config, provider, model):
        ...
        assert_valid_cohere_embedding_response(response, expected_count=1)
        print(f"✓ Multimodal mixed inputs embedding: provider={provider} dims={len(response.embeddings.float_[0])}")
