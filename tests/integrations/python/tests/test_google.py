...
Joe: Pretty good, thanks for asking."""
    ...
    @pytest.mark.parametrize("provider,model", get_cross_provider_params_for_scenario("multimodal_embeddings"))
    def test_52_retrieval_query_prompt(self, test_config, provider, model):
        """Test Case 52: Retrieval-query prompt produces a valid embedding (embedding-2 ignores task_type)."""
        if provider == "_no_providers_" or model == "_no_model_":
            pytest.skip("No providers configured for multimodal_embeddings scenario")
        ...
        response = client.models.embed_content(
            model=format_provider_model(provider, model),
            contents="task: search result | query: What are the best practices for machine learning?",
        ...
        print(f"✓ Retrieval query embedding: provider={provider} dims={len(values)}")

    @pytest.mark.parametrize("provider,model", get_cross_provider_params_for_scenario("multimodal_embeddings"))
    def test_53_retrieval_document_prompt(self, test_config, provider, model):
        """Test Case 53: Retrieval-document prompt produces a valid embedding (embedding-2 ignores task_type)."""
        if provider == "_no_providers_" or model == "_no_model_":
            pytest.skip("No providers configured for multimodal_embeddings scenario")
        ...
        response = client.models.embed_content(
            model=format_provider_model(provider, model),
            contents="title: none | text: Machine learning best practices include data preprocessing, "
        ...
        print(f"✓ Retrieval document embedding: provider={provider} dims={len(values)}")
