...
class TestOpenAIIntegration:
    ...
    def test_26_embedding_long_text(self, test_config, provider, model, vk_enabled):
        ...
        if (
            provider != "gemini" and provider != "bedrock"
        ):  # gemini does not return usage data and openai does not return usage data for long text
    ...
    def test_56_web_search_wildcard_domains(self, provider, model, vk_enabled):
        ...
        response = client.responses.create(
            model=format_provider_model(provider, model),
            tools=[
                {"type": "web_search", "allowed_domains": ["wikipedia.org", "en.wikipedia.org"]}
            ],
