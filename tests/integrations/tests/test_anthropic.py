...
class TestAnthropicIntegration:
    ...
    def test_24_batch_retrieve(self, anthropic_client, test_config, provider, model):
        ...
        try:
            ...
            requests = create_batch_inline_requests(model=model, num_requests=1, provider=provider, sdk="anthropic")
            batch = client.beta.messages.batches.create(requests=requests)
            batch_id = batch.id
    ...
    def test_25_batch_cancel(self, anthropic_client, test_config, provider, model):
        ...
        try:
            ...
            requests = create_batch_inline_requests(model=model, num_requests=1, provider=provider)
            batch = client.beta.messages.batches.create(requests=requests)
            batch_id = batch.id
    ...
    def test_27_batch_e2e(self, anthropic_client, test_config, provider, model):
        ...
        requests = create_batch_inline_requests(model=model, num_requests=2, provider=provider, sdk="anthropic")
