...
def create_bedrock_batch_jsonl(model_id: str, num_requests: int = 2) -> str:
    ...
    for i in range(num_requests):
        record = {
            ...
            "modelInput": {
                "messages": [
                    {"role": "user", "content": [{"text": f"Hello, this is test message {i+1}. Say hi back briefly."}]}
                ],
                "inferenceConfig": {
                    "maxTokens": 100
                }
...
class TestBedrockIntegration:
    ...
    def test_11_file_upload(self, test_config, provider, model):
        ...
        if provider != "openai":
            pytest.skip("Only openai supports file upload")
        ...
        print(f"Success: Uploaded file with ID: {file_id} to {s3_uri} for provider {provider}")
    ...
    def test_13_file_retrieve(self, test_config, provider, model):
        ...
        try:
            ...
            print(f"Success: Retrieved metadata for s3://{s3_bucket}/{s3_key}, file ID: {head_file_id}, size: {response['ContentLength']} bytes (provider: {provider})")
    ...
    def test_14_file_delete(self, test_config, provider, model):
        ...
        try:
            ...
            print(f"Success: Deleted file s3://{s3_bucket}/{s3_key} (provider: {provider})")
    ...
    def test_15_file_content(self, test_config, provider, model):
        ...
        try:
            ...
            assert jsonl_content == downloaded_content, "Downloaded content should match uploaded content"

            # Verify ETag contains file ID
            get_file_id = response.get('ETag', '').strip('"')
            assert get_file_id, "GET response should contain file ID in ETag"
            print(f"Success: Downloaded {len(downloaded_content)} bytes from s3://{s3_bucket}/{s3_key}, file ID: {get_file_id} (provider: {provider})")
    ...
    @pytest.mark.parametrize("provider,model", get_cross_provider_params_for_scenario("batch_file_upload"))
    def test_16_batch_create_with_file(self, test_config, provider, model):
        ...
        print(f"Uploaded input file with ID: {file_id} to {input_uri}")
    ...
    def test_17_batch_list(self, test_config, provider, model):
        ...
        try:
            ...
            assert "invocationJobSummaries" in response, "Response should contain invocationJobSummaries"
            print(f"Success: Listed {len(response['invocationJobSummaries'])} batch jobs for provider {provider}")
    ...
    def test_18_batch_retrieve(self, test_config, provider, model):
        ...
        try:
            ...
            print(f"Success: Retrieved job {response['jobArn']} with status {response['status']} for provider {provider}")
    ...
    def test_19_batch_cancel(self, test_config, provider, model):
        ...
        jsonl_content = create_bedrock_batch_jsonl(model, num_requests=5)
        ...
        try:
            ...
            create_response = bedrock_client.create_model_invocation_job(
                ...
                outputDataConfig={
                    "s3OutputDataConfig": {"s3Uri": f"s3://{s3_bucket}/bifrost-batch-cancel-output/"}
                }
            )
            ...
            print(f"Success: Cancelled job {job_arn} for provider {provider}")
    ...
    def test_20_batch_e2e(self, test_config, provider, model):
        ...
        file_id = upload_response.get('ETag', '').strip('"')
        input_uri = f"s3://{s3_bucket}/{s3_key}"
        print(f"Step 1: Uploaded input file with ID: {file_id} to {input_uri} for provider {provider}")
