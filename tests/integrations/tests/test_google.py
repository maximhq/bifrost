...
def assert_valid_google_file_response(response, expected_display_name: str = None) -> None:
    """
...
def assert_valid_google_batch_response(response, expected_state: str = None) -> None:
    """
...
Joe: Pretty good, thanks for asking."""
    ...
    def test_30_batch_create_with_file(self, test_config, provider, model):
        ...
        try:
            ...
            # Google batch API uses the display_name prefixed with "files/"
            batch_job = client.batches.create(
                model=format_provider_model(provider, model),
                src=f"files/{uploaded_file.display_name}",
            )
