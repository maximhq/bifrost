...
func RunImageURLTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig config.ComprehensiveTestConfig) {
	...
	t.Run("ImageURL", func(t *testing.T) {
		...
		// Base request configuration (input will be set per API)
		...
		expectations := VisionExpectations([]string{"ant"}) // Primary expectation: should identify it as an ant
		...
		expectations.ShouldNotContainWords = []string{"cannot see", "unable to view", "no image"} // Vision failure indicators
