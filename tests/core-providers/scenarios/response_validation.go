...
func validateBasicStructure(t *testing.T, response *schemas.BifrostResponse, expectations ResponseExpectations, result *ValidationResult) {
	...
	if expectations.ExpectedChoiceCount > 0 {
		...
		if response.Output != nil {
			actualCount = len(response.Output)
		}
...
func validateToolCalls(t *testing.T, response *schemas.BifrostResponse, expectations ResponseExpectations, result *ValidationResult) {
	...
	for _, choice := range choices {
		if choice.Message.AssistantMessage != nil && choice.Message.AssistantMessage.ToolCalls != nil {
			totalToolCalls += len(*choice.Message.AssistantMessage.ToolCalls)
		}
	}
...
func validateSpecificToolCalls(response *schemas.BifrostResponse, expectedCalls []ToolCallExpectation, result *ValidationResult) {
	...
	for _, expected := range expectedCalls {
		...
		// Look for the expected tool call across all choices
		for choiceIdx, choice := range choices {
			if choice.Message.AssistantMessage != nil && choice.Message.AssistantMessage.ToolCalls != nil {
				toolCalls := *choice.Message.AssistantMessage.ToolCalls

				for callIdx, toolCall := range toolCalls {
					if toolCall.Function.Name != nil && *toolCall.Function.Name == expected.FunctionName {
						found = true

						// Validate this specific tool call
						validateSingleToolCall(toolCall.Function.Arguments, expected, choiceIdx, callIdx, result)
						break
					}
				}
		...
		for _, message := range messages {
			if message.Name != nil && *message.Name == expected.FunctionName {
				arguments := *message.ResponsesAPIExtendedBifrostMessage.Arguments
				found = true
...
func validateTechnicalFields(t *testing.T, response *schemas.BifrostResponse, expectations ResponseExpectations, result *ValidationResult) {
	...
	if expectations.ShouldHaveTimestamps {
		if (response.ResponseAPIExtendedResponse == nil && response.Created == 0) || (response.ResponseAPIExtendedResponse != nil && response.ResponseAPIExtendedResponse.CreatedAt == 0) {
...
func collectResponseMetrics(response *schemas.BifrostResponse, result *ValidationResult) {
	...
	if response.ChatCompletionsExtendedResponse != nil {
		choiceCount = len(response.ChatCompletionsExtendedResponse.Choices)
	}
	result.MetricsCollected["choice_count"] = choiceCount
