...
func RunSimpleChatTest(t *testing.T, client *bifrost.Bifrost, ctx context.Context, testConfig config.ComprehensiveTestConfig) {
	...
	t.Run("SimpleChat", func(t *testing.T) {
		...
		// Base request configuration (will be adapted for both APIs)
		...
		expectations.ShouldContainKeywords = []string{"paris"}                      // Should mention Paris as the capital
		expectations.ShouldNotContainWords = []string{"berlin", "london", "madrid"} // Common wrong answers
