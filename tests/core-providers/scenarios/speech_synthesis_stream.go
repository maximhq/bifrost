				...
				for {
					...
					case response, ok := <-responseChannel:
						...
						if response.BifrostResponse.Speech.Audio != nil {
							...
							if response.BifrostResponse.Model != "" && response.BifrostResponse.Model != "gpt-4o-mini-tts" {
		...
		t.Run("LongText_HDModel_Streaming", func(t *testing.T) {
			...
			for {
				...
				case response, ok := <-responseChannel:
					...
					if response.BifrostResponse != nil && response.BifrostResponse.Model != "" && response.BifrostResponse.Model != "gpt-4o-mini-tts" {
