...
func (c *EmptyResponseCondition) ShouldRetry(response *schemas.BifrostResponse, err *schemas.BifrostError, context TestRetryContext) (bool, string) {
	...
	if response.ChatCompletionsExtendedResponse == nil && response.ResponseAPIExtendedResponse == nil {
...
func (c *MalformedToolArgsCondition) ShouldRetry(response *schemas.BifrostResponse, err *schemas.BifrostError, context TestRetryContext) (bool, string) {
	...
	for i, toolCall := range toolCalls {
		...
		// Check for empty arguments when some are expected
		if len(args) == 0 && toolCall.Name != "" {
			return true, fmt.Sprintf("tool call %d (%s) has empty arguments", i, toolCall.Name)
		}
	}
...
func (c *IncompleteStreamCondition) ShouldRetry(response *schemas.BifrostResponse, err *schemas.BifrostError, context TestRetryContext) (bool, string) {
	...
	if response.ChatCompletionsExtendedResponse != nil {
		for i, choice := range response.ChatCompletionsExtendedResponse.Choices {
	...
	if response.ResponseAPIExtendedResponse != nil {
		for i, output := range response.ResponseAPIExtendedResponse.Output {
			if output.ResponsesAPIExtendedBifrostMessage != nil {
				if output.ResponsesAPIExtendedBifrostMessage.Status == nil {
					return true, fmt.Sprintf("output %d has no status (stream may be incomplete)", i)
				}

				status := *output.ResponsesAPIExtendedBifrostMessage.Status
